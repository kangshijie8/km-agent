"""
Kunming MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Kunming-specific extra)

Usage:
    kunming mcp serve
    kunming mcp serve --verbose
...
logger = logging.getLogger("kunming.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using KUNMING_HOME."""
    try:
        from kunming_constants import get_kunming_home
        return get_kunming_home() / "sessions"
    except ImportError:
        from kunming_constants import get_kunming_home as _gkh
        return Path(os.environ.get("KUNMING_HOME", str(_gkh()))) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from kunming_constants import get_kunming_home
        directory_file = get_kunming_home() / "channel_directory.json"
    except ImportError:
        from kunming_constants import get_kunming_home as _gkh
        directory_file = Path(
            os.environ.get("KUNMING_HOME", str(_gkh()))
...
class EventBridge:
    ...
    WebSocket gateway bridge for MCP tool integration.
    ...
    def _poll_once(self, db):
        ...
        try:
            from kunming_constants import get_kunming_home
            db_file = get_kunming_home() / "state.db"
        except ImportError:
            from kunming_constants import get_kunming_home as _gkh
            db_file = Path(os.environ.get("KUNMING_HOME", str(_gkh()))) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Kunming MCP server with all tools registered."""
    if not _MCP_SERVER_AVAILABLE:
        raise ImportError(
            "MCP server requires the 'mcp' package. "
            "Install with: pip install 'kunming-agent[mcp]'"
        )

    mcp = FastMCP(
        "kunming",
        instructions=(
            "Kunming Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Kunming MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
