...
title: "Git Worktrees"
description: "Run multiple Kunming agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Kunming Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Kunming so each session has a clean, isolated working directory.

## Why Use Worktrees with Kunming?

Kunming treats the **current working directory** as the project root:

- CLI: the directory where you run `km chat`
...
git worktree add ../repo-feature feature/kunming-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/kunming-experiment` checked out in that directory

Now you can `cd` into the new worktree and run Kunming there:

```bash
cd ../repo-feature

# Start Kunming in the worktree
km chat
```
...
git worktree add ../repo-experiment-a feature/kunming-a
git worktree add ../repo-experiment-b feature/kunming-b
```
...
Each Kunming process:

- Works on its own branch (`feature/kunming-a` vs `feature/kunming-b`).
...
- Kunming checkpoint data under `~/.kunming/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Kunming experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/kunming-checkpoints-docs`, `feature/kunming-refactor-tests`.
- **Commit frequently**
...
- **Avoid running Kunming from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `km chat -w` (Automatic Worktree Mode)

Kunming has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `kunming/kunming-<hash>`).
...
```bash
km chat -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `km chat -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Kunming session its own clean checkout.
