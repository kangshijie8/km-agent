...
Kunming has two hook systems that run custom code at key lifecycle points:

| System | Registered via | Runs in | Use case |
|--------|---------------|---------|----------|
| **[Gateway hooks](#gateway-event-hooks)** | `HOOK.yaml` + `handler.py` in `~/.kunming/hooks/` | Gateway only | Logging, alerts, webhooks |
...
Each hook is a directory under `~/.kunming/hooks/` containing two files:
...
LOG_FILE = Path.home() / ".kunming" / "hooks" / "my-hook" / "activity.log"
...
The gateway ships with a built-in `boot-md` hook that looks for `~/.kunming/BOOT.md` on every startup. If the file exists, the agent runs its instructions in a background session. No installation needed — just create the file.

**Create `~/.kunming/BOOT.md`:**

```markdown
# Startup Checklist

1. Check if any cron jobs failed overnight — run `km cron list`
...
```yaml
# ~/.kunming/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.kunming/hooks/long-task-alert/handler.py
import os
import httpx
...
```yaml
# ~/.kunming/hooks/command-logger/HOOK.yaml
name: command-logger
...
```python
# ~/.kunming/hooks/command-logger/handler.py
import json
from datetime import datetime
from pathlib import Path

LOG = Path.home() / ".kunming" / "logs" / "command_usage.jsonl"
...
```yaml
# ~/.kunming/hooks/session-webhook/HOOK.yaml
...
```python
# ~/.kunming/hooks/session-webhook/handler.py
import httpx

WEBHOOK_URL = "https://your-service.example.com/kunming-events"
...
1. On gateway startup, `HookRegistry.discover_and_load()` scans `~/.kunming/hooks/`
...
See the **[Build a Plugin guide](/docs/guides/build-a-kunming-plugin)** for the full walkthrough including tool schemas, handlers, and advanced hook patterns.
