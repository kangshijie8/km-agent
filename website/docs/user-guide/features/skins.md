...
sidebar_position: 10
title: "Skins & Themes"
description: "Customize the Kunming CLI with built-in and user-defined skins"
---

# Skins & Themes

Skins control the **visual presentation** of the Kunming CLI: banner colors, spinner faces and verbs, response-box labels, branding text, and the tool activity prefix.
...
/skin mytheme        # switch to a custom skin from ~/.kunming/skins/mytheme.yaml
```

Or set the default skin in `~/.kunming/config.yaml`:
...
|------|-------------|----------------|------------------|
| `default` | Classic Kunming �?gold and kawaii | `Kunming Agent` | Warm gold borders, cornsilk text, kawaii faces in spinners. The familiar caduceus banner. Clean and inviting. |
...
| `mono` | Monochrome �?clean grayscale | `Kunming Agent` | All grays �?no color. Borders are `#555555`, text is `#c9d1d9`. Ideal for minimal terminal setups or screen recordings. |
| `slate` | Cool blue �?developer-focused | `Kunming Agent` | Royal blue borders (`#4169e1`), soft blue text. Calm and professional. No custom spinner �?uses default faces. |
...
|-----|-------------|---------|
| `agent_name` | Name shown in banner title and status display | `Kunming Agent` |
| `welcome` | Welcome message shown at CLI startup | `Welcome to Kunming Agent! Type your message or /help for commands.` |
| `goodbye` | Message shown on exit | `Goodbye! ⚕` |
| `response_label` | Label on the response box header | ` �?Kunming ` |
...
| `banner_logo` | string | Rich-markup ASCII art logo (replaces the default KUNMING_AGENT banner) | `""` |
| `banner_hero` | string | Rich-markup hero art (replaces the default caduceus art) | `""` |

## Custom skins

Create YAML files under `~/.kunming/skins/`. User skins inherit missing values from the built-in `default` skin, so you only need to specify the keys you want to change.

### Full custom skin YAML template

```yaml
# ~/.kunming/skins/mytheme.yaml
...
## Kunming Mod �?Visual Skin Editor

[Kunming Mod](https://github.com/cocktailpeanut/kunming-mod) is a community-built web UI for creating and managing skins visually. Instead of writing YAML by hand, you get a point-and-click editor with live preview.

![Kunming Mod skin editor](https://raw.githubusercontent.com/cocktailpeanut/kunming-mod/master/nous.png)

**What it does:**

- Lists all built-in and custom skins
- Opens any skin into a visual editor with all Kunming skin fields (colors, spinner, branding, tool prefix, tool emojis)
...
- Saves directly to `~/.kunming/skins/`
- Activates a skin by updating `~/.kunming/config.yaml`
...
```bash
git clone https://github.com/cocktailpeanut/kunming-mod.git
cd kunming-mod/app
npm install
npm start
```
...
6. Click **Save** to write the skin YAML to `~/.kunming/skins/`.
7. Click **Activate** to set it as the current skin (updates `display.skin` in `config.yaml`).

Kunming Mod respects the `KUNMING_HOME` environment variable, so it works with [profiles](/docs/user-guide/profiles) too.

## Operational notes

- Built-in skins load from `kunming_cli/skin_engine.py`.
- Unknown skins automatically fall back to `default`.
- `/skin` updates the active CLI theme immediately for the current session.
- User skins in `~/.kunming/skins/` take precedence over built-in skins with the same name.
