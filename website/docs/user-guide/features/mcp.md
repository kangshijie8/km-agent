...
description: "Connect Kunming Agent to external tool servers via MCP — and control exactly which MCP tools Kunming loads"
---

# MCP (Model Context Protocol)

MCP lets Kunming Agent connect to external tool servers so the agent can use tools that live outside Kunming itself — GitHub, databases, file systems, browser stacks, internal APIs, and more.

If you have ever wanted Kunming to use a tool that already exists somewhere else, MCP is usually the cleanest way to do it.

## What MCP gives you

- Access to external tool ecosystems without writing a native Kunming tool first
- Local stdio servers and remote HTTP MCP servers in the same config
- Automatic tool discovery and registration at startup
- Utility wrappers for MCP resources and prompts when supported by the server
- Per-server filtering so you can expose only the MCP tools you actually want Kunming to see
...
```bash
cd ~/.kunming/kunming-agent
uv pip install -e ".[mcp]"
```

2. Add an MCP server to `~/.kunming/config.yaml`:
...
4. Ask Kunming to use the MCP-backed capability.
...
Kunming will discover the MCP server's tools and use them like any other tool.
...
HTTP MCP servers are remote endpoints Kunming connects to directly.
...
- you do not want Kunming spawning a local subprocess for that integration

## Basic configuration reference

Kunming reads MCP config from `~/.kunming/config.yaml` under `mcp_servers`.
...
| `enabled` | bool | If `false`, Kunming skips the server entirely |
...
## How Kunming registers MCP tools

Kunming prefixes MCP tools so they do not collide with built-in names:
...
In practice, you usually do not need to call the prefixed name manually — Kunming sees the tool and chooses it during normal reasoning.

## MCP utility tools

When supported, Kunming also registers utility tools around MCP resources and prompts:
...
- Kunming only registers resource utilities if the MCP session actually supports resource operations
- Kunming only registers prompt utilities if the MCP session actually supports prompt operations

So a server that exposes callable tools but no resources/prompts will not get those extra wrappers.

## Per-server filtering

You can control which tools each MCP server contributes to Kunming, allowing fine-grained management of your tool namespace.
...
If `enabled: false`, Kunming skips the server completely and does not even attempt a connection.
...
You can also separately disable Kunming-added utility wrappers:
...
If your config filters out all callable tools and disables or omits all supported utilities, Kunming does not create an empty runtime MCP toolset for that server.
...
Kunming discovers MCP servers at startup and registers their tools into the normal tool registry.

### Dynamic Tool Discovery

MCP servers can notify Kunming when their available tools change at runtime by sending a `notifications/tools/list_changed` notification. When Kunming receives this notification, it automatically re-fetches the server's tool list and updates the registry — no manual `/reload-mcp` required.
...
For stdio servers, Kunming does not blindly pass your full shell environment.
...
### Stripe server with dangerous actions Nous Portal
...
Then verify your config and restart Kunming.
...
Because Kunming now only registers those wrappers when both are true:
...
MCP servers can request LLM inference from Kunming via the `sampling/createMessage` protocol. This allows an MCP server to ask Kunming to generate text on its behalf — useful for servers that need LLM capabilities but don't have their own model access.
...
## Running Kunming as an MCP server

In addition to connecting **to** MCP servers, Kunming can also **be** an MCP server. This lets other MCP-capable agents (Claude Code, Cursor, Codex, or any MCP client) use Kunming's messaging capabilities — list conversations, read message history, and send messages across all your connected platforms.

### When to use this

- You want Claude Code, Cursor, or another coding agent to send and read Telegram/Discord/Slack messages through Kunming
- You want a single MCP server that bridges to all of Kunming's connected messaging platforms at once
- You already have a running km gateway with connected platforms
...
Add Kunming to your MCP client config. For example, in Claude Code's `~/.claude/claude_desktop_config.json`:
...
Or if you installed Kunming in a specific location:

```json
{
  "mcpServers": {
    "kunming": {
      "command": "/home/user/.kunming/kunming-agent/venv/bin/kunming",
      "args": ["mcp", "serve"]
    }
  }
}
...
The MCP server exposes 10 tools, matching the standard channel bridge surface plus a Kunming-specific channel browser:
...
The MCP server includes a live event bridge that polls Kunming's session database for new messages. This gives MCP clients near-real-time awareness of incoming conversations:
...
```bash
km mcp serve              # Normal mode
km mcp serve --verbose    # Debug logging on stderr
```

### How it works

The MCP server reads conversation data directly from Kunming's session store (`~/.kunming/sessions/sessions.json` and the SQLite database). A background thread polls the database for new messages and maintains an in-memory event queue. For sending messages, it uses the same `send_message` infrastructure as the Kunming agent itself.
