...
Kunming Agent supports credential pools. Credential pools let you register multiple API keys or OAuth tokens for the same provider. When one key hits a rate limit or billing quota, Kunming automatically rotates to the next healthy key — keeping your session alive without switching providers.
...
If you already have an API key set in `.env`, Kunming auto-discovers it as a 1-key pool. To benefit from pooling, add more keys:

```bash
# Add a second OpenRouter key
km auth add openrouter --api-key sk-or-v1-your-second-key

# Add a second Anthropic key
km auth add anthropic --type api-key --api-key sk-ant-api03-your-second-key

# Add an Anthropic OAuth credential (Claude Code subscription)
km auth add anthropic --type oauth
...
anthropic (3 credentials):
  #1  kunming_pkce          oauth   kunming_pkce �?  #2  claude_code          oauth   claude_code
...
Run `km auth` with no subcommand for an interactive wizard:
...
| Command | Description |
|---------|-------------|
| `km auth` | Interactive pool management wizard |
| `km auth list` | Show all pools and credentials |
| `km auth list <provider>` | Show a specific provider's pool |
| `km auth add <provider>` | Add a credential (prompts for type and key) |
| `km auth add <provider> --type api-key --api-key <key>` | Add an API key non-interactively |
| `km auth add <provider> --type oauth` | Add an OAuth credential via browser login |
| `km auth remove <provider> <index>` | Remove credential by 1-based index |
| `km auth reset <provider>` | Clear all cooldowns/exhaustion status |

## Rotation Strategies

Configure via `km auth` �?"Set rotation strategy" or in `config.yaml`:
...
When you set up a custom endpoint via `km model`, it auto-generates a name like "Together.ai" or "Local (localhost:8080)". This name becomes the pool key.

```bash
# After setting up a custom endpoint via km model:
km auth list
# Shows:
#   Together.ai (1 credential):
#     #1  config key    api_key config:Together.ai �?
# Add a second key for the same endpoint:
km auth add Together.ai --api-key sk-together-second-key
```
...
Kunming automatically discovers credentials from multiple sources and seeds the pool on startup:

| Source | Example | Auto-seeded? |
|--------|---------|-------------|
| Environment variables | `OPENROUTER_API_KEY`, `ANTHROPIC_API_KEY` | Yes |
| OAuth tokens (auth.json) | Codex device code,  device code | Yes |
| Claude Code credentials | `~/.claude/.credentials.json` | Yes (Anthropic) |
| Kunming PKCE OAuth | `~/.kunming/auth.json` | Yes (Anthropic) |
| Custom endpoint config | `model.api_key` in config.yaml | Yes (custom endpoints) |
| Manual entries | Added via `km auth add` | Persisted in auth.json |

Auto-seeded entries are updated on each pool load �?if you remove an env var, its pool entry is automatically pruned. Manual entries (added via `km auth add`) are never auto-pruned.
...
2. **`kunming_cli/auth_commands.py`** �?CLI commands and interactive wizard
3. **`kunming_cli/runtime_provider.py`** �?Pool-aware credential resolution
4. **`run_agent.py`** �?Error recovery: 429/402/401 �?pool rotation �?fallback

## Storage

Pool state is stored in `~/.kunming/auth.json` under the `credential_pool` key:
