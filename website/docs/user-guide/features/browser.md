...
Kunming Agent includes a full browser automation toolset with multiple backend options:
...
Then set in `~/.kunming/.env`:

```bash
CAMOFOX_URL=http://localhost:9377
```

Or configure via `km tools` → Browser Automation → Camofox.
...
When enabled, Kunming sends a stable profile-scoped identity to Camofox. The Camofox server maps this identity to a persistent browser profile directory, so cookies, logins, and localStorage survive across restarts. Different Kunming profiles get different browser profiles (profile isolation).
...
When Camofox runs in headed mode (with a visible browser window), it exposes a VNC port in its health check response. Kunming automatically discovers this and includes the VNC URL in navigation responses, so the agent can share a link for you to watch the browser live.

### Local Chrome via CDP (`/browser connect`)

Instead of a cloud provider, you can attach Kunming browser tools to your own running Chrome instance via the Chrome DevTools Protocol (CDP). This is useful when you want to see what the agent is doing in real-time, interact with pages that require your own cookies/sessions, or avoid cloud browser costs.
...
If Chrome isn't already running with remote debugging, Kunming will attempt to auto-launch it with `--remote-debugging-port=9222`.
...
If you do **not** set any cloud credentials and don't use `/browser connect`, Kunming can still use the browser tools through a local Chromium install driven by `agent-browser`.
...
:::info
The `browser` toolset must be included in your config's `toolsets` list or enabled via `km config set toolsets '["kunming-cli", "browser"]'}.
:::
...
```
Navigate to https://github.com/kunming
```
...
```
Type "kunming agent" into the search field @e3
```
...
Screenshots are stored in `~/.kunming/cache/screenshots/` and automatically cleaned up after 24 hours.
...
When enabled, recording starts automatically on the first `browser_navigate` and saves to `~/.kunming/browser_recordings/` when the session closes. Works in both local and cloud (Browserbase) modes. Recordings older than 72 hours are automatically cleaned up.
...
:::note
If paid features aren't available on your plan, Kunming automatically falls back — first disabling `keepAlive`, then proxies — so browsing still works on free plans.
:::
