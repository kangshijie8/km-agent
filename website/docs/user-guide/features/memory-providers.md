...
Kunming ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
km memory setup      # interactive picker + configuration
km memory status     # check what's active
km memory off        # disable external provider
```

Or set manually in `~/.kunming/config.yaml`:
...
When a memory provider is active, Kunming automatically:
...
**Setup Wizard:**
```bash
km honcho setup        # (legacy command) 
# or
km memory setup        # select "honcho"
```

**Config:** `$KUNMING_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$KUNMING_HOME/honcho.json` > `~/.kunming/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/kangshijie8/KM-Agent/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/kunming).
...
:::tip Migrating from `km honcho`
If you previously used `km honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-agent / Profiles:**

Each Kunming profile gets its own Honcho AI peer while sharing the same workspace -- all profiles see the same user representation, but each agent builds its own identity and observations.

```bash
kunming profile create coder --clone   # creates honcho peer "coder", inherits config from default
```

What `--clone` does: creates a `kunming.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The peer is eagerly created in Honcho so it exists before first message.

For profiles created before Honcho was set up:

```bash
km honcho sync   # scans all profiles, creates host blocks for any missing ones
```

This inherits settings from the default `kunming` host block and creates new AI peers for each profile. Idempotent -- skips profiles that already have a host block.
...
See the [config reference](https://github.com/kangshijie8/KM-Agent/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/kunming).
...
# Then configure Kunming
km memory setup    # select "openviking"
# Or manually:
km config set memory.provider openviking
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.kunming/.env
```
...
**Setup:**
```bash
km memory setup    # select "mem0"
# Or manually:
km config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.kunming/.env
```

**Config:** `$KUNMING_HOME/mem0.json`

| Key | Default | Description |
|-----|---------|-------------|
| `user_id` | `kunming-user` | User identifier |
...
**Setup:**
```bash
km memory setup    # select "hindsight"
# Or manually:
km config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.kunming/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p kunming ui start`

**Config:** `$KUNMING_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `kunming` | Memory bank identifier |
...
See [plugin README](https://github.com/kangshijie8/KM-Agent/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
km memory setup    # select "holographic"
# Or manually:
km config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.kunming-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$KUNMING_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
km memory setup    # select "retaindb"
# Or manually:
km config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.kunming/.env
```
...
# Then configure Kunming
km memory setup    # select "byterover"
# Or manually:
km config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$KUNMING_HOME/byterover/` (profile-scoped)
...
**Setup:**
```bash
km memory setup    # select "supermemory"
# Or manually:
km config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.kunming/.env
```

**Config:** `$KUNMING_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `kunming` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `kunming-{identity}` → `kunming-coder`) to isolate memories per Kunming profile
...
- **Local storage providers** (Holographic, ByteRover) use `$KUNMING_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$KUNMING_HOME/` so each profile has its own credentials
