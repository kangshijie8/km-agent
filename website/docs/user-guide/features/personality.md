...
description: "Customize Kunming Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Kunming Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.

- `SOUL.md` — a durable persona file that lives in `KUNMING_HOME` and serves as the agent's identity (slot #1 in the system prompt)
- built-in or custom `/personality` presets — session-level system-prompt overlays

If you want to change who Kunming is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Kunming now seeds a default `SOUL.md` automatically in:

```text
~/.kunming/SOUL.md
```

More precisely, it uses the current instance's `KUNMING_HOME`, so if you run Kunming with a custom home directory, it will use:

```text
$KUNMING_HOME/SOUL.md
```

### Important behavior

- **SOUL.md is the agent's primary identity.** It occupies slot #1 in the system prompt, replacing the hardcoded default identity.
- Kunming creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Kunming loads `SOUL.md` only from `KUNMING_HOME`
- Kunming does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Kunming falls back to a built-in default identity
...
If Kunming loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `KUNMING_HOME`, the personality belongs to the Kunming instance itself.

That also makes it easier to teach users:
- "Edit `~/.kunming/SOUL.md` to change Kunming' default personality."
...
- how Kunming should handle uncertainty, disagreement, or ambiguity
...
## What Kunming injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Kunming falls back to a built-in default identity ("You are Kunming Agent, an intelligent AI assistant created by km..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Kunming ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Kunming, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Kunming its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.kunming/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.kunming/SOUL.md`
...
- [SOUL.md Guide](/docs/guides/use-soul-with-kunming)

## CLI appearance vs conversational personality

Conversational personality and CLI appearance are separate:

- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Kunming speaks
- `display.skin` and `/skin` affect how Kunming looks in the terminal
