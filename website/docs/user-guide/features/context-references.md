...
Type `@` followed by a reference to inject content directly into your message. Kunming expands the reference inline and appends the content under an `--- Attached Context ---` section.
...
- Kunming env: `$KUNMING_HOME/.env`

These directories are fully blocked (any file inside):
- `~/.ssh/`, `~/.aws/`, `~/.gnupg/`, `~/.kube/`, `$KUNMING_HOME/skills/.hub/`
