...
description: "Overview of Kunming Agent's tools — what's available, how toolsets work, and terminal backends"
---

# Tools & Toolsets

Tools are the primary way Kunming interacts with the world outside of conversation. They're organized into logical **toolsets** that can be enabled or disabled per platform.

## Available Tools

Kunming ships with a broad built-in tool registry covering web search, browser automation, terminal execution, file editing, memory, delegation, RL training, messaging delivery, Home Assistant, and more.
...
See [Toolsets Reference](/docs/reference/toolsets-reference) for the full set, including platform presets such as `kunming-cli`, `kunming-telegram`, and dynamic MCP toolsets like `mcp-<server>`.
...
Enable or disable tools in `~/.kunming/config.yaml`:
...
```
```bash
# Set credentials in ~/.kunming/.env
...
# Configure
km config set terminal.backend singularity
km config set terminal.singularity_image ~/python.sif
```
...
modal setup
km config set terminal.backend modal
```
...
If a command needs sudo, you'll be prompted for your password (cached for the session). Or set `SUDO_PASSWORD` in `~/.kunming/.env`.

:::warning
On messaging platforms, if sudo fails, the output includes a tip to add `SUDO_PASSWORD` to `~/.kunming/.env`.
:::
