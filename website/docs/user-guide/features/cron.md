...
Schedule tasks to run automatically with natural language or cron expressions. Kunming exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
:::warning
Cron-run sessions cannot recursively create more cron jobs. Kunming disables cron management tools inside cron executions to prevent runaway scheduling loops.
:::
...
```bash
km cron create "every 2h" "Check server status"
km cron create "every 1h" "Summarize new feed items" --skill blogwatcher
km cron create "every 1h" "Use both skills and combine the result" \
  --skill blogwatcher \
  --skill find-nearby \
  --name "Skill combo"
```
...
Ask Kunming normally:

```text
Every morning at 9am, check Hacker News for AI news and send me a summary on Telegram.
```

Kunming will use the unified `cronjob` tool internally.
...
```bash
km cron edit <job_id> --schedule "every 4h"
km cron edit <job_id> --prompt "Use the revised task"
km cron edit <job_id> --skill blogwatcher --skill find-nearby
km cron edit <job_id> --add-skill find-nearby
km cron edit <job_id> --remove-skill blogwatcher
km cron edit <job_id> --clear-skills
```
...
```bash
km cron list
km cron pause <job_id>
km cron resume <job_id>
km cron run <job_id>
km cron remove <job_id>
km cron status
km cron tick
```
...
sudo km gateway install --system   # Linux: boot-time system service for servers
km gateway             # Or run in foreground
...
On each tick Kunming:

1. loads jobs from `~/.kunming/cron/jobs.json`
...
A file lock at `~/.kunming/cron/.tick.lock` prevents overlapping scheduler ticks from double-running the same job batch.
...
| `"local"` | Save to local files only (`~/.kunming/cron/output/`) | Default on CLI |
...
If the agent's final response starts with `[SILENT]`, delivery is suppressed entirely. The output is still saved locally for audit (in `~/.kunming/cron/output/`), but no message is sent to the delivery target.
...
The agent's final response is automatically delivered — you do **not** need to include `send_message` in the cron prompt for that same destination. If a cron run calls `send_message` to the exact target the scheduler will already deliver to, Kunming skips that duplicate send and tells the model to put the user-facing content in the final response instead. Use `send_message` only for additional or different targets.
...
| Interval (`every 2h`) | forever | Runs until Nous Portal |
| Cron expression | forever | Runs until Nous Portal |
...
Jobs are stored in `~/.kunming/cron/jobs.json`. Output from job runs is saved to `~/.kunming/cron/output/{job_id}/{timestamp}.md`.
