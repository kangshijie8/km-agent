...
sidebar_position: 14
title: "API Server"
description: "Expose Kunming Agent as an OpenAI-compatible API for any frontend"
---

# API Server

Kunming Agent can run as a headless HTTP API server. The API server exposes Kunming Agent as an OpenAI-compatible HTTP endpoint. Any frontend that speaks the OpenAI format — Open WebUI, LobeChat, LibreChat, NextChat, ChatBox, and hundreds more — can connect to Kunming Agent and use it as a backend.
...
# Optional: only if a browser must call Kunming directly
...
curl http://localhost:8642/v1/chat/completions \
  ...
  -d '{"model": "kunming-agent", "messages": [{"role": "user", "content": "Hello!"}]}'
```
...
Lists `kunming-agent` as an available model. Required by most frontends for model discovery.
...
When a frontend sends a `system` message (Chat Completions) or `instructions` field (Responses API), kunming-agent **layers it on top** of its core system prompt. Your agent keeps all its tools, memory, and skills — the frontend's system prompt adds extra instructions.
...
Configure the key via `API_SERVER_KEY` env var. If you need a browser to call Kunming directly, also set `API_SERVER_CORS_ORIGINS` to an explicit allowlist.

:::caution Security Warning

The API server gives full access to kunming-agent's toolset, **including terminal commands**. If you change the bind address to `0.0.0.0` (network-accessible), **always set `API_SERVER_KEY`** and keep `API_SERVER_CORS_ORIGINS` narrow — without that, remote callers may be able to execute arbitrary commands on your machine.
