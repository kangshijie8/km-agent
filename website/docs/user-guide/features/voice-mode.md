...
title: "Voice Mode"
description: "Voice input and output for Kunming Agent"
---

# Voice Mode

Kunming Agent supports voice input (speech-to-text) and voice output (text-to-speech) for hands-free interaction.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with Kunming](/docs/guides/use-voice-mode-with-kunming).

## Prerequisites

Before using voice features, make sure you have:

1. **Kunming Agent installed** — `pip install kunming-agent` (see [Installation](/docs/getting-started/installation))
2. **An LLM provider configured** — run `km model` or set your preferred provider credentials in `~/.kunming/.env`
3. **A working base setup** — run `km` to verify the agent responds to text before enabling voice

:::tip
The `~/.kunming/` directory and default `config.yaml` are created automatically the first time you run `km`. You only need to create `~/.kunming/.env` manually for API keys.
:::
...
pip install "kunming-agent[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
pip install "kunming-agent[all]"
```
...
Or toggle voice mode during a session with the `/voice` command.

Voice messages work on platforms that support them:

- **Telegram** — Send voice messages, receive voice replies
- **Discord** — Send voice messages, receive voice replies, join voice channels
- **Matrix** — Send voice messages, receive voice replies
...
1. Start the CLI with `km` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.kunming/config.yaml` (default: `ctrl+b`).
:::
...
```bash
km gateway        # Start the gateway (connects to configured platforms)
km gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@kunming hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.kunming/.env`:
```bash
...
```bash
km gateway        # Start with existing configuration
```
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.kunming/.env`
...
- Review gateway logs: `tail -f ~/.kunming/logs/gateway.log`
