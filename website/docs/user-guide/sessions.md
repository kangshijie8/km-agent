...
Kunming Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.kunming/state.db`) — structured session metadata with FTS5 full-text search
2. **JSONL transcripts** (`~/.kunming/sessions/`) — raw conversation transcripts including tool calls (gateway)
...
km chat -c "my project"

# If there are lineage variants (my project, my project #2, my project #3),
# this automatically resumes the most recent one
km chat -c "my project"   # → resumes "my project #3"
```
...
km chat --resume 20250305_091523_a1b2c3d4
km chat -r 20250305_091523_a1b2c3d4

# Resume by title
km chat --resume "refactoring auth"
```

Session IDs are shown when you exit a CLI session, and can be found with `km sessions list`.

### Conversation Recap on Resume

When you resume a session, Kunming displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src="/img/docs/session-recap.svg" alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Kunming session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.kunming/config.yaml`:
...
Kunming automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `km sessions list` or `km sessions browse`.
...
```bash
km sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Kunming creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`km chat -c "my project"`), it automatically picks the most recent session in the lineage.
...
Kunming provides a full set of session management commands via `km sessions`:
...
# Filter by platform
km sessions list --source telegram

# Show more sessions
km sessions list --limit 50
```
...
km sessions export backup.jsonl

# Export sessions from a specific platform
km sessions export telegram-history.jsonl --source telegram

# Export a single session
km sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4
```
...
km sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
km sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
km sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
km sessions prune --older-than 30

# Only prune sessions from a specific platform
km sessions prune --source telegram --older-than 60

# Skip confirmation
km sessions prune --older-than 30 --yes
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`km insights`](/docs/reference/cli-commands#km-insights).
...
When Kunming cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Kunming uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Kunming in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.kunming/state.db` | All session metadata + messages with FTS5 |
| Gateway transcripts | `~/.kunming/sessions/` | JSONL transcripts per session + sessions.json index |
| Gateway index | `~/.kunming/sessions/sessions.json` | Maps session keys to active session IDs |
