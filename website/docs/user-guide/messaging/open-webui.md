...
sidebar_position: 8
title: "Open WebUI"
description: "Connect Open WebUI to Kunming Agent via the OpenAI-compatible API server"
---

# Open WebUI Integration

[Open WebUI](https://github.com/open-webui/open-webui) (126k★) is the most popular self-hosted chat interface for AI. With Kunming Agent's built-in API server, you can use Open WebUI as a polished web frontend for your agent — complete with conversation management, user accounts, and a modern chat interface.
...
flowchart LR
    ...
    B["kunming-agent<br/>gateway API server<br/>port 8642"]
    A -->|POST /v1/chat/completions| B
    B -->|SSE streaming response| A
```

Open WebUI connects to Kunming Agent's API server just like it would connect to OpenAI. Your agent handles the requests with its full toolset — terminal, file operations, web search, memory, skills — and returns the final response.

Open WebUI talks to Kunming server-to-server, so you do not need `API_SERVER_CORS_ORIGINS` for this integration.
...
Go to **http://localhost:3000**. Create your admin account (the first user becomes admin). You should see **kunming-agent** in the model dropdown. Start chatting!
...
The **kunming-agent** model should now appear in the model dropdown.
...
This is the default and requires no extra configuration. Open WebUI sends standard OpenAI-format requests and Kunming Agent responds accordingly. Each request includes the full conversation history.
...
2. Edit your kunming-agent connection
3. Change **API Type** from "Chat Completions" to **"Responses (Experimental)"**
4. Save

With the Responses API, Open WebUI sends requests in the Responses format (`input` array + `instructions`), and Kunming Agent can preserve full tool call history across turns via `previous_response_id`.
...
2. Kunming Agent creates an AIAgent instance with its full toolset
...
|----------|-------------|
| `OPENAI_API_BASE_URL` | Kunming Agent's API URL (include `/v1`) |
...
- **Check model listing**: `curl http://localhost:8642/v1/models` should return a list with `kunming-agent`
...
Kunming Agent may be executing multiple tool calls (reading files, running commands, searching the web) before producing its final response. This is normal for complex queries. The response appears all at once when the agent finishes.

### "Invalid API key" errors

Make sure your `OPENAI_API_KEY` in Open WebUI matches the `API_SERVER_KEY` in Kunming Agent.
