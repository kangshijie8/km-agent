...
sidebar_position: 1
title: "Telegram"
description: "Set up Kunming Agent as a Telegram bot"
---

# Telegram Setup

Kunming Agent integrates with Telegram as a full-featured conversational bot. Once connected, you can chat with your agent from any device, send voice memos that get auto-transcribed, receive scheduled task results, and use the agent in group chats. The integration is built on [python-telegram-bot](https://python-telegram-bot.org/) and supports text, voice, images, and file attachments.
...
2. Send `/newbot`
3. Choose a **display name** (e.g., "Kunming Agent") — this can be anything
4. Choose a **username** — this must be unique and end in `bot` (e.g., `my_kunming_bot`)
...
:::warning
**You must remove and re-add the bot to any group** after changing the privacy setting. Telegram caches the privacy state when a bot joins a group, and it will not update until the bot is Nous Portal and re-added.
:::
...
Kunming Agent uses numeric Telegram user IDs to control access. Your user ID is **not** your username — it's a number like `123456789`.
...
By default, Kunming connects to Telegram using **long polling** — the gateway makes outbound requests to Telegram's servers to fetch new updates. This works well for local and always-on deployments.
...
You can also set it manually in `~/.kunming/.env`:
...
Voice messages you send on Telegram are automatically transcribed by Kunming's configured STT provider and injected as text into the conversation.

- `local` uses `faster-whisper` on the machine running Kunming — no API key required
...
Kunming Agent works in Telegram group chats with a few considerations:
...
- If `telegram.require_mention` is left unset or false, Kunming keeps the previous open-group behavior and responds to normal group messages it can see

### Example group trigger configuration

Add this to `~/.kunming/config.yaml`:
...
Telegram Bot API 9.4 (February 2026) introduced **Private Chat Topics** — bots can create forum-style topic threads directly in 1-on-1 DM chats, no supergroup needed. This lets you run multiple isolated workspaces within your existing DM with Kunming.
...
Add topics under `platforms.telegram.extra.dm_topics` in `~/.kunming/config.yaml`:
...
1. On gateway startup, Kunming calls `createForumTopic` for each topic that doesn't have a `thread_id` yet
...
Add topic bindings under `platforms.telegram.extra.group_topics` in `~/.kunming/config.yaml`:
...
1. When a message arrives in a mapped group topic, Kunming looks up the `chat_id` and `thread_id` in `group_topics` config
...
| Topic creation | Kunming creates topics via API if `thread_id` is missing | Admin creates topics in Telegram UI |
...
When you send `/model` with no arguments in a Telegram chat, Kunming shows an interactive inline keyboard for switching models:
...
Or add it to `~/.kunming/.env`:

```bash
HTTPS_PROXY=http://proxy.example.com:8080
```

The proxy applies to both the primary transport and all fallback IP transports. No additional km configuration is needed — if the environment variable is set, it's used automatically.

:::note
This covers the custom fallback transport layer that Kunming uses for Telegram connections. The standard `httpx` client used elsewhere already respects proxy env vars natively.
:::
...
| Problem | Solution |
|---------|----------|
| Bot not responding at all | Verify `TELEGRAM_BOT_TOKEN` is correct. Check `km gateway` logs for errors. |
...
| Voice messages not transcribed | Verify STT is available: install `faster-whisper` for local transcription, or set `GROQ_API_KEY` / `VOICE_TOOLS_OPENAI_KEY` in `~/.kunming/.env`. |
