...
Connect Kunming to Apple iMessage via [BlueBubbles](https://bluebubbles.app/) — a free, open-source macOS server that bridges iMessage to any device.
...
- Network connectivity between Kunming and the BlueBubbles server
...
Or set environment variables directly in `~/.kunming/.env`:
...
```bash
km pairing generate bluebubbles
```
...
Kunming will connect to your BlueBubbles server, register a webhook, and start listening for iMessage messages.

## How It Works

```
iMessage → Messages.app → BlueBubbles Server → Webhook → Kunming
Kunming → BlueBubbles REST API → Messages.app → iMessage
```

- **Inbound:** BlueBubbles sends webhook events to a local listener when new messages arrive. No polling — instant delivery.
- **Outbound:** Kunming sends messages via the BlueBubbles REST API.
...
### Chat Addressing
You can address chats by email or phone number — Kunming resolves them to BlueBubbles chat GUIDs automatically. No need to use raw GUID format.
...
- Check `km gateway logs` for webhook errors
