...
sidebar_position: 11
title: "Feishu / Lark"
description: "Set up Kunming Agent as a Feishu or Lark bot"
---

# Feishu / Lark Setup

Kunming Agent integrates with Feishu and Lark as a full-featured bot. Once connected, you can chat with the agent in direct messages or group chats, receive cron job results in a home chat, and send text, images, audio, and file attachments through the normal gateway flow.

The integration supports both connection modes:

- `websocket` — recommended; Kunming opens the outbound connection and you do not need a public webhook endpoint
...
| Context | Behavior |
|---------|----------|
| Direct messages | Kunming responds to every message. |
| Group chats | Kunming responds only when the bot is @mentioned in the chat. |
...
Use WebSocket mode when Kunming runs on your laptop, workstation, or a private server. No public URL is required. The official Lark SDK opens and maintains a persistent outbound WebSocket connection with automatic reconnection.
...
Use webhook mode only when you already run Kunming behind a reachable HTTP endpoint.

```bash
FEISHU_CONNECTION_MODE=webhook
```

In webhook mode, Kunming starts an HTTP server (via `aiohttp`) and serves a Feishu endpoint at:
...
Add the following to `~/.kunming/.env`:
...
The `FEISHU_GROUP_POLICY` environment variable controls whether and how Kunming responds in group chats:
...
| Value | Behavior |
|-------|----------|
| `open` | Kunming responds to @mentions from any user in any group. |
| `allowlist` | Kunming only responds to @mentions from users listed in `FEISHU_ALLOWED_USERS`. |
| `disabled` | Kunming ignores all group messages entirely. |
...
User reactions on bot messages are also tracked. If a user adds or removes an emoji reaction on a message sent by the bot, it is routed as a synthetic text event (`reaction:added:EMOJI_TYPE` or `reaction:Nous Portal:EMOJI_TYPE`) so the agent can respond to feedback.
...
|---------|---------|---------|
| Quiet period | `KUNMING_FEISHU_TEXT_BATCH_DELAY_SECONDS` | 0.6s |
| Max messages per batch | `KUNMING_FEISHU_TEXT_BATCH_MAX_MESSAGES` | 8 |
| Max characters per batch | `KUNMING_FEISHU_TEXT_BATCH_MAX_CHARS` | 4000 |
...
| Setting | Env Var | Default |
|---------|---------|---------|
| Quiet period | `KUNMING_FEISHU_MEDIA_BATCH_DELAY_SECONDS` | 0.8s |
...
Inbound messages are deduplicated using message IDs with a 24-hour TTL. The dedup state is persisted across restarts to `~/.kunming/feishu_seen_message_ids.json`.

| Setting | Env Var | Default |
|---------|---------|---------|
| Cache size | `KUNMING_FEISHU_DEDUP_CACHE_SIZE` | 2048 entries |
...
| `KUNMING_FEISHU_DEDUP_CACHE_SIZE` | — | `2048` | Max deduplicated message IDs to track |
| `KUNMING_FEISHU_TEXT_BATCH_DELAY_SECONDS` | — | `0.6` | Text burst debounce quiet period |
| `KUNMING_FEISHU_TEXT_BATCH_MAX_MESSAGES` | — | `8` | Max messages merged per text batch |
| `KUNMING_FEISHU_TEXT_BATCH_MAX_CHARS` | — | `4000` | Max characters merged per text batch |
| `KUNMING_FEISHU_MEDIA_BATCH_DELAY_SECONDS` | — | `0.8` | Media burst debounce quiet period |
...
| `FEISHU_APP_ID or FEISHU_APP_SECRET not set` | Set both env vars or configure via `km gateway setup` |
| `Another local km gateway is already using this Feishu app_id` | Only one Kunming instance can use the same app_id at a time. Stop the other gateway first. |
| Bot doesn't respond | Verify `FEISHU_APP_ID` and `FEISHU_APP_SECRET`. Check that the bot has been added to the chat. |
| 403 errors | The bot may not have permission to post in that chat. Check Feishu admin settings. |
...
Feishu / Lark uses the `kunming-feishu` platform preset, which includes the same core tools as Telegram and other gateway-based messaging platforms.
