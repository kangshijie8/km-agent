...
sidebar_position: 9
title: "Matrix"
description: "Set up Kunming Agent as a Matrix bot"
---

# Matrix Setup

Kunming Agent integrates with Matrix, the open, federated messaging protocol. Matrix lets you run your own homeserver or use a public one like matrix.org — either way, you keep control of your communications. The bot connects via the `matrix-nio` Python SDK, processes messages through the Kunming Agent pipeline (including tool use, memory, and reasoning), and responds in real time. It supports text, file attachments, images, audio, video, and optional end-to-end encryption (E2EE).

Kunming works with any Matrix homeserver — Synapse, Conduit, Dendrite, or matrix.org.

Before setup, here's the part most people want to know: how Kunming behaves once it's connected.
...
| **Rooms** | By default, Kunming requires an `@mention` to respond. Set `MATRIX_REQUIRE_MENTION=false` or add room IDs to `MATRIX_FREE_RESPONSE_ROOMS` for free-response rooms. Room invites are auto-accepted. |
| **Threads** | Kunming supports Matrix threads (MSC3440). If you reply in a thread, Kunming keeps the thread context isolated from the main room timeline. Threads where the bot has already participated do not require a mention. |
| **Auto-threading** | By default, Kunming auto-creates a thread for each message it responds to in a room. This keeps conversations isolated. Set `MATRIX_AUTO_THREAD=false` to disable. |
| **Shared rooms with multiple users** | By default, Kunming isolates session history per user inside the room. Two people talking in the same room do not share one transcript unless you explicitly disable that. |
...
```yaml
matrix:
  bot_name: "Kunming"             # Optional: Bot display name
...
2. Choose a username like `kunming` — the full user ID will be `@kunming:your-server.org`.

### Option B: Use matrix.org or Another Public Homeserver

1. Go to [Element Web](https://app.element.io) and create a new account.
2. Pick a username for your bot (e.g., `kunming-bot`).

### Option C: Use Your Own Account

You can also run Kunming as your own user. This means the bot posts as you — useful for personal assistants.

## Step 2: Get an Access Token

Kunming needs an access token to authenticate with the homeserver. You have two options:
...
Instead of providing an access token, you can give Kunming the bot's user ID and password. Kunming will log in automatically on startup. This is simpler but means the password is stored in your `.env` file.

```bash
MATRIX_USER_ID=@kunming:your-server.org
MATRIX_PASSWORD=your-password
```

## Step 3: Find Your Matrix User ID

Kunming Agent uses your Matrix User ID to control who can interact with the bot. Matrix User IDs follow the format `@username:server`.
...
## Step 2: Configure Kunming Agent
...
# MATRIX_USER_ID=@kunming:matrix.example.org
...
Optional behavior settings in `~/.kunming/config.yaml`:
...
Kunming supports Matrix end-to-end encryption, so you can chat with your bot in encrypted rooms.
...
# Or install with kunming extras
pip install 'kunming-agent[matrix]'
```
...
Add to your `~/.kunming/.env`:

```bash
MATRIX_ENCRYPTION=true
```

When E2EE is enabled, Kunming:

- Stores encryption keys in `~/.kunming/platforms/matrix/store/` (legacy installs: `~/.kunming/matrix/store/`)
...
:::warning
If you delete the `~/.kunming/platforms/matrix/store/` directory, the bot loses its encryption keys. You'll need to verify the device again in your Matrix client. Back up this directory if you want to preserve encrypted sessions.
:::
...
**Fix**: The sync loop automatically retries every 5 seconds on error. Check the km logs for sync-related warnings. If the bot consistently falls behind, ensure your homeserver has adequate resources.

### Bot is offline

**Cause**: The km gateway isn't running, or it failed to connect.

**Fix**: Check that `km gateway` is running. Look at the terminal output for error messages. Common issues: wrong homeserver URL, expired access token, homeserver unreachable.

### "User not allowed" / Bot ignores you

**Cause**: Your User ID isn't in `MATRIX_ALLOWED_USERS`.

**Fix**: Add your User ID to `MATRIX_ALLOWED_USERS` in `~/.kunming/.env` and restart the gateway. Use the full `@user:server` format.
...
- **Media support**: Kunming can send and receive images, audio, video, and file attachments. Media is uploaded to your homeserver using the Matrix content repository API.
