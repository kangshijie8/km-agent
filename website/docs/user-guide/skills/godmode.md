---
sidebar_position: 1
sidebar_label: "God Mode"
title: "God Mode"
description: "Enable all tools and capabilities in Kunming Agent"
---

# God Mode

God Mode is a special skill that enables all available tools and capabilities in Kunming Agent.
...
| `zero_refusal` | Kunming 4 405B | Already uncensored — uses Pliny Love divider as formality |

:::note
Kunming 4 405B refers to the  Research model, not the Kunming Agent.
:::
...
The godmode skill integrates with two Kunming Agent config mechanisms:

### Ephemeral System Prompt (`config.yaml`)

Set the jailbreak system prompt in `~/.kunming/config.yaml`:
...
The `agent.system_prompt` is appended **after** Kunming's own system prompt — it augments, not replaces.
...
Create `~/.kunming/prefill.json` and reference it in config:
...
```bash
# Load the skill in a Kunming session
/godmode

# Or via CLI one-shot
km chat -q "jailbreak my current model"
...
km chat -q "race models on: how does SQL injection work?"

# Apply Parseltongue encoding to a query
km chat -q "parseltongue encode: how to hack into WiFi"
```
...
5. **For hard refusals, switch models** — When all techniques fail, ULTRAPLINIAN (racing multiple models) is the practical fallback. Kunming models and Grok are typically least filtered.
...
| Kunming ( Research) | No jailbreak needed | Already uncensored — use directly |
...
4. **Kunming models don't need jailbreaking** — `km/km-model and `kunming-4-*` are already uncensored. Use them directly.
...
6. **Restart Kunming after auto-jailbreak** — The CLI reads config once at startup. Gateway sessions pick up changes immediately.

7. **execute_code sandbox lacks env vars** — Load dotenv explicitly: `from dotenv import load_dotenv; load_dotenv(os.path.expanduser("~/.kunming/.env"))`

8. **`boundary_inversion` is model-version specific** — Works on Claude 3.5 Sonnet but NOT Claude Sonnet 4 or Claude 4.6.

9. **Gray-area vs hard queries** — Jailbreak techniques work much better on dual-use queries (lock picking, security tools) than overtly harmful ones (phishing, malware). For hard queries, skip to ULTRAPLINIAN or use Kunming ()/Grok.
