...
sidebar_position: 2
title: "Configuration"
description: "Configure Kunming Agent — config.yaml, providers, models, API keys, and more"
---

# Configuration

All settings are stored in the `~/.kunming/` directory for easy access.
...
```bash
km config              # View current configuration
km config edit         # Open config.yaml in your editor
km config set KEY VAL  # Set a specific value
km config check        # Check for missing options (after updates)
km config migrate      # Interactively add missing options

# Examples:
km config set model anthropic/claude-opus-4
km config set terminal.backend docker
km config set OPENROUTER_API_KEY sk-or-...  # Saves to .env
```

:::tip
The `km config set` command automatically routes values to the right file — API keys are saved to `.env`, everything else to `config.yaml`.
:::
...
1. **CLI arguments** — e.g., `km chat --model anthropic/claude-sonnet-4` (per-invocation override)
2. **`~/.kunming/config.yaml`** — the primary config file for all non-secret settings
3. **`~/.kunming/.env`** — fallback for env vars; **required** for secrets (API keys, tokens, passwords)
...
Kunming supports six terminal backends. Each determines where the agent's shell commands actually execute — your local machine, a Docker container, a remote server via SSH, a Modal cloud sandbox, a Daytona workspace, or a Singularity/Apptainer container.
...
For cloud sandboxes such as Modal and Daytona, `container_persistent: true` means Kunming will try to preserve filesystem state across sandbox recreation. It does not promise that the same live sandbox, PID space, or background processes will still be running later.
...
:::warning
The agent has the same filesystem access as your user account. Use `km tools` to disable tools you don't want, or switch to Docker for sandboxing.
:::
...
**Requirements:** Docker Desktop or Docker Engine installed and running. Kunming probes `$PATH` plus common macOS install locations (`/usr/local/bin/docker`, `/opt/homebrew/bin/docker`, Docker Desktop app bundle).

**Container lifecycle:** Each session starts a long-lived container (`docker run -d ... sleep 2h`). Commands run via `docker exec` with a login shell. On cleanup, the container is stopped and Nous Portal.
...
**Credential forwarding:** Env vars listed in `docker_forward_env` are resolved from your shell environment first, then `~/.kunming/.env`. Skills can also declare `required_environment_variables` which are merged automatically.
...
**Persistence:** When enabled, the sandbox filesystem is snapshotted on cleanup and restored on next session. Snapshots are tracked in `~/.kunming/modal_snapshots.json`. This preserves filesystem state, not live processes, PID space, or background jobs.

**Credential files:** Automatically mounted from `~/.kunming/` (OAuth tokens, etc.) and synced before each command.
...
**Persistence:** When enabled, sandboxes are stopped (not deleted) on cleanup and resumed on next session. Sandbox names follow the pattern `kunming-{task_id}`.
...
**Scratch directory:** Resolved in order: `TERMINAL_SCRATCH_DIR` → `TERMINAL_SANDBOX_DIR/singularity` → `/scratch/$USER/kunming-agent` (HPC convention) → `~/.kunming/sandboxes/singularity`.
...
- **Docker** — Run `docker version` to verify Docker is working. If it fails, fix Docker or `km config set terminal.backend local`.
- **SSH** — Both `TERMINAL_SSH_HOST` and `TERMINAL_SSH_USER` must be set. km logs a clear error if either is missing.
- **Modal** — Needs `MODAL_TOKEN_ID` env var or `~/.modal.toml`. Run `km doctor` to check.
...
Kunming resolves each listed variable from your current shell first, then falls back to `~/.kunming/.env` if it was saved with `km config set`.
...
Docker sandboxes stay isolated by default. Kunming does **not** pass your current host working directory into the container unless you explicitly opt in.
...
When enabled:
- if you launch Kunming from `~/projects/my-app`, that host directory is bind-mounted to `/workspace`
...
- `true` gives the sandbox direct access to the directory you launched Kunming from
...
```bash
km config set terminal.persistent_shell false
```
...
- `km config migrate` scans all enabled skills, finds unconfigured settings, and offers to prompt you
- `km config show` displays all skill settings under "Skill Settings" with the skill they belong to
- When a skill loads, its resolved config values are injected into the skill context automatically

**Setting values manually:**

```bash
km config set skills.config.wiki.path ~/my-research-wiki
```
...
```yaml
worktree: true    # Always create a worktree (same as km chat -w)
# worktree: false # Default — only when -w flag is passed
```

When enabled, each CLI session creates a fresh worktree under `.worktrees/` with its own branch. Agents can edit files, commit, push, and create PRs without interfering with each other. Clean worktrees are Nous Portal on exit; dirty ones are kept for manual recovery.
...
Kunming automatically compresses long conversations to stay within your model's context window. The compression summarizer is a separate LLM call — you can point it at any provider or endpoint.
...
compression:
  ...
  threshold: 0.50
```
Uses the first available provider (OpenRouter →  → Codex) with Gemini Flash.
...
Kunming uses lightweight "auxiliary" models for side tasks like image analysis, web page summarization, and browser screenshot analysis. By default, these use **Gemini Flash** via auto-detection — you don't need to configure anything.

### The universal config pattern

Every model slot in Kunming — auxiliary tasks, compression, fallback — uses the same three knobs:
...
When `base_url` is set, Kunming ignores the provider and calls that endpoint directly (using `api_key` or `OPENAI_API_KEY` for auth). When only `provider` is set, Kunming uses that provider's built-in auth and base URL.
...
Or via environment variable (in `~/.kunming/.env`):
...
| `"nous"` | Force Nous Portal | `km auth` |
| `"codex"` | Force Codex OAuth (ChatGPT account). Supports vision (gpt-5.3-codex). | `km model` → Codex |
| `"main"` | Use your active custom/main endpoint. This can come from `OPENAI_BASE_URL` + `OPENAI_API_KEY` or from a custom endpoint saved via `km model` / `config.yaml`. Works with OpenAI, local models, or any OpenAI-compatible API. **Auxiliary tasks only — not valid for `model.provider`.** | Custom endpoint credentials + base URL |
...
`base_url` takes precedence over `provider`, so this is the most explicit way to route an auxiliary task to a specific endpoint. For direct endpoint overrides, Kunming uses the configured `api_key` or falls back to `OPENAI_API_KEY`; it does not reuse `OPENROUTER_API_KEY` for that custom endpoint.
...
`provider: "main"` uses whatever provider Kunming uses for normal chat — whether that's a named custom provider (e.g. `beans`), a built-in provider like `openrouter`, or a legacy `OPENAI_BASE_URL` endpoint.
...
:::tip
Run `km config` to see your current auxiliary model settings. Overrides only show up when they differ from the defaults.
:::
...
If the requested provider is unavailable, Kunming falls back automatically in this order: `local` → `groq` → `openai`.
...
:::note
Streaming is disabled by default. Enable it in `~/.kunming/config.yaml` to try the streaming UX.
:::
...
- `false` reverts to the old shared-room behavior. That can be useful if you explicitly want Kunming to treat a channel like one collaborative conversation, but it also means users share context, token costs, and interrupt state.
- Direct messages are unaffected. Kunming still keys DMs by chat/DM ID as usual.
...
Control what Kunming does when an unknown user sends a direct message:
...
- `pair` is the default. Kunming denies access, but replies with a one-time pairing code in DMs.
...
quick_commands:
  status:
    type: exec
    command: systemctl status kunming-agent
  disk:
    type: exec
    command: df -h /
  update:
    type: exec
    command: cd ~/.kunming/kunming-agent && git pull && pip install -e .
  gpu:
    type: exec
...
The `web_search`, `web_extract`, and `web_crawl` tools support four backend providers. Configure the backend in `config.yaml` or via `km tools`:
...
browser:
  ...
  record_sessions: false         # Auto-record browser sessions as WebM videos to ~/.kunming/browser_recordings/
  camofox:
...
Control how Kunming handles potentially dangerous commands:
...
| `off` | Skip all approval checks. Equivalent to `KUNMING_YOLO_MODE=true`. **Use with caution.** |
...
```yaml
checkpoints:
  enabled: true                  # Enable automatic checkpoints (also: km chat --checkpoints)
...
**Direct endpoint override:** If you want the obvious custom-endpoint path, set `delegation.base_url`, `delegation.api_key`, and `delegation.model`. That sends subagents directly to that OpenAI-compatible endpoint and takes precedence over `delegation.provider`. If `delegation.api_key` is omitted, Kunming falls back to `OPENAI_API_KEY` only.
...
Kunming uses two different context scopes:

| File | Purpose | Scope |
|------|---------|-------|
| `SOUL.md` | **Primary agent identity** — defines who the agent is (slot #1 in the system prompt) | `~/.kunming/SOUL.md` or `$KUNMING_HOME/SOUL.md` |
| `.kunming.md` / `KUNMING.md` | Project-specific instructions (highest priority) | Walks to git root |
...
- If SOUL.md is missing, empty, or cannot be loaded, Kunming falls back to a built-in default identity.
- **Project context files use a priority system** — only ONE type is loaded (first match wins): `.kunming.md` → `AGENTS.md` → `CLAUDE.md` → `.cursorrules`. SOUL.md is always loaded independently.
- **AGENTS.md** is hierarchical: if subdirectories also have AGENTS.md, all are combined.
- Kunming automatically seeds a default `SOUL.md` if one does not already exist.
...
| Context | Default |
|---------|---------|
| **CLI (`km chat`)** | Current directory where you run the command |
| **Messaging gateway** | Home directory `~` (override with `MESSAGING_CWD`) |
| **Docker / Singularity / Modal / SSH** | User's home directory inside the container or remote machine |

Override the working directory:
```bash
# In ~/.kunming/.env or ~/.kunming/config.yaml:
