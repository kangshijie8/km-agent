...
description: "How Kunming stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `kunming_cli/cron.py` | CLI `km cron` subcommands |
...
Jobs are stored in `~/.kunming/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `km cron` commands are run or during active CLI sessions.
...
```python
# ~/.kunming/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.kunming/cron/output/` |
...
The `km cron` CLI provides direct job management:

```bash
km cron list                    # Show all jobs
km cron create                  # Interactive job creation (alias: add)
km cron edit <job_id>           # Edit job configuration
km cron pause <job_id>          # Pause a running job
km cron resume <job_id>         # Resume a paused job
km cron run <job_id>            # Trigger immediate execution
km cron remove <job_id>         # Delete a job
```
