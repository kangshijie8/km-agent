...
description: "How to add a new inference provider to Kunming Agent �?auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

Kunming can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:

- provider-specific auth or token refresh
- a curated model catalog
- setup / `km model` menu entries
...
1. `kunming_cli/auth.py` decides how credentials are found.
2. `kunming_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `kunming_cli/models.py` and `kunming_cli/main.py` make the provider show up in the CLI. (`kunming_cli/setup.py` delegates to `main.py` automatically �?no changes needed there.)
...
1. `kunming_cli/auth.py`
2. `kunming_cli/models.py`
3. `kunming_cli/runtime_provider.py`
4. `kunming_cli/main.py`
...
:::tip
`kunming_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` �?any provider added there is automatically available in `km setup`.
:::
...
- `PROVIDER_REGISTRY` in `kunming_cli/auth.py`
- `_PROVIDER_LABELS` in `kunming_cli/models.py`
- `_PROVIDER_ALIASES` in both `kunming_cli/auth.py` and `kunming_cli/models.py`
- CLI `--provider` choices in `kunming_cli/main.py`
...
## Step 2: Add auth metadata in `kunming_cli/auth.py`
...
- OAuth / auth-store path: , OpenAI Codex

Questions to answer here:

- What env vars should Kunming check, and in what priority order?
...
## Step 3: Add model catalog and aliases in `kunming_cli/models.py`
...
## Step 4: Resolve runtime data in `kunming_cli/runtime_provider.py`
...
Be careful with API-key precedence. Kunming already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `kunming_cli/main.py`

A provider is not discoverable until it shows up in the interactive `km model` flow.

Update these in `kunming_cli/main.py`:
...
:::tip
`kunming_cli/setup.py` does not need changes �?it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `km model` and `km setup` automatically.
:::
...
When you add a native provider, double-check that Kunming is only sending fields that provider actually understands.
...
```bash
source venv/bin/activate
python -m kunming_cli.main chat -q "Say hello" --provider your-provider --model your-model
```
...
```bash
source venv/bin/activate
python -m kunming_cli.main model
python -m kunming_cli.main setup
```
...
- [ ] `ProviderConfig` added in `kunming_cli/auth.py`
- [ ] aliases added in `kunming_cli/auth.py` and `kunming_cli/models.py`
- [ ] model catalog added in `kunming_cli/models.py`
- [ ] runtime branch added in `kunming_cli/runtime_provider.py`
- [ ] CLI wiring added in `kunming_cli/main.py` (setup.py inherits automatically)
...
### 7. Updating `km model` but not `km setup`
