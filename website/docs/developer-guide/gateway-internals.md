...
The messaging gateway is the long-running process that connects Kunming to 14+ external messaging platforms through a unified architecture.
...
1. `resolve_command()` from `kunming_cli/commands.py` maps input to canonical name (handles aliases, prefix matching)
...
|--------|-----------------|
| `~/.kunming/.env` | API keys, bot tokens, platform credentials |
| `~/.kunming/config.yaml` | Model settings, tool configuration, display options |
...
Hooks are discovered from `gateway/builtin_hooks/` (always active) and `~/.kunming/hooks/` (user-installed). Each hook is a directory with a `HOOK.yaml` manifest and `handler.py`.
...
- `km gateway start` / `km gateway stop` �?manual control
- `systemctl` (Linux) or `launchctl` (macOS) �?service management
- PID file at `~/.kunming/gateway.pid` �?profile-scoped process tracking

**Profile-scoped vs global**: `start_gateway()` uses profile-scoped PID files. `km gateway stop` stops only the current profile's gateway. `km gateway stop --all` uses global `ps aux` scanning to kill all gateway processes (used during updates).
