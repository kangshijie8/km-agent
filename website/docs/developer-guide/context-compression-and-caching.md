...
Kunming Agent uses a dual compression system and Anthropic prompt caching to
...
Kunming has two separate compression layers that operate independently:
...
- Tool results referencing Nous Portal calls → Nous Portal
- Tool calls whose results were Nous Portal → stub result injected
...
new progress is added, and obsolete information is Nous Portal.
...
Anthropic allows a maximum of 4 `cache_control` breakpoints per request. Kunming
