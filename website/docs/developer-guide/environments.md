...
description: "Building RL training environments, running evaluation benchmarks, and generating SFT data with the Kunming Agent Atropos integration"
---

# Environments, Benchmarks & Data Generation

Kunming Agent includes a full environment framework that connects its tool-calling capabilities to the [Atropos](https://github.com/kunming/atropos) RL training framework. This enables three workflows:
...
The Python environment framework documented here lives under the repo's `environments/` directory and is the implementation-level API for Kunming/Atropos integration. This is separate from the user-facing `rl_*` tools, which operate as an orchestration surface for remote RL training workflows.
:::
...
classDiagram
    ...
    BaseEnv <|-- KunmingAgentBaseEnv
    KunmingAgentBaseEnv <|-- TerminalTestEnv
    KunmingAgentBaseEnv <|-- KunmingSweEnv
    KunmingAgentBaseEnv <|-- TerminalBench2EvalEnv
...
The kunming-agent layer (`environments/kunming_base_env.py`). Adds:
- **Terminal backend configuration** — sets `TERMINAL_ENV` for sandboxed execution (local, Docker, Modal, Daytona, SSH, Singularity)
- **Tool resolution** — `_resolve_tools_for_group()` calls kunming-agent's `get_tool_definitions()` to get the right tool schemas based on enabled/disabled toolsets
- **Agent loop integration** — `collect_trajectory()` runs `KunmingAgentLoop` and scores the result
...
Your environment inherits from `KunmingAgentBaseEnv` and implements five methods:
...
`KunmingAgentLoop` (`environments/agent_loop.py`) is the reusable multi-turn agent engine. It runs the same tool-calling pattern as kunming-agent's main loop:
...
| **Generic** | `call_tool(name, args)` — escape hatch for any kunming-agent tool |
...
parser = get_parser("kunming")  # or "mistral", "llama3_json", "qwen", "deepseek_v3", etc.
content, tool_calls = parser.parse(raw_model_output)
```

Available parsers: `kunming`, `mistral`, `llama3_json`, `qwen`, `qwen3_coder`, `deepseek_v3`, `deepseek_v3_1`, `kimi_k2`, `longcat`, `glm45`, `glm47`.
...
Dataset: [km/km-model](https://huggingface.co/datasets/kangshijie8/terminal-bench-2) on HuggingFace.
...
TBLite is a thin subclass of TerminalBench2 — only the dataset and timeouts differ. Created by the OpenThoughts Agent team (Snorkel AI + Bespoke Labs). Dataset: [km/km-model](https://huggingface.co/datasets/kangshijie8/openthoughts-tblite).
...
pip install "kunming-agent[yc-bench]"
...
```bash
python environments/kunming_swe_env/kunming_swe_env.py serve \
...
- Set `tool_call_parser` in config to match your model's format (e.g., `"kunming"`, `"qwen"`, `"mistral"`)
...
```python
from environments.kunming_base_env import KunmingAgentBaseEnv, KunmingAgentEnvConfig
from atroposlib.envs.server_handling.server_manager import APIServerConfig

class MyEnvConfig(KunmingAgentEnvConfig):
    my_custom_field: str = "default_value"

class MyEnv(KunmingAgentBaseEnv):
    name = "my-env"
...
### KunmingAgentEnvConfig Fields

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled_toolsets` | `List[str]` | `None` (all) | Which km toolsets to enable |
...
| `tool_call_parser` | `str` | `"kunming"` | Parser for Phase 2 raw output |
...
env:
  ...
  dataset_name: "km/km-model"
  tokenizer_name: "km/km-model"
  use_wandb: true
...
- Python >= 3.11
- `atroposlib`: `pip install git+https://github.com/kunming/atropos.git`
- An LLM API key (OpenRouter, OpenAI, or self-hosted VLLM/SGLang)

### For Modal-sandboxed benchmarks (TB2, TBLite)

- [Modal](https://modal.com) account and CLI: `pip install "kunming-agent[modal]"`
- `MODAL_TOKEN_ID` and `MODAL_TOKEN_SECRET` environment variables

### For YC-Bench

- `pip install "kunming-agent[yc-bench]"` (installs the yc-bench CLI + SQLAlchemy)
...
```
environments/
├── kunming_base_env.py          # Abstract base class (KunmingAgentBaseEnv)
├── agent_loop.py               # Multi-turn agent engine (KunmingAgentLoop)
├── tool_context.py             # Per-rollout tool access for reward functions
├── patches.py                  # Async-safety patches for Modal backend
│
├── tool_call_parsers/          # Phase 2 client-side parsers
│   ├── kunming_parser.py        # Kunming/ChatML format
...
├── kunming_swe_env/             # SWE-bench training environment
│
