...
The ACP adapter wraps Kunming' synchronous `AIAgent` in an async JSON-RPC stdio server.
...
```text
kunming acp / kunming-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> load ~/.kunming/.env
  -> configure stderr logging
  -> construct KunmingACPAgent
  -> acp.run_agent(agent)
```
...
- `allow_once` -> Kunming `once`
- `allow_always` -> Kunming `always`
- reject options -> Kunming `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps km tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["kunming-acp"])
...
Instead it reuses Kunming' runtime resolver:

- `acp_adapter/auth.py`
- `kunming_cli/runtime_provider.py`

So ACP advertises and uses the currently configured Kunming provider/credentials.
...
- `toolsets.py` �?`kunming-acp` toolset definition
- `kunming_cli/main.py` �?`kunming acp` CLI subcommand
- `pyproject.toml` �?`[acp]` optional dependency + `kunming-acp` script
