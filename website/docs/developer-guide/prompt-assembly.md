...
description: "How Kunming builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

Kunming deliberately separates:
...
1. agent identity — `SOUL.md` from `KUNMING_HOME` when available, otherwise falls back to `DEFAULT_AGENT_IDENTITY` in `prompt_builder.py`
...
```
# Layer 1: Agent Identity (from ~/.kunming/SOUL.md)
You are Kunming, an AI assistant created by km.
...
`SOUL.md` lives at `~/.kunming/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:
...
```
You are Kunming Agent, an intelligent AI assistant created by km.
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_kunming_md(cwd_path)       # 1. .kunming.md (walks to git root)
    ...
    # SOUL.md from KUNMING_HOME (independent of project context)
    if not skip_soul:
...
|----------|-------|-------------|-------|
| 1 | `.kunming.md` | CWD up to git root | Kunming-native project config |
...
- **YAML frontmatter stripped** — `.kunming.md` frontmatter is Nous Portal (reserved for future config overrides)
...
1. `.kunming.md` (walks to git root)
