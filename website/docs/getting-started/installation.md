...
sidebar_position: 2
title: "Installation"
description: "Install Kunming Agent on Linux, macOS, or WSL2"
---

# Installation

Get Kunming Agent up and running in under two minutes with the one-line installer, or follow the manual steps for full control.
...
:::warning Windows
Native Windows is **not supported**. Please install [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) and run Kunming Agent from there. The install command above works inside WSL2.
:::

### What the Installer Does

The installer handles everything automatically — all dependencies (Python, Node.js, ripgrep, ffmpeg), the repo clone, virtual environment, global `kunming` command setup, and LLM provider configuration. By the end, you're ready to chat.
...
km gateway setup  # Set up messaging platforms
km config set     # Set individual config values
km setup          # Or run the full setup wizard to configure everything at once
```
...
```bash
git clone --recurse-submodules https://github.com/kangshijie8/KM-Agent.git
cd kunming-agent
```
...
:::tip
You do **not** need to activate the venv to use `kunming`. The entry point has a hardcoded shebang pointing to the venv Python, so it works globally once symlinked.
:::
...
mkdir -p ~/.kunming/{cron,sessions,logs,memories,skills,pairing,hooks,image_cache,audio_cache,whatsapp/session}

# Copy the example config file
cp cli-config.yaml.example ~/.kunming/config.yaml
...
Open `~/.kunming/.env` and add at minimum an LLM provider key:
...
```bash
km config set OPENROUTER_API_KEY sk-or-v1-your-key-here
```

### Step 8: Add `kunming` to Your PATH

```bash
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/kunming" ~/.local/bin/kunming
```
...
```bash
km model       # Select your LLM provider and model
```

### Step 10: Verify the Installation

```bash
kunming version    # Check that the command is available
km doctor     # Run diagnostics to verify everything is working
kunming status     # Check your configuration
km chat -q "Hello! What tools do you have available?"
```
...
cp cli-config.yaml.example ~/.kunming/config.yaml
touch ~/.kunming/.env
echo 'OPENROUTER_API_KEY=sk-or-v1-your-key' >> ~/.kunming/.env

# Make kunming available globally
mkdir -p ~/.local/bin
ln -sf "$(pwd)/venv/bin/kunming" ~/.local/bin/kunming
...
| Problem | Solution |
|---------|----------|
| `kunming: command not found` | Reload your shell (`source ~/.bashrc`) or check PATH |
| `API key not set` | Run `km model` to configure your provider, or `km config set OPENROUTER_API_KEY your_key` |
| Missing config after update | Run `km config check` then `km config migrate` |

For more diagnostics, run `km doctor` — it will tell you exactly what's missing and how to fix it.
