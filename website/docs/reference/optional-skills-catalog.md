...
description: "Official optional skills shipped with kunming-agent — install via km skills install official/<category>/<skill>"
---

# Optional Skills Catalog

Official optional skills ship with the kunming-agent repository under `optional-skills/` but are **not active by default**. Install them explicitly:
...
| **honcho** | Configure and use Honcho memory with Kunming — cross-session user modeling, multi-profile peer isolation, observation config, and dialectic reasoning. |
...
| Skill | Description |
|-------|-------------|
| **blender-mcp** | Control Blender directly from Kunming via socket connection to the blender-mcp addon. Create 3D objects, materials, animations, and run arbitrary Blender Python (bpy) code. |
...
| **kunming-atropos-environments** | Build, test, and debug Kunming Agent RL environments for Atropos training. Covers the KunmingAgentBaseEnv interface, reward functions, agent loop integration, and evaluation. |
...
| **telephony** | Give Kunming phone capabilities — provision a Twilio number, send/receive SMS/MMS, make calls, and place AI-driven outbound calls through Bland.ai or Vapi. |
