...
description: "Frequently asked questions and solutions to common issues with Kunming Agent"
---
...
### What LLM providers work with Kunming?

Kunming Agent works with any OpenAI-compatible API. Supported providers include:

- **[OpenRouter](https://openrouter.ai/)** — access hundreds of models through one API key (recommended for flexibility)
- **Nous Portal** —  Research's own inference endpoint
...
Set your provider with `km model` or by editing `~/.kunming/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.

### Does it work on Windows?

**Not natively.** Kunming Agent requires a Unix-like environment. On Windows, install [WSL2](https://learn.microsoft.com/en-us/windows/wsl/install) and run Kunming from inside it. The standard install command works perfectly in WSL2:
...
API calls go **only to the LLM provider you configure** (e.g., OpenRouter, your local Ollama instance). Kunming Agent does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.kunming/`.

### Can I use it offline / with local models?

Yes. Run `km model`, select **Custom endpoint**, and enter your server's URL:
...
Kunming persists the endpoint, provider, and base URL in `config.yaml` so it survives restarts. If your local server has exactly one model loaded, `/model custom` auto-detects it. You can also set `provider: custom` in config.yaml — it's a first-class provider, not an alias for anything else.
...
:::tip Ollama users
If you set a custom `num_ctx` in Ollama (e.g., `ollama run --num_ctx 16384`), make sure to set the matching context length in Kunming — Ollama's `/api/show` reports the model's *maximum* context, not the effective `num_ctx` you configured.
:::

### How much does it cost?

Kunming Agent itself is **free and open-source** (MIT license). You pay only for the LLM API usage from your chosen provider. Local models are completely free to run.

### Can multiple people use one instance?

Yes. The [messaging gateway](../user-guide/messaging/index.md) lets multiple users interact with the same Kunming Agent instance via Telegram, Discord, Slack, WhatsApp, or Home Assistant. Access is controlled through allowlists (specific user IDs) and DM pairing (first user to message claims access).
...
Yes. Import the `AIAgent` class and use Kunming programmatically:

```python
from run_agent import AIAgent

agent = AIAgent(model="openrouter/nous/kunming-3-llama-3.1-70b")
...
#### `km: command not found` after installation
...
**Cause:** Kunming requires Python 3.11 or newer.
...
# Or set directly
km config set OPENROUTER_API_KEY sk-or-v1-xxxxxxxxxxxx
```

:::warning
Make sure the key matches the provider. An OpenAI key won't work with OpenRouter and vice versa. Check `~/.kunming/.env` for conflicting entries.
:::
...
# Set a valid model
km config set KUNMING_MODEL openrouter/nous/kunming-3-llama-3.1-70b

# Or specify per-session
km chat --model openrouter/meta-llama/llama-3.1-70b-instruct
```
...
- Using `km chat --provider <alternative>` to route to a different backend

#### Context length exceeded

**Cause:** The conversation has grown too long for the model's context window, or Kunming detected the wrong context length for your model.
...
km chat --model openrouter/google/gemini-3-flash-preview
```

If this happens on the first long conversation, Kunming may have the wrong context length for your model. Check what it detected:
...
**Cause:** Kunming detected a potentially destructive command (e.g., `rm -rf`, `DROP TABLE`). This is a safety feature.
...
:::tip
This is working as intended — Kunming never silently runs destructive commands. The approval prompt shows you exactly what will execute.
:::
...
- Or switch to the terminal interface for administrative tasks: `km chat`
...
**Solution:**
- Verify your bot token is valid with `km gateway setup`
- Check gateway logs: `cat ~/.kunming/logs/gateway.log | tail -50`
...
Configure in `~/.kunming/config.yaml` under your gateway's settings. See the [Messaging docs](../user-guide/messaging/index.md).
...
pip install "kunming-agent[telegram]"   # or [discord], [slack], [whatsapp]
...
**Solution:** The gateway captures your shell PATH when you run `km gateway install`. If you installed tools after setting up the gateway, re-run the install to capture the updated PATH:

```bash
km gateway install    # Re-snapshots your current PATH
km gateway start      # Detects the updated plist and reloads
```

You can verify the plist has the correct PATH:
```bash
/usr/libexec/PlistBuddy -c "Print :EnvironmentVariables:PATH" \
  ~/Library/LaunchAgents/ai.kunming.gateway.plist
```
...
**Solution:**
- Try a faster/smaller model: `km chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- Reduce active toolsets: `km chat -t "terminal"`
...
cd ~/.kunming/kunming-agent && uv pip install -e ".[mcp]"
...
Verify your `~/.kunming/config.yaml` MCP configuration:
```yaml
mcp_servers:
  filesystem:
    command: "npx"
...
km config show | grep -A 12 mcp_servers

# Restart Kunming or reload MCP after config changes
km chat
```

See also:
- [MCP (Model Context Protocol)](/docs/user-guide/features/mcp)
- [Use MCP with Kunming](/docs/guides/use-mcp-with-kunming)
...
:::warning
If an MCP server crashes mid-request, Kunming will report a timeout. Check the server's own logs (not just km logs) to diagnose the root cause.
:::
...
No. Each profile has its own memory store, session database, and skills directory. They are completely isolated. If you want to start a new profile with existing memories and sessions, use `km profile create newname --clone-all` to copy everything from the current profile.

### What happens when I run `km update`?

`km update` pulls the latest code and reinstalls dependencies **once** (not per-profile). It then syncs updated skills to all profiles automatically. You only need to run `km update` once — it covers every profile on the machine.
...
km profile export work ./work-backup.tar.gz

# Copy the file to the target machine, then:
km profile import ./work-backup.tar.gz work
```
...
There is no hard limit. Each profile is just a directory under `~/.kunming/profiles/`. The practical limit depends on your disk space and how many concurrent gateways your system can handle (each gateway is a lightweight Python process). Running dozens of profiles is fine; each idle profile uses no resources.
...
**Solution: Delegation config.** Kunming can route subagents to a different model automatically. Set this in `~/.kunming/config.yaml`:
...
Now when you tell Kunming "write me a Twitter thread about X" and it spawns a `delegate_task` subagent, that subagent runs on Gemini instead of your main model. Your primary conversation stays on GPT-5.4.
...
**Scenario:** In a previous setup, you had multiple independent agents bound to specific WhatsApp chats — one for a family shopping list group, another for your private chat. Can Kunming do this?

**Current limitation:** Kunming profiles each require their own WhatsApp number/session. You cannot bind multiple profiles to different chats on the same WhatsApp number — the WhatsApp bridge (Baileys) uses one authenticated session per number.
...
**Scenario:** You see gateway exec logs, Kunming reasoning, and tool call details in Telegram instead of just the final output.
...
**Scenario:** Telegram has a 100 slash command limit, and your skills are pushing past it. You want to disable skills you don't need on Telegram, but `km skills config` settings don't seem to take effect.

**Solution:** Use `km skills config` to disable skills per-platform. This writes to `config.yaml`:
...
After changing this, **restart the gateway** (`km gateway restart` or kill and relaunch). The Telegram bot command menu rebuilds on startup.
...
**Current behavior:** Kunming creates sessions keyed by user ID on most platforms, so each person gets their own conversation context. This is by design for privacy and context isolation.
...
### Exporting Kunming to another machine
...
1. Install Kunming Agent on the new machine:
   ```bash
   curl -fsSL https://raw.githubusercontent.com/kunming/km-agent/main/scripts/install.sh | bash
   ```

2. Copy your entire `~/.kunming/` directory **except** the `kunming-agent` subdirectory (that's the code repo — the new install has its own):
   ```bash
   # On the source machine
   rsync -av --exclude='kunming-agent' ~/.kunming/ newmachine:~/.kunming/
   ```

   Or use profile export/import:
   ```bash
   # On source machine
   km profile export default ./kunming-backup.tar.gz

   # On target machine
   km profile import ./kunming-backup.tar.gz default
   ```

3. On the new machine, run `km setup` to verify API keys and provider config are working. Re-authenticate any messaging platforms (especially WhatsApp, which uses QR pairing).

The `~/.kunming/` directory contains everything: `config.yaml`, `.env`, `SOUL.md`, `memories/`, `skills/`, `state.db` (sessions), `cron/`, and any custom plugins. The code itself lives in `~/.kunming/kunming-agent/` and is installed fresh.

### Permission denied when reloading shell after install

**Scenario:** After running the Kunming installer, `source ~/.zshrc` gives a permission denied error.

**Cause:** This usually happens when `~/.zshrc` (or `~/.bashrc`) has incorrect file permissions, or when the installer couldn't write to it cleanly. It's not a Kunming-specific issue — it's a shell config permissions problem.
...
km config show | head -20

# Re-run model selection
km model

# Or test with a known-good model
km chat -q "hello" --model anthropic/claude-sonnet-4.6
```
...
1. **Search existing issues:** [GitHub Issues](https://github.com/kangshijie8/KM-Agent/issues)
2. **Ask the community:** [ Research Discord](https://discord.gg/kunming)
3. **File a bug report:** Include your OS, Python version (`python3 --version`), Kunming version (`km --version`), and the full error message
