...
description: "Catalog of bundled skills that ship with Kunming Agent"
---

# Bundled Skills Catalog

Kunming ships with a large built-in skill library copied into `~/.kunming/skills/` on install. This page catalogs the bundled skills that live in the repository under `skills/`.
...
| `kunming-agent-spawning` | Spawn additional Kunming Agent instances as autonomous subprocesses for independent long-running tasks. Supports non-interactive one-shot mode (-q) and interactive PTY mode for multi-turn collaboration. Different from delegate_task — this runs a full separate km process. | `autonomous-ai-agents/kunming-agent` |
...
| `kunming-agent-setup` | Help users configure Kunming Agent — CLI usage, setup wizard, model/provider selection, tools, skills, voice/STT/TTS, gateway, and troubleshooting. | `dogfood/kunming-agent-setup` |
...
| `native-mcp` | Built-in MCP (Model Context Protocol) client that connects to external MCP servers, discovers their tools, and registers them as native Kunming Agent tools. Supports stdio and HTTP transports with automatic reconnection, security filtering, and zero-config tool injection. | `mcp/native-mcp` |
...
| `kunming-atropos-environments` | Build, test, and debug Kunming Agent RL environments for Atropos training. Covers the KunmingAgentBaseEnv interface, reward functions, agent loop integration, evaluation with tools, wandb logging, and the three CLI modes (serve/process/evaluate). Use when creating, reviewing, or f… | `mlops/training/kunming-atropos-environments` |
...
| Skill | Description | Path |
|-------|-------------|------|
| `google-workspace` | Gmail, Calendar, Drive, Contacts, Sheets, and Docs integration via Python. Uses OAuth2 with automatic token refresh. No external binaries needed — runs entirely with Google's Python client libraries in the Kunming venv. | `productivity/google-workspace` |
...
| `plan` | Plan mode for Kunming — inspect context, write a markdown plan into `.kunming/plans/` in the active workspace/backend working directory, and do not execute the work. | `software-development/plan` |
...
```bash
km skills install official/<category>/<skill>
```
...
| Skill | Description | Path |
|-------|-------------|------|
| `blender-mcp` | Control Blender directly from Kunming via socket connection to the blender-mcp addon. Create 3D objects, materials, animations, and run arbitrary Blender Python (bpy) code. | `creative/blender-mcp` |
...
| Skill | Description | Path |
|-------|-------------|------|
| `agentmail` | Give the agent its own dedicated email inbox via AgentMail. Send, receive, and manage email autonomously using agent-owned email addresses (e.g. kunming-agent@agentmail.to). | `email/agentmail` |
...
| Skill | Description | Path |
|-------|-------------|------|
| `telephony` | Give Kunming phone capabilities — provision and persist a Twilio number, send and receive SMS/MMS, make direct calls, and place AI-driven outbound calls through Bland.ai or Vapi. | `productivity/telephony` |

## research

| Skill | Description | Path |
|-------|-------------|------|
| `bioinformatics` | Gateway to 400+ bioinformatics skills from bioSkills and ClawBio. Covers genomics, transcriptomics, single-cell, variant calling, pharmacogenomics, metagenomics, and structural biology. | `research/bioinformatics` |
