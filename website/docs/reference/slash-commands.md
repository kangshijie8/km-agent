...
Kunming has two slash-command surfaces, both driven by a central `COMMAND_REGISTRY` in `kunming_cli/commands.py`:
...
Installed skills are also exposed as dynamic slash commands on both surfaces. That includes bundled skills like `/plan`, which opens plan mode and saves markdown plans under `.kunming/plans/` relative to the active workspace/backend working directory.
...
| `/plan [request]` | Load the bundled `plan` skill to write a markdown plan instead of executing the work. Plans are saved under `.kunming/plans/` relative to the active workspace/backend working directory. |
...
User-defined quick commands map a short alias to a longer prompt. Configure them in `~/.kunming/config.yaml`:
