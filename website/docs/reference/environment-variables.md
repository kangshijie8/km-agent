...
description: "Complete reference of all environment variables used by Kunming Agent"
---

# Environment Variables Reference

All variables go in `~/.kunming/.env`. You can also set them with `km config set VAR value`.
...
| `KUNMING_COPILOT_ACP_COMMAND` | Override Copilot ACP CLI binary path (default: `copilot`) |
| `COPILOT_CLI_PATH` | Alias for `KUNMING_COPILOT_ACP_COMMAND` |
| `KUNMING_COPILOT_ACP_ARGS` | Override Copilot ACP arguments (default: `--acp --stdio`) |
...
| `KUNMING_MODEL` | Preferred model name (checked before `LLM_MODEL`, used by gateway) |
| `LLM_MODEL` | Default model name (fallback when not set in config.yaml) |
| `VOICE_TOOLS_OPENAI_KEY` | Preferred OpenAI key for OpenAI speech-to-text and text-to-speech providers |
| `KUNMING_LOCAL_STT_COMMAND` | Optional local speech-to-text command template. Supports `{input_path}`, `{output_dir}`, `{language}`, and `{model}` placeholders |
| `KUNMING_LOCAL_STT_LANGUAGE` | Default language passed to `KUNMING_LOCAL_STT_COMMAND` or auto-detected local `whisper` CLI fallback (default: `en`) |
| `KUNMING_HOME` | Override km config directory (default: `~/.kunming`). Also scopes the gateway PID file and systemd service name, so multiple installations can run concurrently |

## Provider Auth (OAuth)

For native Anthropic auth, Kunming prefers Claude Code's own credential files when they exist because those credentials can refresh automatically. Environment variables such as `ANTHROPIC_TOKEN` remain useful as manual overrides, but they are no longer the preferred path for Claude Pro/Max login.

| Variable | Description |
|----------|-------------|
| `KUNMING_INFERENCE_PROVIDER` | Override provider selection: `auto`, `openrouter`, `nous`, `openai-codex`, `copilot`, `copilot-acp`, `anthropic`, `huggingface`, `zai`, `kimi-coding`, `minimax`, `minimax-cn`, `kilocode`, `alibaba`, `deepseek`, `opencode-zen`, `opencode-go`, `ai-gateway` (default: `auto`) |
| `KUNMING_PORTAL_BASE_URL` | Override Nous Portal URL (for development/testing) |
| `NOUS_INFERENCE_BASE_URL` | Override  inference API URL |
| `KUNMING_NOUS_MIN_KEY_TTL_SECONDS` | Min agent key TTL before re-mint (default: 1800 = 30min) |
| `KUNMING_NOUS_TIMEOUT_SECONDS` | HTTP timeout for  credential / token flows |
| `KUNMING_DUMP_REQUESTS` | Dump API request payloads to log files (`true`/`false`) |
| `KUNMING_PREFILL_MESSAGES_FILE` | Path to a JSON file of ephemeral prefill messages injected at API-call time |
| `KUNMING_TIMEZONE` | IANA timezone override (for example `America/New_York`) |
...
For cloud sandbox backends, persistence is filesystem-oriented. `TERMINAL_LIFETIME_SECONDS` controls when Kunming cleans up an idle terminal session, and later resumes may recreate the sandbox rather than keep the same live processes running.
...
| `TERMINAL_SANDBOX_DIR` | Host directory for workspaces and overlays (default: `~/.kunming/sandboxes/`) |
...
| `MATRIX_USER_ID` | Matrix user ID (e.g. `@kunming:matrix.org`) — required for password login, optional with access token |
...
| Variable | Description |
|----------|-------------|
| `KUNMING_MAX_ITERATIONS` | Max tool-calling iterations per conversation (default: 90) |
| `KUNMING_TOOL_PROGRESS` | Deprecated compatibility variable for tool progress display. Prefer `display.tool_progress` in `config.yaml`. |
| `KUNMING_TOOL_PROGRESS_MODE` | Deprecated compatibility variable for tool progress mode. Prefer `display.tool_progress` in `config.yaml`. |
| `KUNMING_HUMAN_DELAY_MODE` | Response pacing: `off`/`natural`/`custom` |
| `KUNMING_HUMAN_DELAY_MIN_MS` | Custom delay range minimum (ms) |
| `KUNMING_HUMAN_DELAY_MAX_MS` | Custom delay range maximum (ms) |
| `KUNMING_QUIET` | Suppress non-essential output (`true`/`false`) |
| `KUNMING_API_TIMEOUT` | LLM API call timeout in seconds (default: `1800`) |
| `KUNMING_EXEC_ASK` | Enable execution approval prompts in gateway mode (`true`/`false`) |
| `KUNMING_ENABLE_PROJECT_PLUGINS` | Enable auto-discovery of repo-local plugins from `./.kunming/plugins/` (`true`/`false`, default: `false`) |
| `KUNMING_BACKGROUND_NOTIFICATIONS` | Background process notification mode in gateway: `all` (default), `result`, `error`, `off` |
| `KUNMING_EPHEMERAL_SYSTEM_PROMPT` | Ephemeral system prompt injected at API-call time (never persisted to sessions) |
...
For task-specific direct endpoints, Kunming uses the task's configured API key or `OPENAI_API_KEY`. It does not reuse `OPENROUTER_API_KEY` for those custom endpoints.
...
These go in `~/.kunming/config.yaml` under the `provider_routing` section:
...
:::tip
Use `km config set` to set environment variables — it automatically saves them to the right file (`.env` for secrets, `config.yaml` for everything else).
:::
