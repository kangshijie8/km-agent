---
sidebar_position: 8
title: "Use Voice Mode with Kunming"
description: "A practical guide to setting up and using Kunming voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with Kunming
...
- you want Kunming sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in Kunming.
...
## Step 1: make sure normal Kunming works first
...
Kunming supports both local and cloud speech stacks.
...
If you choose NeuTTS in the setup wizard, Kunming checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. Kunming transcribes and responds
...
- asking Kunming to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full Kunming loop.

## Tuning CLI behavior

### Silence threshold

If Kunming starts/stops too aggressively, tune:
...
Kunming stays a normal chat bot, but can speak replies.
...
- you want Kunming to function like a portable research or ops assistant
...
Kunming joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- Kunming detects speech boundaries
- transcripts are posted in the associated text channel
- Kunming responds in text and audio
...
1. get text Kunming working
2. install `kunming-agent[voice]`
