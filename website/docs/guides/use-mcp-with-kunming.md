---
sidebar_position: 6
title: "Use MCP with Kunming"
description: "A practical guide to connecting MCP servers to Kunming Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Kunming

This guide shows how to actually use MCP with Kunming Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Kunming tool
- you want Kunming to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Kunming to internal APIs, databases, or company systems without modifying Kunming core

Do not use MCP when:
- a built-in Kunming tool already solves the job well
...
- Kunming remains the agent
- MCP servers contribute tools
- Kunming discovers those tools at startup or reload time
...
If you installed Kunming with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Kunming banner/status should show MCP integration when configured
- ask Kunming what tools it has available
...
There are two categories of MCP-exposed functionality in Kunming:
...
2. Kunming-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Kunming will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Kunming to reason over a bounded workspace.
...
Now Kunming can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Kunming core.
...
Because Kunming now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
