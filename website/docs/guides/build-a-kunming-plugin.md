...
title: "Build a Kunming Plugin"
description: "Step-by-step guide to building a complete Kunming plugin with tools, hooks, data files, and skills"
---

# Build a Kunming Plugin

This guide walks through building a complete Kunming plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill �?everything the plugin system supports.
...
```bash
mkdir -p ~/.kunming/plugins/calculator
cd ~/.kunming/plugins/calculator
```
...
This tells Kunming: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.
...
4. **Accept `**kwargs`:** Kunming may pass additional context in the future.
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `kunming my-plugin <subcommand>`)
- If this function crashes, the plugin is disabled but Kunming continues fine
...
def _install_skill():
    """Copy our skill to ~/.kunming/skills/ on first load."""
    try:
        from kunming_cli.config import get_kunming_home
        dest = get_kunming_home() / "skills" / "my-plugin" / "SKILL.md"
    except Exception:
        dest = Path.home() / ".kunming" / "skills" / "my-plugin" / "SKILL.md"
...
When users run `kunming plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
This is the only hook whose return value matters. When a `pre_llm_call` callback returns a dict with a `"context"` key (or a plain string), Kunming injects that text into the **current turn's user message**. This is the mechanism for memory plugins, RAG integrations, guardrails, and any plugin that needs to provide the model with additional context.

#### Return format

```python
# Dict with context key
return {"context": "Recalled memories:\n- User prefers dark mode\n- Last project: kunming-agent"}
...
- **The system prompt is Kunming's territory** �?it contains model-specific guidance, tool enforcement rules, personality instructions, and cached skill content. Plugins contribute context alongside the user's input, not by altering the agent's core instructions.
...
Plugins can add their own `kunming <plugin>` subcommand tree:

```python
def _my_command(args):
    """Handler for kunming my-plugin <subcommand>."""
    ...
    if sub == "status":
        ...
        print("Usage: kunming my-plugin <status|config>")

def _setup_argparse(subparser):
    """Build the argparse tree for kunming my-plugin."""
...
After registration, users can run `kunming my-plugin status`, `kunming my-plugin config`, etc.
...
```toml
# pyproject.toml
[project.entry-points."kunming_agent.plugins"]
my-plugin = "my_plugin_package"
```

```bash
pip install kunming-plugin-calculator
# Plugin auto-discovered on next kunming startup
```
...
```python
# Wrong �?will break if Kunming passes extra context
def handler(args):
    ...
