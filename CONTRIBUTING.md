# Contributing to Kunming Agent

Thank you for contributing to Kunming Agent! This guide covers everything you need: setting up your dev environment, understanding the architecture, deciding what to build, and getting your PR merged.
...
Bundled skills (in `skills/`) ship with every km install. They should be **broadly useful to most users**:
...
```bash
mkdir -p ~/.kunming/{cron,sessions,logs,memories,skills}
...
### User configuration (stored in `~/.kunming/`)

| Path | Purpose |
|------|---------|
| `~/.kunming/config.yaml` | Settings (model, terminal, toolsets, compression, etc.) |
| `~/.kunming/.env` | API keys and secrets |
| `~/.kunming/auth.json` | OAuth credentials (Nous Portal) |
| `~/.kunming/skills/` | All active skills (bundled + hub-installed + agent-created) |
| `~/.kunming/memories/` | Persistent memory (MEMORY.md, USER.md) |
| `~/.kunming/state.db` | SQLite session database |
| `~/.kunming/sessions/` | JSON session logs |
| `~/.kunming/cron/` | Scheduled job data |
| `~/.kunming/whatsapp/session/` | WhatsApp bridge credentials |
...
- **Session persistence**: All conversations are stored in SQLite (`kunming_state.py`) with full-text search and unique session titles. JSON logs go to `~/.kunming/sessions/`.
...
Gateway and messaging sessions never collect secrets in-band; they instruct the user to run `km setup` or update `~/.kunming/.env` locally.
...
Create `~/.kunming/skins/<name>.yaml`:
...
Add to `_BUILTIN_SKINS` dict in `kunming_cli/skin_engine.py`. Use the same schema as above but as a Python dict. Built-in skins ship with the package and are always available.

**Activating:**
- CLI: `/skin mytheme` or set `display.skin: mytheme` in config.yaml
- Config: `display: { skin: mytheme }`

See `kunming_cli/skin_engine.py` for the full schema and existing skins as examples.

---

## Cross-Platform Compatibility

km runs on Linux, macOS, and Windows. When writing code that touches the OS:
...
km has terminal access. Security matters.
...
2. **Test manually**: Run `km` and exercise the code path you changed
...
- Use [GitHub Issues](https://github.com/kangshijie8/KM-Agent/issues)
- Include: OS, Python version, km version (`km version`), full error traceback
