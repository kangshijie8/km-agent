#!/usr/bin/env python3
"""
SWE Runner with Kunming Trajectory Format

A runner that uses kunming-agent's built-in execution environments
(local, docker, modal) and outputs trajectories in the kunming-agent format
compatible with batch_runner.py and trajectory_compressor.py.

Features:
- Uses kunming-agent's Docker, Modal, or Local environments for command execution
- Outputs trajectories in Kunming format (from/value pairs with <tool_call>/<tool_response> XML)
...
# ============================================================================
# Terminal Tool Definition (matches kunming-agent format)
# ============================================================================
...
def create_environment(
    ...
    **kwargs
    ...
    """
    Create an execution environment using kunming-agent's built-in backends.
...
# ============================================================================
# Mini-SWE Runner with Kunming Trajectory Format
# ============================================================================

class MiniSWERunner:
    """
    Agent runner that uses kunming-agent's built-in execution environments
    and outputs trajectories in kunming-agent format.
    """
    ...
    def _convert_to_kunming_format(
        self,
        ...
        """
        Convert internal message format to Kunming trajectory format.
    ...
    def run_task(self, task: str) -> Dict[str, Any]:
        ...
        """
        print(f"\n{'='*60}")
        print(f"[CONFIG] Task: {task[:80]}{'...' if len(task) > 80 else ''}")
        print(f"{'='*60}")
...
Complete the user's task step by step."""
        ...
        # Convert to Kunming trajectory format
        trajectory = self._convert_to_kunming_format(messages, task, completed)
    ...
    def run_batch(
        ...
        print(f"[FILES] Output: {output_file}")
...
def main(
    ...
    """
    Run SWE tasks with Kunming trajectory format output.
    ...
    """
    print("[READY] Mini-SWE Runner with Kunming Trajectory Format")
    print("=" * 60)
    ...
    if task:
        ...
        print(f"\n[FILES] Trajectory saved to: {output_file}")
        print(f"✅ Completed: {result['completed']}")
        print(f"📞 API calls: {result['api_calls']}")
        print(f"[CHAT] Turns: {len(result['conversations'])}")
