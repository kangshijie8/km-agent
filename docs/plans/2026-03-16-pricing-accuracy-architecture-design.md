...
km should only show dollar costs when they are backed by an official source for the user's actual billing path.
...
Current km behavior has four structural issues:
...
km must stop keying pricing solely by `model`.
...
- manual `km pricing sync`

## Reconciliation Architecture

Live requests may produce only an estimate initially. km should reconcile them later when a provider exposes actual billed cost.

Suggested flow:

1. Agent call completes.
2. km stores canonical usage plus reconciliation ids.
3. km computes an immediate estimate if a pricing source exists.
...
That path cannot satisfy the product requirement. km should instead migrate to:
...
This is the minimum architecture that makes the statement "km pricing is backed by official sources where possible, and otherwise clearly labeled" defensible.
