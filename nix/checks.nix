...
{ inputs, ... }: {
  perSystem = { pkgs, system, lib, ... }:
    let
      kunming-agent = inputs.self.packages.${system}.default;
      ...
      configKeys = pkgs.runCommand "kunming-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${kunmingVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = lib.optionalAttrs pkgs.stdenv.hostPlatform.isLinux {
        ...
        package-contents = pkgs.runCommand "kunming-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${kunming-agent}/bin/kunming || (echo "FAIL: kunming binary missing"; exit 1)
          test -x ${kunming-agent}/bin/kunming-agent || (echo "FAIL: kunming-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${kunming-agent}/bin/kunming version 2>&1 | grep -qi "kunming" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "kunming-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in kunming kunming-agent kunming-acp; do
            test -x ${kunming-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "kunming-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking kunming --help ==="
          ${kunming-agent}/bin/kunming --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${kunming-agent}/bin/kunming --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "kunming-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${kunming-agent}/share/kunming-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${kunming-agent}/share/kunming-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "KUNMING_BUNDLED_SKILLS" ${kunming-agent}/bin/kunming || \
            (echo "FAIL: KUNMING_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: KUNMING_BUNDLED_SKILLS set in wrapper"
        ...
        # Verify KUNMING_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "kunming-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(KUNMING_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking KUNMING_MANAGED guards ==="
          check_blocked "config set" ${kunming-agent}/bin/km config set model foo
          check_blocked "config edit" ${kunming-agent}/bin/km config edit
        ...
        in pkgs.runCommand "kunming-config-roundtrip" {
          ...
          merge_and_load() {
            local kunming_home="$1"
            export KUNMING_HOME="$kunming_home"
            ${configMergeScript} ${nixSettings} "$kunming_home/config.yaml"
            ${kunmingVenv}/bin/python3 -c '
import json, sys
