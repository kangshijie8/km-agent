...
discovers their tools, and registers them into the kunming-agent tool registry
so the agent can call them like any built-in tool.

Configuration is read from ~/.kunming/config.yaml under the ``mcp_servers`` key.
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            candidates = [
                os.path.join(kunming_home, "node", "bin", resolved_command),
...
class MCPServerTask:
    ...
    async def _refresh_tools(self):
        ...
        async with self._refresh_lock:
            ...
            # 2. Remove old tools from kunming-* umbrella toolsets
            for ts_name, ts in TOOLSETS.items():
                if ts_name.startswith("kunming-"):
...
def _snapshot_child_pids() -> set:
    ...
    # Linux: read from /proc (not available on Windows)
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Kunming config file.
    ...
    ``os.environ`` (which includes ``~/.kunming/.env`` loaded at startup).
    """
    try:
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Converts hyphens to underscores, and also replaces any other character outside
...
def _sync_mcp_toolsets(server_names: Optional[List[str]] = None) -> None:
    """Expose each MCP server as a standalone toolset and inject into kunming-* sets.
    ...
    Also injects all MCP tools into kunming-* umbrella toolsets for the
    default behavior.
    ...
    # Also inject into kunming-* umbrella toolsets for default behavior.
    for ts_name, ts in TOOLSETS.items():
        if not ts_name.startswith("kunming-"):
            continue
...
def _register_server_tools(name: str, server: MCPServerTask, config: dict) -> List[str]:
    ...
    and kunming-* umbrella toolset injection.
    ...
    if registered_names:
        ...
        # Inject into kunming-* umbrella toolsets for default behavior
        for ts_name, ts in TOOLSETS.items():
            if ts_name.startswith("kunming-"):
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``kunming tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Kunming registry.
