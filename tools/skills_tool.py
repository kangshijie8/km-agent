...
Directory Structure:
    skills/
    â-- my-skill/
    +  â-- SKILL.md           # Main instructions (required)
    +  â-- references/        # Supporting documentation
    +  +  â-- api.md
    +  +  â-- examples.md
    +  â-- templates/         # Templates for output
    +  +  â-- template.md
    +  â-- assets/            # Supplementary files (agentskills.io standard)
...
# All skills live in ~/.kunming/skills/ (seeded from bundled skills/ on install).
...
def _is_gateway_surface() -> bool:
    if os.getenv("KUNMING_GATEWAY_SESSION"):
        return True
    return bool(os.getenv("KUNMING_SESSION_PLATFORM"))
...
def _gateway_setup_hint() -> str:
    try:
        ...
        return "Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to ~/.kunming/.env manually."
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.kunming/skills/mlops/axolotl/SKILL.md -> "mlops"
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    try:
        ...
        resolved_platform = platform or os.getenv("KUNMING_PLATFORM")
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.kunming/skills/ and external dirs.

    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``km skills`` config UI). Default False
            ...
            return json.dumps(
                {
                    ...
                    "skills": [],
                    "categories": [],
                    "message": "No skills found. Skills directory created at ~/.kunming/skills/",
                },
                ensure_ascii=False,
            )
    ...
    try:
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.kunming/skills/): {skill_md}")
            ...
            return json.dumps(
                {
                    ...
                    "error": (
                        ...
                        "Enable it with `km skills` or inspect the files directly on disk."
                    ),
                },
                ensure_ascii=False,
            )
        ...
        # Check metadata.kunming.* first (agentskills.io convention), fall back to top-level
        kunming_meta = {}
        metadata = frontmatter.get("metadata")
        if isinstance(metadata, dict):
            kunming_meta = metadata.get("kunming", {}) or {}

        tags = _parse_tags(kunming_meta.get("tags") or frontmatter.get("tags", ""))
        related_skills = _parse_tags(
            kunming_meta.get("related_skills") or frontmatter.get("related_skills", "")
        )
