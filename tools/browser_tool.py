...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-kunming_...`` the resulting socket path exceeds the
...
BROWSER_TOOL_SCHEMAS = [
    {
        ...
        "description": "Navigate to a URL in the browser. Initializes the session and loads the page. Must be called before other browser tools. For simple information retrieval, prefer search or extract tools if available (faster, cheaper). Use browser tools when you need to interact with a page (click, fill forms, dynamic content). Returns a compact page snapshot with interactive elements and ref IDs -no need to call browser_snapshot separately after navigating.",
        "parameters": {
            "type": "object",
            "properties": {
                "url": {
    ...
    },
    {
        ...
        "description": "Get a list of all images on the current page with their URLs and alt text. Useful for finding images to analyze visually. Requires browser_navigate to be called first.",
        "parameters": {
            "type": "object",
            "properties": {},
            "required": []
...
def _find_agent_browser() -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, Kunming-managed
    ...
    # Build an extended search PATH including Homebrew and Kunming-managed dirs.
    ...
    kunming_node_bin = str(kunming_home / "node" / "bin")
    if os.path.isdir(kunming_node_bin):
        extra_dirs.append(kunming_node_bin)
...
def _run_browser_command(
    ...
    try:
        ...
        # Ensure PATH includes Kunming-managed Node first, Homebrew versioned
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = kunming_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> str:
    ...
    screenshots_dir = get_kunming_dir("cache/screenshots", "browser_screenshots")
...
if __name__ == "__main__":
    ...
    print("\nð Available Browser Tools:")
    for schema in BROWSER_TOOL_SCHEMAS:
        print(f"  ð¹ {schema['name']}: {schema['description'][:60]}...")
