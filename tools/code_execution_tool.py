...
Lets the LLM write a Python script that calls Kunming tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `kunming_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `kunming_tools.py` with file-based RPC stubs
...
# ---------------------------------------------------------------------------
# kunming_tools.py code generator
# ---------------------------------------------------------------------------
...
def generate_kunming_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the kunming_tools.py stub module.
...
"""Auto-generated Kunming tools RPC stubs."""
...
def _connect():
    ...
    if _sock is None:
        ...
        _sock.connect(os.environ["KUNMING_RPC_SOCKET"])
...
"""Auto-generated Kunming tools RPC stubs (file-based transport)."""
import json, os, shlex, time

_RPC_DIR = os.environ.get("KUNMING_RPC_DIR", os.path.join(tempfile.gettempdir(), "kunming_rpc"))
_seq = 0
''' + _COMMON_HELPERS + '''\
...
def _execute_remote(
    ...
    The script and the generated kunming_tools.py module are shipped to
    ...
    sandbox_dir = os.path.join(tempfile.gettempdir(), f"kunming_exec_{sandbox_id}")
    ...
    try:
        ...
        tools_src = generate_kunming_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/kunming_tools.py", tools_src)
...
def execute_code(
    ...
    # --- Set up temp directory with kunming_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="kunming_sandbox_")
    # Use /tmp on macOS to avoid the long /var/folders/... path that pushes
    # Unix domain socket paths past the 104-byte macOS AF_UNIX limit.
    # On Linux, tempfile.gettempdir() already returns /tmp.
    _sock_tmpdir = "/tmp" if sys.platform == "darwin" else tempfile.gettempdir()
    sock_path = os.path.join(_sock_tmpdir, f"kunming_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated kunming_tools module
        # sandbox_tools is already the correct set (intersection with session
        # tools, or SANDBOX_ALLOWED_TOOLS as fallback — see lines above).
        tools_src = generate_kunming_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "kunming_tools.py"), "w") as f:
            f.write(tools_src)
...
def build_execute_code_schema(enabled_sandbox_tools: set = None) -> dict:
    ...
    When tools are disabled via ``km tools`` (e.g. web is turned off),
    ...
    description = (
        "Run a Python script that can call Kunming tools programmatically. "
        ...
        f"Available via `from kunming_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed — built into kunming_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from kunming_tools import {import_str}` "
