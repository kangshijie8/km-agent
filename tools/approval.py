...
def get_current_session_key(default: str = "default") -> str:
    ...
    if session_key:
        return session_key
    return os.getenv("KUNMING_SESSION_KEY", default)

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $KUNMING_HOME.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_KUNMING_ENV_PATH = (
    r'(?:~\/\.kunming/|'
    r'(?:\$home|\$\{home\})/\.kunming/|'
    r'(?:\$kunming_home|\$\{kunming_home\})/)'
    r'\.env\b'
)
_SENSITIVE_WRITE_TARGET = (
    r'(?:/etc/|/dev/sd|'
    rf'{_SSH_SENSITIVE_PATH}|'
    rf'{_KUNMING_ENV_PATH})'
)
...
DANGEROUS_PATTERNS = [
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart kunming-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart kunming-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(kunming|gateway|cli\.py)\b', "kill kunming/gateway process (self-termination)"),
...
def prompt_dangerous_approval(command: str, description: str,
    ...
    try:
        while True:
            print()
            print(f"  [!]  DANGEROUS COMMAND: {description}")
            print(f"      {command}")
            print()
            if allow_permanent:
...
def check_dangerous_command(command: str, env_type: str,
    ...
    is_cli = os.getenv("KUNMING_INTERACTIVE")
    is_gateway = os.getenv("KUNMING_GATEWAY_SESSION")

    if not is_cli and not is_gateway:
        return {"approved": True, "message": None}

    if is_gateway or os.getenv("KUNMING_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    if os.getenv("KUNMING_YOLO_MODE") or approval_mode == "off":
        return {"approved": True, "message": None}

    is_cli = os.getenv("KUNMING_INTERACTIVE")
    is_gateway = os.getenv("KUNMING_GATEWAY_SESSION")
    is_ask = os.getenv("KUNMING_EXEC_ASK")
