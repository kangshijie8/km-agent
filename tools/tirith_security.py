...
it is automatically downloaded from GitHub releases to $KUNMING_HOME/bin/tirith.
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $KUNMING_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_kunming_bin_dir(), "tirith")
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $KUNMING_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases ->$KUNMING_HOME/bin/tirith
    ...
    kunming_bin = os.path.join(_kunming_bin_dir(), "tirith")
    if os.path.isfile(kunming_bin) and os.access(kunming_bin, os.X_OK):
        _resolved_path = kunming_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return kunming_bin
    ...
    # If a background install thread is running, don't start a parallel one -    # return the configured path; the OSError handler in check_command_security
