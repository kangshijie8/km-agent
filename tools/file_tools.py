...
def _is_blocked_device(filepath: str) -> bool:
    ...
    import platform
    if platform.system() != "Windows" and normalized.startswith("/proc/") and normalized.endswith(
        ("/fd/0", "/fd/1", "/fd/2")
    ):
        return True
    return False
...
def read_file_tool(path: str, offset: int = 1, limit: int = 500, task_id: str = "default") -> str:
    ...
    try:
        ...
        # -- Kunming internal path guard --------------------------------
        # Prevent prompt injection via catalog or hub metadata files.
        from kunming_constants import get_kunming_home as _get_hh
        _kunming_home = _get_hh().resolve()
        _blocked_dirs = [
            _kunming_home / "skills" / ".hub" / "index-cache",
            _kunming_home / "skills" / ".hub",
        ]
        for _blocked in _blocked_dirs:
            try:
                ...
                return json.dumps({
                    "error": (
                        f"Access denied: {path} is an internal Kunming cache file "
...
READ_FILE_SCHEMA = {
    "name": "read_file",
    "description": "Read a text file with line numbers and pagination. Use this instead of cat/head/tail in terminal. Output format: 'LINE_NUM|CONTENT'. Suggests similar filenames if not found. Use offset and limit for large files. Reads exceeding ~100K characters are rejected; use offset and limit to read specific sections of large files. NOTE: Cannot read images or binary files -use a vision tool if available for images.",
    "parameters": {
        "type": "object",
        "properties": {
...
registry.register(name="write_file", toolset="file", schema=WRITE_FILE_SCHEMA, handler=_handle_write_file, check_fn=_check_file_reqs, emoji="âï¸", max_result_size_chars=100_000)
