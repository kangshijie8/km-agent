...
# Resolution: config.yaml auxiliary.vision.download_timeout 鈫?env var 鈫?30s default.
def _resolve_download_timeout() -> float:
    env_val = os.getenv("KUNMING_VISION_DOWNLOAD_TIMEOUT", "").strip()
    if env_val:
        try:
...
async def vision_analyze_tool(
    ...
    try:
        ...
        # Extract the analysis 鈥?fall back to reasoning if content is empty
        ...
        # Detect vision capability errors 鈥?give the model a clear message
...
if __name__ == "__main__":
    ...
    if _debug.active:
        ...
        print("馃悰 Debug mode disabled (set VISION_TOOLS_DEBUG=true to enable)")
