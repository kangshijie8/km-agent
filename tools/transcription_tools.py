...
LOCAL_STT_COMMAND_ENV = "KUNMING_LOCAL_STT_COMMAND"
LOCAL_STT_LANGUAGE_ENV = "KUNMING_LOCAL_STT_LANGUAGE"
...
def get_stt_model_from_config() -> Optional[str]:
    """Read the STT model name from ~/.kunming/config.yaml.
    ...
    if explicit:
        if provider == "local":
            ...
            logger.warning(
                ...
                "(install faster-whisper or set KUNMING_LOCAL_STT_COMMAND)"
            )
            return "none"
...
def _transcribe_local_command(file_path: str, model_name: str) -> Dict[str, Any]:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="kunming-local-stt-") as output_dir:
