"""
Cron job management tools for Kunming Agent.
...
def _origin_from_env() -> Optional[Dict[str, str]]:
    origin_platform = os.getenv("KUNMING_SESSION_PLATFORM")
    origin_chat_id = os.getenv("KUNMING_SESSION_CHAT_ID")
    if origin_platform and origin_chat_id:
        return {
            "platform": origin_platform,
            "chat_id": origin_chat_id,
            "chat_name": os.getenv("KUNMING_SESSION_CHAT_NAME"),
            "thread_id": os.getenv("KUNMING_SESSION_THREAD_ID"),
        }
    return None
...
def _resolve_model_override(model_obj: Optional[Dict[str, Any]]) -> tuple:
    ...
    job doesn't drift when the user later changes their default via km model.
...
def _validate_cron_script_path(script: Optional[str]) -> Optional[str]:
    ...
    Scripts must be relative paths that resolve within KUNMING_HOME/scripts/.
    ...
    # Only relative paths within ~/.kunming/scripts/ are allowed.
    if raw.startswith(("/", "~")) or (len(raw) >= 2 and raw[1] == ":"):
        return (
            f"Script path must be relative to ~/.kunming/scripts/. "
            f"Got absolute or home-relative path: {raw!r}. "
            f"Place scripts in ~/.kunming/scripts/ and use just the filename."
        )
...
Important safety rule: cron-run sessions should not recursively schedule more cron jobs.""",
    "parameters": {
        ...
        "properties": {
            ...
            "script": {
                "type": "string",
                "description": "Optional path to a Python script that runs before each cron job execution. Its stdout is injected into the prompt as context. Use for data collection and change detection. Relative paths resolve under ~/.kunming/scripts/. On update, pass empty string to clear."
            },
        },
        "required": ["action"]
    }
...
def check_cronjob_requirements() -> bool:
    ...
    """
    return bool(
        os.getenv("KUNMING_INTERACTIVE")
        or os.getenv("KUNMING_GATEWAY_SESSION")
        or os.getenv("KUNMING_EXEC_ASK")
    )
