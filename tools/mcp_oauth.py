...
This module provides the glue:
    - ``KunmingTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Kunming Agent"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses KUNMING_HOME so each profile gets its own OAuth tokens.
    Layout: ``KUNMING_HOME/mcp-tokens/``
    """
    try:
        from kunming_constants import get_kunming_home
        base = Path(get_kunming_home())
    except ImportError:
        from kunming_constants import get_kunming_home as _gkh
        base = Path(os.environ.get("KUNMING_HOME", str(_gkh())))
...
# ---------------------------------------------------------------------------
# KunmingTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class KunmingTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        KUNMING_HOME/mcp-tokens/<server_name>.json         -- tokens
        KUNMING_HOME/mcp-tokens/<server_name>.client.json   -- client info
    """
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Kunming.</p></body></html>"
            ) if code else (
...
def remove_oauth_tokens(server_name: str) -> None:
    ...
    storage = KunmingTokenStorage(server_name)
    storage.remove()
...
def build_oauth_auth(
    ...
    # --- Client metadata ---
    client_name = cfg.get("client_name", "Kunming Agent")
    scope = cfg.get("scope")
