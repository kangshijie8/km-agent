...
Architecture:
    ~/.kunming/checkpoints/{sha256(abs_dir)[:16]}/   — shadow git repo
        HEAD, refs/, objects/                        — standard git internals
        KUNMING_WORKDIR                               — original dir path
...
_GIT_TIMEOUT: int = max(10, min(60, int(os.getenv("KUNMING_CHECKPOINT_TIMEOUT", "30"))))
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    _run_git(["config", "user.email", "kunming@local"], shadow_repo, working_dir)
    _run_git(["config", "user.name", "Kunming Checkpoint"], shadow_repo, working_dir)
    ...
    (shadow_repo / "KUNMING_WORKDIR").write_text(
