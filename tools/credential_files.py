...
def _resolve_kunming_home() -> Path:
...
def register_credential_file(
    relative_path: str,
    container_base: str = "/root/.kunming",
) -> bool:
    """Register a credential file for mounting into remote sandboxes.

    *relative_path* is relative to ``KUNMING_HOME`` (e.g. ``google_token.json``).
    ...
    The resolved host path must remain inside KUNMING_HOME so that a malicious
    skill cannot declare ``required_credential_files: ['../../.ssh/id_rsa']``
    and exfiltrate sensitive host files into a container sandbox.
    """
    kunming_home = _resolve_kunming_home()

    # Reject absolute paths -they bypass the KUNMING_HOME sandbox entirely.
    if os.path.isabs(relative_path):
        logger.warning(
            "credential_files: rejected absolute path %r (must be relative to KUNMING_HOME)",
            relative_path,
        )
        return False

    host_path = kunming_home / relative_path

    # Resolve symlinks and normalise ``..`` before the containment check so
    # that traversal like ``../. ssh/id_rsa`` cannot escape KUNMING_HOME.
    try:
        resolved = host_path.resolve()
        kunming_home_resolved = kunming_home.resolve()
        resolved.relative_to(kunming_home_resolved)  # raises ValueError if outside
    except ValueError:
        logger.warning(
            "credential_files: rejected path traversal %r "
            "(resolves to %s, outside KUNMING_HOME %s)",
            relative_path,
            resolved,
            kunming_home_resolved,
        )
        return False
...
def _load_config_files() -> List[Dict[str, str]]:
    ...
    try:
        from kunming_cli.config import read_raw_config
        kunming_home = _resolve_kunming_home()
        cfg = read_raw_config()
            ...
            for item in cred_files:
                if isinstance(item, str) and item.strip():
                    ...
                    if os.path.isabs(rel):
                        ...
                        continue
                    host_path = (kunming_home / rel).resolve()
                    try:
                        host_path.relative_to(kunming_home_resolved)
                    except ValueError:
                        logger.warning(
                            "credential_files: rejected config path traversal %r "
                            "(resolves to %s, outside KUNMING_HOME %s)",
                            rel, host_path, kunming_home_resolved,
                        )
                        continue
                    if host_path.is_file():
                        container_path = f"/root/.kunming/{rel}"
                        result.append({
...
def _safe_skills_path(skills_dir: Path) -> str:
    ...
    safe_dir = Path(tempfile.mkdtemp(prefix="kunming-skills-safe-"))
...
# Each tuple is (new_subpath, old_name) matching kunming_constants.get_kunming_dir().
...
def get_cache_directory_mounts(
    ...
    ``get_kunming_dir()`` for backward compatibility with old directory layouts.
    """
    from kunming_constants import get_kunming_dir

    mounts: List[Dict[str, str]] = []
    for new_subpath, old_name in _CACHE_DIRS:
        host_dir = get_kunming_dir(new_subpath, old_name)
        if host_dir.is_dir():
