#!/usr/bin/env python3
"""
Skills Hub -Source adapters and hub state management for the Kunming Skills Hub.
...
Used by kunming_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            kunming_meta = metadata.get("kunming", {})
            if isinstance(kunming_meta, dict):
                tags = kunming_meta.get("tags", [])
        if not tags:
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by Kunming) but not activated
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                kunming_meta = meta_block.get("kunming", {})
                if isinstance(kunming_meta, dict):
                    tags = kunming_meta.get("tags", [])
