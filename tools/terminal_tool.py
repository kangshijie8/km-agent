...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or Kunming exit
...
# display_kunming_home imported lazily at call site (stale-module safety during km update)
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of kunming directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "kunming-*")):
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = os.getenv("KUNMING_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (KUNMING_INTERACTIVE=1).
    ...
    try:
        os.environ["KUNMING_SPINNER_PAUSE"] = "1"
        ...
        if "KUNMING_SPINNER_PAUSE" in os.environ:
            del os.environ["KUNMING_SPINNER_PAUSE"]
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    If SUDO_PASSWORD is not set and in interactive mode (KUNMING_INTERACTIVE=1):
    ...
    if not sudo_password:
        ...
        if os.getenv("KUNMING_INTERACTIVE"):
...
TERMINAL_TOOL_DESCRIPTION = """Execute shell commands. Filesystem usually persists between calls.
...
def _parse_env_var(name: str, default: str, converter=int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.kunming/.env or environment variables."
        )
        ...
        if modal_state["selected_backend"] != "direct":
            if modal_state["managed_mode_blocked"]:
                raise ValueError(
                    ...
                    "KUNMING_ENABLE_NOUS_MANAGED_TOOLS is not enabled and no direct "
        ...
        if background:
            ...
            try:
                ...
                if notify_on_complete and background:
                    ...
                    _gw_platform = os.getenv("KUNMING_SESSION_PLATFORM", "")
                    if _gw_platform and not check_interval:
                        _gw_chat_id = os.getenv("KUNMING_SESSION_CHAT_ID", "")
                        _gw_thread_id = os.getenv("KUNMING_SESSION_THREAD_ID", "")
                ...
                if check_interval and background:
                    ...
                    watcher_platform = os.getenv("KUNMING_SESSION_PLATFORM", "")
                    watcher_chat_id = os.getenv("KUNMING_SESSION_CHAT_ID", "")
                    watcher_thread_id = os.getenv("KUNMING_SESSION_THREAD_ID", "")
...
registry.register(
    ...
    allow_override=True,  # Allow override for testing
)
