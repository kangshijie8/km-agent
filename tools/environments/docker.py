...
from tools.environments.local import _KUNMING_PROVIDER_ENV_BLOCKLIST
...
def _load_kunming_env_vars() -> dict[str, str]:
    """Load ~/.kunming/.env values without failing Docker command execution."""
    try:
        from kunming_cli.config import load_env
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.kunming/sandboxes/). Non-persistent
        ...
        container_name = f"kunming-{uuid.uuid4().hex[:8]}"
        run_cmd = [
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic Kunming secret blocklist. Only implicit passthrough
        # keys are filtered.
        forward_keys = explicit_forward_keys | (passthrough_keys - _KUNMING_PROVIDER_ENV_BLOCKLIST)
        kunming_env = _load_kunming_env_vars() if forward_keys else {}
        for key in sorted(forward_keys):
            value = os.getenv(key)
            if value is None:
                value = kunming_env.get(key)
