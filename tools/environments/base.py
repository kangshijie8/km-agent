"""Base class for all Kunming execution environment backends.
...
def get_sandbox_dir() -> Path:
    ...
    Configurable via TERMINAL_SANDBOX_DIR. Defaults to {KUNMING_HOME}/sandboxes/.
    """
    custom = os.getenv("TERMINAL_SANDBOX_DIR")
    if custom:
        p = Path(custom)
    else:
        p = get_kunming_home() / "sandboxes"
...
def _cwd_marker(session_id: str) -> str:
    return f"__KUNMING_CWD_{session_id}__"
...
class BaseEnvironment(ABC):
    """Common interface and unified execution flow for all Kunming backends.
    ...
    def __init__(self, cwd: str, timeout: int, env: dict = None):
        ...
        tmp_dir = tempfile.gettempdir()
        self._snapshot_path = os.path.join(tmp_dir, f"kunming-snap-{self._session_id}.sh")
        self._cwd_file = os.path.join(tmp_dir, f"kunming-cwd-{self._session_id}.txt")
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        parts.append("exit $__kunming_ec")
    ...
    def _embed_stdin_heredoc(command: str, stdin_data: str) -> str:
        ...
        delimiter = f"KUNMING_STDIN_{uuid.uuid4().hex[:12]}"
    ...
    def _extract_cwd_from_output(self, result: dict):
        """Parse the __KUNMING_CWD_{session}__ marker from stdout output.
