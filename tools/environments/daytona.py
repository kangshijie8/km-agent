...
class DaytonaEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        labels = {"kunming_task_id": task_id}
        sandbox_name = f"kunming-{task_id}"
    ...
    def _sync_files(self) -> None:
        container_base = f"{self._remote_home}/.kunming"
        try:
            from tools.credential_files import get_credential_file_mounts, iter_skills_files
            for mount_entry in get_credential_file_mounts():
                remote_path = mount_entry["container_path"].replace("/root/.kunming", container_base, 1)
