...
Backend is selected during ``km tools`` setup (web.backend in config.yaml).
When available, Kunming can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.kunming/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``km tools``).
...
def _raise_web_backend_configuration_error() -> None:
    ...
    if managed_nous_tools_enabled():
        message += (
            ...
            "(`km model`) and provide FIRECRAWL_GATEWAY_URL or TOOL_GATEWAY_DOMAIN."
        )
...
def _get_firecrawl_client():
    ...
    Kunming falls back to the Firecrawl tool-gateway for logged-in Nous Subscribers.
    """
...
def _is_nous_auxiliary_client(client: Any) -> bool:
    ...
    return host == "kunming.dev" or host.endswith(".kunming.dev")
...
def _resolve_web_extract_auxiliary(model: Optional[str] = None) -> tuple[Optional[Any], Optional[str], Dict[str, Any]]:
    ...
    if client is not None and _is_nous_auxiliary_client(client):
        ...
        extra_body = get_auxiliary_extra_body() or {"tags": ["product=kunming-agent"]}
...
def _get_exa_client():
    ...
    if _exa_client is None:
        ...
        _exa_client.headers["x-exa-integration"] = "kunming-agent"
    return _exa_client
...
async def web_extract_tool(
    ...
    try:
        ...
        results = trimmed_response.get("results", [])
        has_content = any(r.get("content") for r in results if isinstance(r, dict))
        if not results or not has_content:
...
if __name__ == "__main__":
    ...
    print("\n[CONFIG] Run 'python test_web_tools_llm.py' to test LLM processing capabilities")
