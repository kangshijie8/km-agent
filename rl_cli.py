...
if tinker_atropos_dir.exists():
    ...
    os.environ['KUNMING_QUIET'] = '1'  # Disable temp subdirectory creation
    print(f"ð Terminal working directory: {tinker_atropos_dir}")
else:
    # Fall back to kunming-agent directory if submodule not found
...
from kunming_constants import get_kunming_home, OPENROUTER_BASE_URL

DEFAULT_MODEL = "anthropic/claude-opus-4.5"
DEFAULT_BASE_URL = OPENROUTER_BASE_URL


def load_kunming_config() -> dict:
    """
...
def main(
    ...
    Args:
        ...
        model: Model to use for the agent (reads from ~/.kunming/config.yaml if not provided)
    ...
    """
    # Load config from ~/.kunming/config.yaml
    config = load_kunming_config()
    ...
    if check_server:
        ...
        if ok:
            ...
            if missing:
                ...
                print("   Add them to ~/.kunming/.env")
            else:
                print("[OK]API keys configured")
        else:
            print(f"[X]tinker-atropos not set up: {result}")
            print("\nTo set up:")
    ...
    if list_environments:
        ...
        try:
            ...
            print(f"[X]Error listing environments: {e}")
            print("\nMake sure tinker-atropos is set up:")
            print("  git submodule update --init")
            print("  pip install -e ./tinker-atropos")
        return
    ...
    if not api_key:
        print("[X]No API key provided. Set OPENROUTER_API_KEY or pass --api-key")
        sys.exit(1)
