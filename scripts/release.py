...
VERSION_FILE = REPO_ROOT / "kunming_cli" / "__init__.py"
...
# Only core team members mapped
AUTHOR_MAP = {
    # Core team
    "kunming@km.dev": "kangshijie8",
}
...
def build_release_artifacts(semver: str) -> list[Path]:
    ...
    if result.returncode != 0:
        print("  [ON]Could not build Python release artifacts.")
    ...
    if not matching:
        print("  [ON]Built artifacts did not match the expected release version.")
        return []
    return matching
...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/kangshijie8/KM-Agent",
    ...
    lines.append(f"# Kunming Agent v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for Kunming Agent. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Kunming Agent Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  Kunming Agent Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        if args.bump:
            ...
            if add_result.returncode != 0:
                print(f"  [OK]Failed to stage version files: {add_result.stderr.strip()}")
                return
            ...
            if commit_result.returncode != 0:
                print(f"  [OK]Failed to commit version bump: {commit_result.stderr.strip()}")
                return
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"Kunming Agent v{new_version} ({calver_date})\n\nWeekly release"
        )
        if tag_result.returncode != 0:
            print(f"  [OK]Failed to create tag {tag_name}: {tag_result.stderr.strip()}")
            return
        ...
        if push_result.returncode == 0:
            ...
            print(f"  [OK]Failed to push to origin: {push_result.stderr.strip()}")
        ...
        gh_cmd = [
            ...
            "--title", f"Kunming Agent v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            if result is None:
                print("  [OK]GitHub release skipped: `gh` CLI not found.")
            else:
                print(f"  [OK]GitHub release failed: {result.stderr.strip()}")
            print(f"    Release notes kept at: {changelog_file}")
            print(f"    Tag was created locally. Create the release manually:")
            print(
                f"    gh release create {tag_name} --title 'Kunming Agent v{new_version} ({calver_date})' "
