 ...
 *
 * Usage:
 *   node bridge.js --port 3000 --session ~/.kunming/whatsapp/session
 */
...
const KUNMING_HOME = process.env.KUNMING_HOME || path.join(process.env.HOME || '~', '.kunming');
const PORT = parseInt(getArg('port', '3000'), 10);
const SESSION_DIR = getArg('session', path.join(KUNMING_HOME, 'whatsapp', 'session'));
const IMAGE_CACHE_DIR = path.join(KUNMING_HOME, 'image_cache');
const DOCUMENT_CACHE_DIR = path.join(KUNMING_HOME, 'document_cache');
const AUDIO_CACHE_DIR = path.join(KUNMING_HOME, 'audio_cache');
const PAIR_ONLY = args.includes('--pair-only');
const WHATSAPP_MODE = getArg('mode', process.env.WHATSAPP_MODE || 'self-chat'); // "bot" or "self-chat"
const ALLOWED_USERS = parseAllowedUsers(process.env.WHATSAPP_ALLOWED_USERS || '');
const DEFAULT_REPLY_PREFIX = '⚕ *Kunming Agent*\n────────────\n';
...
async function startSocket() {
  ...
  sock = makeWASocket({
    ...
    browser: ['Kunming Agent', 'Chrome', '120.0'],
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      // Ignore Kunming' own reply messages in self-chat mode to avoid loops.
