...
def load_dataset_from_hf(dataset_name: str) -> List[Dict[str, Any]]:
    ...
    Args:
        dataset_name: HuggingFace dataset name (e.g., "km/km-model")
...
def sample_from_datasets(
    ...
    for dataset_name in datasets:
        ...
        if not entries:
            print(f"   [!]  Skipping {dataset_name} (no entries loaded)")
            continue
    ...
    if len(filtered_entries) <= total_samples:
        print(f"\n[!]  Only {len(filtered_entries):,} trajectories available, using all of them")
...
def main(
    ...
    print("✅ COMPLETE!")
    print("=" * 70)
    print(f"\n[FILES] Raw samples:        {sampled_dir}")
    print(f"[FILES] Compressed batches: {compressed_dir}")
    print(f"[FILES] Final output:       {final_output}")
    print(f"\nTo upload to HuggingFace:")
    print(f"   huggingface-cli upload km/km-model {final_output}")
