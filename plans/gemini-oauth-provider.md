...
## Client ID
- Need to register a "Desktop app" OAuth client on a GCP project
...
## Token Lifecycle
- Store at `~/.kunming/gemini_oauth.json` (NOT sharing with `~/.gemini/oauth_creds.json`)
...
2. `kunming_cli/auth.py` �?Add ProviderConfig for "gemini" with auth_type="oauth_google"
3. `kunming_cli/models.py` �?Add Gemini model catalog
4. `kunming_cli/runtime_provider.py` �?Add gemini branch (read OAuth token, build OpenAI client)
5. `kunming_cli/main.py` �?Add `_model_flow_gemini()`, add to provider choices
6. `kunming_cli/setup.py` �?Add gemini auth flow (trigger browser OAuth)
...
## Prerequisites
- GCP project with Desktop OAuth client registered
- OR: accept user-provided client_id via KUNMING_GEMINI_CLIENT_ID env var
...
- kunming-agent Copilot OAuth: `kunming_cli/main.py` `_copilot_device_flow()` (different flow type but same lifecycle pattern)
