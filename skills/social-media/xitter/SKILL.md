...
version: 1.0.0
author: Siddharth Balyan + Kunming Agent
license: MIT
...
This skill intentionally does not vendor a separate CLI implementation into Kunming. Install and use upstream `x-cli` instead.
...
If you already keep your X credentials in `~/.kunming/.env`, the cleanest setup is:

```bash
mkdir -p ~/.config/x-cli
ln -sf ~/.kunming/.env ~/.config/x-cli/.env
```
...
x-cli tweet search "from:kangshijie8" --max 3
...
- **Credential drift**: if you rotate tokens in `~/.kunming/.env`, make sure `~/.config/x-cli/.env` still points at the current file.
