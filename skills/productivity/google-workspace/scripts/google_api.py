#!/usr/bin/env python3
"""Google Workspace API CLI for Kunming Agent.
...
def get_credentials():
    ...
    if missing_scopes:
        ...
        print(
            f"Re-run setup.py from the active Kunming profile ({display_kunming_home()}) to restore full access.",
            file=sys.stderr,
        )
        sys.exit(1)
    return creds
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Kunming Agent")
