#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Kunming Agent.
...
try:
    ...
    KUNMING_AGENT_ROOT = Path(__file__).resolve().parents[4]
    if KUNMING_AGENT_ROOT.exists():
        sys.path.insert(0, str(KUNMING_AGENT_ROOT))
    from kunming_constants import display_kunming_home, get_kunming_home

KUNMING_HOME = get_kunming_home()
TOKEN_PATH = KUNMING_HOME / "google_token.json"
CLIENT_SECRET_PATH = KUNMING_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = KUNMING_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Kunming profile to refresh consent."
    )
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Kunming home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_kunming_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Kunming")
