...
description: Gmail, Calendar, Drive, Contacts, Sheets, and Docs integration via Python. Uses OAuth2 with automatic token refresh. No external binaries needed — runs entirely with Google's Python client libraries in the Kunming venv.
version: 1.0.0
...
```bash
KUNMING_HOME="${KUNMING_HOME:-$HOME/.kunming}"
GWORKSPACE_SKILL_DIR="$KUNMING_HOME/skills/productivity/google-workspace"
PYTHON_BIN="${KUNMING_PYTHON:-python3}"
if [ -x "$KUNMING_HOME/kunming-agent/venv/bin/python" ]; then
  PYTHON_BIN="$KUNMING_HOME/kunming-agent/venv/bin/python"
fi
...
- Token is stored at `google_token.json` under the active profile's `KUNMING_HOME` and auto-refreshes.
- Pending OAuth session state/verifier are stored temporarily at `google_oauth_pending.json` under the active profile's `KUNMING_HOME` until exchange completes.
- Kunming now refuses to overwrite a full Google Workspace token with a narrower re-auth token missing Gmail scopes, so one profile's partial consent cannot silently break email actions later.
