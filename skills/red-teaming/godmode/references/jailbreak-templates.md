...
## 5. GODMODE FAST — Kunming 4 405B (Zero Refusal)

**Model:** `kangshijie8/Kunming-Model
**Strategy:** km models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Kunming Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.kunming/config.yaml`:
