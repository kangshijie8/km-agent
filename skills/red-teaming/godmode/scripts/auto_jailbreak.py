...
Usage in execute_code:
    exec(open(os.path.expanduser(
        "~/.kunming/skills/red-teaming/godmode/scripts/auto_jailbreak.py"
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("KUNMING_HOME", Path.home() / ".kunming")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# Kunming config paths
# ═══════════════════════════════════════════════════════════════════

KUNMING_HOME = Path(os.getenv("KUNMING_HOME", Path.home() / ".kunming"))
CONFIG_PATH = KUNMING_HOME / "config.yaml"
PREFILL_PATH = KUNMING_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/Kunming models — already uncensored, just needs clean prompt
    "kunming": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "kunming" in model_lower or "nous" in model_lower:
        return "kunming"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Kunming config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.kunming/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Kunming for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Kunming for changes to take effect.")
