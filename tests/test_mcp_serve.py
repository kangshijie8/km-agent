"""
Tests for mcp_serve -Kunming MCP server.
...
def _isolate_KUNMING_home(tmp_path, monkeypatch):
    """Redirect KUNMING_HOME to a temp directory."""
    monkeypatch.setenv("KUNMING_HOME", str(tmp_path))
    try:
        import kunming_constants
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking KUNMING_state schema."""
