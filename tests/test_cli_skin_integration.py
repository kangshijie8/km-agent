...
from cli import KunmingCLI, _build_compact_banner, _rich_text_from_ansi
from kunming_cli.skin_engine import get_active_skin, set_active_skin


def _make_cli_stub():
    cli = KunmingCLI.__new__(KunmingCLI)
    ...
    cli._command_spinner_frame = lambda: "â"
    cli._tui_style_base = {
        "prompt": "#fff",
        "input-area": "#fff",
        "input-rule": "#aaa",
...
class TestCliSkinPromptIntegration:
    def test_default_prompt_fragments_use_default_symbol(self):
        ...
        assert cli._get_tui_prompt_fragments() == [("class:prompt", "[X]")]

    def test_ares_prompt_fragments_use_skin_symbol(self):
        cli = _make_cli_stub()

        set_active_skin("ares")
        assert cli._get_tui_prompt_fragments() == [("class:prompt", "[ON][X]")]
    ...
    def test_secret_prompt_fragments_preserve_secret_state(self):
        ...
        set_active_skin("ares")
        assert cli._get_tui_prompt_fragments() == [("class:sudo-prompt", "ð [X]")]

    def test_icon_only_skin_symbol_still_visible_in_special_states(self):
        cli = _make_cli_stub()
        cli._secret_state = {"response_queue": object()}

        with patch("kunming_cli.skin_engine.get_active_prompt_symbol", return_value="[ON]"):
            assert cli._get_tui_prompt_fragments() == [("class:sudo-prompt", "ð [ON]")]
...
class TestCompactBannerSkinIntegration:
    def test_default_compact_banner_keeps_legacy_nous_kunming_branding(self):
        set_active_skin("default")

        with patch("cli.shutil.get_terminal_size", return_value=SimpleNamespace(columns=90)), \
             patch("cli.format_banner_version_label", return_value="Kunming Agent v0.1.0 (test)"):
            banner = _build_compact_banner()

        assert "KUNMING" in banner

    def test_poseidon_compact_banner_uses_skin_branding_instead_of_kunming(self):
    ...
    def test_compact_banner_shows_version_label(self):
        ...
        with patch("cli.shutil.get_terminal_size", return_value=SimpleNamespace(columns=90)), \
             patch("cli.format_banner_version_label", return_value="Kunming Agent v1.0 (test) Â· upstream abc12345"):
            banner = _build_compact_banner()
