...
def _write_auth_store(tmp_path, payload: dict) -> None:
    ...
    (KUNMING_home / "auth.json").write_text(json.dumps(payload, indent=2))
...
def test_auth_add_nous_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "kunming_cli.auth._nous_device_code_login",
        lambda **kwargs: {
            "portal_base_url": "https://portal.example.com",
            "inference_base_url": "https://inference.example.com/v1",
            "client_id": "kunming-cli",
...
def test_auth_add_codex_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "kunming_cli.auth._codex_device_code_login",
        lambda: {
            "tokens": {
                "access_token": token,
                "refresh_token": "refresh-token",
...
def test_auth_remove_reindexes_priorities(tmp_path, monkeypatch):
    ...
    from kunming_cli.auth_commands import auth_remove_command
...
def test_auth_reset_clears_provider_statuses(tmp_path, monkeypatch, capsys):
    ...
    from kunming_cli.auth_commands import auth_reset_command
...
def test_clear_provider_auth_removes_provider_pool_entries(tmp_path, monkeypatch):
    ...
    from kunming_cli.auth import clear_provider_auth
...
def test_auth_list_does_not_call_mutating_select(monkeypatch, capsys):
    from kunming_cli.auth_commands import auth_list_command
    ...
    monkeypatch.setattr(
        "kunming_cli.auth_commands.load_pool",
...
def test_auth_list_shows_exhausted_cooldown(monkeypatch, capsys):
    ...
    monkeypatch.setattr("kunming_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr("kunming_cli.auth_commands.time.time", lambda: 1030.0)
...
def test_auth_list_prefers_explicit_reset_time(monkeypatch, capsys):
    ...
    monkeypatch.setattr("kunming_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr(
        "kunming_cli.auth_commands.time.time",
