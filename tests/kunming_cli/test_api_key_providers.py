...
PROVIDER_ENV_VARS = (
    ...
    "OPENAI_BASE_URL", "KUNMING_COPILOT_ACP_COMMAND", "COPILOT_CLI_PATH",
    "KUNMING_COPILOT_ACP_ARGS", "COPILOT_ACP_BASE_URL",
)
...
def _clear_provider_env(monkeypatch):
    ...
    monkeypatch.setattr("kunming_cli.auth._load_auth_store", lambda: {})
...
class TestApiKeyProviderStatus:
    ...
    def test_copilot_status_uses_gh_cli_token(self, monkeypatch):
        monkeypatch.setattr("kunming_cli.copilot_auth._try_gh_cli_token", lambda: "gho_gh_cli_token")
    ...
    def test_copilot_acp_status_detects_local_cli(self, monkeypatch):
        monkeypatch.setenv("KUNMING_COPILOT_ACP_ARGS", "--acp --stdio --debug")
        monkeypatch.setattr("kunming_cli.auth.shutil.which", lambda command: f"/usr/local/bin/{command}")
    ...
    def test_get_auth_status_dispatches_to_external_process(self, monkeypatch):
        monkeypatch.setattr("kunming_cli.auth.shutil.which", lambda command: f"/opt/bin/{command}")
...
class TestResolveApiKeyProviderCredentials:
    ...
    def test_resolve_zai_with_key(self, monkeypatch):
        ...
        monkeypatch.setattr("kunming_cli.auth.detect_zai_endpoint", lambda *a, **kw: None)
    ...
    def test_resolve_copilot_with_gh_cli_fallback(self, monkeypatch):
        monkeypatch.setattr("kunming_cli.copilot_auth._try_gh_cli_token", lambda: "gho_cli_secret")
        creds = resolve_api_key_provider_credentials("copilot")
        assert creds["provider"] == "copilot"
    ...
    def test_try_gh_cli_token_uses_homebrew_path_when_not_on_path(self, monkeypatch):
        monkeypatch.setattr("kunming_cli.auth.shutil.which", lambda command: None)
        monkeypatch.setattr(
            "kunming_cli.auth.os.path.isfile",
            lambda path: path == "/opt/homebrew/bin/gh",
        )
        monkeypatch.setattr(
            "kunming_cli.auth.os.access",
        ...
        monkeypatch.setattr("kunming_cli.auth.subprocess.run", _fake_run)
    ...
    def test_resolve_copilot_acp_with_local_cli(self, monkeypatch):
        monkeypatch.setenv("KUNMING_COPILOT_ACP_ARGS", "--acp --stdio")
        monkeypatch.setattr("kunming_cli.auth.shutil.which", lambda command: f"/usr/local/bin/{command}")
...
class TestRuntimeProviderResolution:
    ...
    def test_runtime_zai(self, monkeypatch):
        monkeypatch.setenv("GLM_API_KEY", "glm-key")
        from kunming_cli.runtime_provider import resolve_runtime_provider
    ...
    def test_runtime_copilot_uses_responses_for_gpt_5_4(self, monkeypatch):
        monkeypatch.setattr("kunming_cli.copilot_auth._try_gh_cli_token", lambda: "gho_cli_secret")
        monkeypatch.setattr(
            "kunming_cli.runtime_provider._get_model_config",
            lambda: {"provider": "copilot", "default": "gpt-5.4"},
        )
        monkeypatch.setattr(
            "kunming_cli.models.fetch_github_model_catalog",
...
class TestHasAnyProviderConfigured:
    ...
    def test_glm_key_counts(self, monkeypatch, tmp_path):
        from kunming_cli import config as config_module
        monkeypatch.setenv("GLM_API_KEY", "test-key")
        KUNMING_home = tmp_path / ".kunming"
        KUNMING_home.mkdir()
        monkeypatch.setattr(config_module, "get_env_path", lambda: KUNMING_home / ".env")
        monkeypatch.setattr(config_module, "get_kunming_home", lambda: KUNMING_home)
        from kunming_cli.main import _has_any_provider_configured
    ...
    def test_claude_code_creds_ignored_on_fresh_install(self, monkeypatch, tmp_path):
        """Claude Code credentials should NOT skip the wizard when Kunming is unconfigured."""
        from kunming_cli import config as config_module
        KUNMING_home = tmp_path / ".kunming"
        KUNMING_home.mkdir()
        monkeypatch.setattr(config_module, "get_env_path", lambda: KUNMING_home / ".env")
    ...
    def test_config_provider_counts(self, monkeypatch, tmp_path):
        ...
        KUNMING_home = tmp_path / ".kunming"
        KUNMING_home.mkdir()
        config_file = KUNMING_home / "config.yaml"
    ...
    def test_claude_code_creds_counted_when_KUNMING_configured(self, monkeypatch, tmp_path):
        """Claude Code credentials should count when Kunming has been explicitly configured."""
        import yaml
        from kunming_cli import config as config_module
        KUNMING_home = tmp_path / ".kunming"
        KUNMING_home.mkdir()
...
class TestZaiEndpointAutoDetect:
    ...
    def test_probe_success_returns_detected_url(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "kunming_cli.auth.detect_zai_endpoint",
            lambda *a, **kw: {
                "id": "coding-global",
    ...
    def test_env_override_skips_probe(self, monkeypatch):
        ...
        monkeypatch.setattr("kunming_cli.auth.detect_zai_endpoint", _never_called)
        creds = resolve_api_key_provider_credentials("zai")
...
class TestKimiMoonshotModelListIsolation:
    ...
    def test_moonshot_list_excludes_coding_plan_only_models(self):
        from kunming_cli.main import _PROVIDER_MODELS
...
class TestHuggingFaceModels:
    ...
    def test_model_lists_match(self):
        ...
        from kunming_cli.main import _PROVIDER_MODELS as main_models
        from kunming_cli.models import _PROVIDER_MODELS as models_models
