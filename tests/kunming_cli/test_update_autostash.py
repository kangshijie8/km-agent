...
from kunming_cli import config as kunming_config
...
def test_stash_local_changes_if_needed_returns_none_when_tree_clean(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(kunming_main.subprocess, "run", fake_run)

    stash_ref = kunming_main._stash_local_changes_if_needed(["git"], tmp_path)
...
def test_stash_local_changes_if_needed_returns_specific_stash_commit(monkeypatch, tmp_path):
    ...
    def fake_run(cmd, **kwargs):
        calls.append((cmd, kwargs))
        if cmd[-2:] == ["status", "--porcelain"]:
            return SimpleNamespace(stdout=" M kunming_cli/main.py\n?? notes.txt\n", returncode=0)
...
def test_resolve_stash_selector_returns_matching_entry(monkeypatch, tmp_path):
    ...
    assert kunming_main._resolve_stash_selector(["git"], tmp_path, "abc123") == "stash@{1}"
...
def test_restore_stashed_changes_prompts_before_applying(monkeypatch, tmp_path, capsys):
    ...
    restored = kunming_main._restore_stashed_changes(["git"], tmp_path, "abc123", prompt_user=True)
...
def test_restore_stashed_changes_applies_without_prompt_when_disabled(monkeypatch, tmp_path, capsys):
    ...
    restored = kunming_main._restore_stashed_changes(["git"], tmp_path, "abc123", prompt_user=False)
...
def test_print_stash_cleanup_guidance_with_selector(capsys):
    kunming_main._print_stash_cleanup_guidance("abc123", "stash@{2}")
...
def test_restore_stashed_changes_prompts_before_reset_on_conflict(monkeypatch, tmp_path, capsys):
    ...
    def fake_run(cmd, **kwargs):
        ...
        if cmd[1:3] == ["diff", "--name-only"]:
            return SimpleNamespace(stdout="kunming_cli/main.py\n", stderr="", returncode=0)
    ...
    with pytest.raises(SystemExit, match="1"):
        kunming_main._restore_stashed_changes(["git"], tmp_path, "abc123", prompt_user=True)

    out = capsys.readouterr().out
    assert "Conflicted files:" in out
    assert "kunming_cli/main.py" in out
...
def test_restore_stashed_changes_auto_resets_non_interactive(monkeypatch, tmp_path, capsys):
    ...
    result = kunming_main._restore_stashed_changes(["git"], tmp_path, "abc123", prompt_user=False)
...
def test_stash_local_changes_if_needed_raises_when_stash_ref_missing(monkeypatch, tmp_path):
    ...
    with pytest.raises(CalledProcessError):
        kunming_main._stash_local_changes_if_needed(["git"], Path(tmp_path))
...
def _setup_update_mocks(monkeypatch, tmp_path):
    ...
    (tmp_path / ".git").mkdir()
    monkeypatch.setattr(kunming_main, "PROJECT_ROOT", tmp_path)
    monkeypatch.setattr(kunming_main, "_stash_local_changes_if_needed", lambda *a, **kw: None)
    monkeypatch.setattr(kunming_main, "_restore_stashed_changes", lambda *a, **kw: True)
    monkeypatch.setattr(kunming_config, "get_missing_env_vars", lambda required_only=True: [])
    monkeypatch.setattr(kunming_config, "get_missing_config_fields", lambda: [])
    monkeypatch.setattr(kunming_config, "check_config_version", lambda: (5, 5))
    monkeypatch.setattr(kunming_config, "migrate_config", lambda **kw: {"env_added": [], "config_added": []})
...
def test_cmd_update_retries_optional_extras_individually_when_all_fails(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(kunming_main, "_load_installable_optional_extras", lambda: ["matrix", "mcp"])
    ...
    kunming_main.cmd_update(SimpleNamespace())
...
def test_cmd_update_falls_back_to_reset_when_ff_only_fails(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(kunming_main.subprocess, "run", side_effect)
...
def test_cmd_update_restores_stash_and_branch_when_already_up_to_date(monkeypatch, tmp_path, capsys):
    ...
    monkeypatch.setattr(
        kunming_main, "_stash_local_changes_if_needed",
        lambda *a, **kw: "abc123deadbeef",
    )
    restore_calls = []
    monkeypatch.setattr(
        kunming_main, "_restore_stashed_changes",
