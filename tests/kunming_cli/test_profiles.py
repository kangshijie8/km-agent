"""Comprehensive tests for kunming_cli.profiles module.
...
from kunming_cli.profiles import (
    ...
    _get_profiles_root,
    _get_default_kunming_home,
)


# ---------------------------------------------------------------------------
# Shared fixture: redirect Path.home() and KUNMING_HOME for profile tests
# ---------------------------------------------------------------------------
...
def profile_env(tmp_path, monkeypatch):
    ...
    * Path.home() -> tmp_path  (so _get_profiles_root() = tmp_path/.kunming/profiles)
    * KUNMING_HOME  -> tmp_path/.kunming  (so get_kunming_home() agrees)
    * Creates the bare-minimum ~/.kunming directory.
    """
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    default_home = tmp_path / ".kunming"
    default_home.mkdir(exist_ok=True)
    monkeypatch.setenv("KUNMING_HOME", str(default_home))
    return tmp_path
...
class TestGetProfileDir:
    ...
    def test_default_returns_KUNMING_home(self, profile_env):
        tmp_path = profile_env
        result = get_profile_dir("default")
        assert result == tmp_path / ".kunming"

    def test_named_profile_returns_profiles_subdir(self, profile_env):
        tmp_path = profile_env
        result = get_profile_dir("coder")
        assert result == tmp_path / ".kunming" / "profiles" / "coder"
...
class TestDeleteProfile:
    ...
    def test_removes_directory(self, profile_env):
        ...
        with patch("kunming_cli.profiles._cleanup_gateway_service"):
...
class TestActiveProfile:
    ...
    def test_empty_file_returns_default(self, profile_env):
        tmp_path = profile_env
        active_path = tmp_path / ".kunming" / "active_profile"
...
class TestGetActiveProfileName:
    ...
    def test_default_KUNMING_home_returns_default(self, profile_env):
        # KUNMING_HOME points to tmp_path/.kunming which is the default
        assert get_active_profile_name() == "default"

    def test_profile_path_returns_profile_name(self, profile_env, monkeypatch):
        tmp_path = profile_env
        create_profile("coder", no_alias=True)
        profile_dir = tmp_path / ".kunming" / "profiles" / "coder"
        monkeypatch.setenv("KUNMING_HOME", str(profile_dir))
    ...
    def test_custom_path_returns_custom(self, profile_env, monkeypatch):
        ...
        monkeypatch.setenv("KUNMING_HOME", str(custom))
...
class TestAliasCollision:
    ...
    def test_reserved_name_returns_message(self, profile_env):
        result = check_alias_collision("kunming")
...
class TestRenameProfile:
    ...
    def test_renames_directory(self, profile_env):
        ...
        old_dir = tmp_path / ".kunming" / "profiles" / "oldname"
        assert old_dir.is_dir()

        # Mock alias collision to avoid subprocess calls
        with patch("kunming_cli.profiles.check_alias_collision", return_value="skip"):
            new_dir = rename_profile("oldname", "newname")

        assert not old_dir.is_dir()
        assert new_dir.is_dir()
        assert new_dir == tmp_path / ".kunming" / "profiles" / "newname"
...
class TestExportImport:
    ...
    def test_export_default_excludes_infrastructure(self, profile_env, tmp_path):
        ...
        for d in ("kunming-agent", ".worktrees", "profiles", "bin",
        ...
        for f in ("state.db", "gateway.pid", "gateway_state.json",
                  "processes.json", "errors.log", ".kunming_history",
        ...
        excluded_prefixes = [
            "default/kunming-agent", "default/.worktrees", "default/profiles",
        ...
        excluded_files = [
            ...
            "default/errors.log", "default/.kunming_history",
...
class TestCompletion:
    ...
    def test_bash_completion_has_KUNMING_profiles_function(self):
        script = generate_bash_completion()
        assert "_KUNMING_profiles" in script

    def test_zsh_completion_has_KUNMING_function(self):
        script = generate_zsh_completion()
        assert "_kunming" in script


# ===================================================================
# TestGetProfilesRoot / TestGetDefaultKunmingHome (internal helpers)
# ===================================================================

class TestInternalHelpers:
    """Tests for _get_profiles_root() and _get_default_kunming_home()."""

    def test_profiles_root_under_home(self, profile_env):
        tmp_path = profile_env
        root = _get_profiles_root()
        assert root == tmp_path / ".kunming" / "profiles"

    def test_default_kunming_home(self, profile_env):
        tmp_path = profile_env
        home = _get_default_kunming_home()
        assert home == tmp_path / ".kunming"
...
class TestEdgeCases:
    ...
    def test_create_profile_returns_correct_path(self, profile_env):
        ...
        expected = tmp_path / ".kunming" / "profiles" / "mybot"
    ...
    def test_gateway_running_check_with_pid_file(self, profile_env):
        ...
        from kunming_cli.profiles import _check_gateway_running
        tmp_path = profile_env
        default_home = tmp_path / ".kunming"
