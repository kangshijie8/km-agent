"""
Tests for --yes / --force flag separation in `kunming skills install`.

--yes / -y  鈫?skip_confirm (bypass interactive prompt, needed in TUI mode)
--force     鈫?force (install despite blocked scan verdict)
...
def test_cli_skills_install_yes_sets_skip_confirm(monkeypatch):
    ...
    from kunming_cli.main import main
    ...
    monkeypatch.setattr("kunming_cli.skills_hub.skills_command", fake_skills_command)
    monkeypatch.setattr(
        sys,
        "argv",
        ["kunming", "skills", "install", "official/email/agentmail", "--yes"],
    )
...
def test_cli_skills_install_y_alias(monkeypatch):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["kunming", "skills", "install", "test/skill", "-y"],
    )
...
def test_cli_skills_install_force_sets_force(monkeypatch):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["kunming", "skills", "install", "test/skill", "--force"],
    )
...
def test_cli_skills_install_force_and_yes_together(monkeypatch):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["kunming", "skills", "install", "test/skill", "--force", "--yes"],
    )
...
def test_cli_skills_install_no_flags(monkeypatch):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["kunming", "skills", "install", "test/skill"],
    )
