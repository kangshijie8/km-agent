"""Tests for kunming_cli.plugins_cmd -the ``kunming plugins`` CLI subcommand."""
...
from kunming_cli.plugins_cmd import (
    _copy_example_files,
    _read_manifest,
    _repo_name_from_url,
    _resolve_git_url,
...
class TestPluginsCommandDispatch:
    ...
    @patch("kunming_cli.plugins_cmd.cmd_remove")
    ...
    @patch("kunming_cli.plugins_cmd.cmd_update")
...
class TestReadManifest:
    ...
    def test_invalid_yaml_returns_empty_and_logs(self, tmp_path, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="kunming_cli.plugins_cmd"):
            result = _read_manifest(tmp_path)
        assert result == {}
...
class TestCmdInstall:
    ...
    def test_install_requires_identifier(self):
        from kunming_cli.plugins_cmd import cmd_install
        import argparse

        with pytest.raises(SystemExit):
            cmd_install("")

    @patch("kunming_cli.plugins_cmd._resolve_git_url")
    ...
    @patch("kunming_cli.plugins_cmd._display_after_install")
    @patch("kunming_cli.plugins_cmd.shutil.move")
    @patch("kunming_cli.plugins_cmd.shutil.rmtree")
    @patch("kunming_cli.plugins_cmd._plugins_dir")
    @patch("kunming_cli.plugins_cmd._read_manifest")
    @patch("kunming_cli.plugins_cmd.subprocess.run")
...
class TestCmdUpdate:
    ...
    @patch("kunming_cli.plugins_cmd._sanitize_plugin_name")
    @patch("kunming_cli.plugins_cmd._plugins_dir")
    @patch("kunming_cli.plugins_cmd.subprocess.run")
    def test_update_git_pull_success(self, mock_run, mock_plugins_dir, mock_sanitize):
        from kunming_cli.plugins_cmd import cmd_update
...
class TestCmdRemove:
    ...
    def test_remove_deletes_plugin(self, mock_rmtree, mock_plugins_dir, mock_sanitize):
        from kunming_cli.plugins_cmd import cmd_remove
...
class TestCmdList:
    ...
    def test_list_empty_plugins_dir(self, mock_plugins_dir):
        from kunming_cli.plugins_cmd import cmd_list
...
class TestCopyExampleFiles:
    ...
    def test_copies_example_files(self, tmp_path):
        from kunming_cli.plugins_cmd import _copy_example_files
    ...
    def test_handles_copy_error_gracefully(self, tmp_path):
        ...
        with patch(
            "kunming_cli.plugins_cmd.shutil.copy2",
...
class TestPromptPluginEnvVars:
    ...
    def test_skips_when_no_requires_env(self):
        from kunming_cli.plugins_cmd import _prompt_plugin_env_vars
        from unittest.mock import MagicMock
    ...
    def test_skips_already_set_vars(self, monkeypatch):
        ...
        console = MagicMock()
        with patch("kunming_cli.config.get_env_value", return_value="already-set"):
    ...
    def test_prompts_for_missing_var_simple_format(self):
        ...
        with patch("kunming_cli.config.get_env_value", return_value=None), \
             patch("builtins.input", return_value="sk-test-123"), \
             patch("kunming_cli.config.save_env_value") as mock_save:
    ...
    def test_secret_uses_getpass(self):
        ...
        with patch("kunming_cli.config.get_env_value", return_value=None), \
             ...
             patch("kunming_cli.config.save_env_value"):
            _prompt_plugin_env_vars(manifest, console)
