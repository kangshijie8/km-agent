...
import os
import pytest
from kunming_cli.config import _expand_env_vars, load_config
...
class TestLoadConfigExpansion:
    def test_load_config_expands_env_vars(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("kunming_cli.config.get_config_path", lambda: config_file)
...
class TestLoadCliConfigExpansion:
    ...
    def test_cli_config_expands_auxiliary_api_key(self, tmp_path, monkeypatch):
        ...
        # Patch the kunming home so load_cli_config finds our test config
        monkeypatch.setattr("cli._kunming_home", tmp_path)
