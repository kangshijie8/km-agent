"""Tests for kunming_cli/skills_config.py and skills_tool disabled filtering."""
import pytest
...
class TestGetDisabledSkills:
    def test_empty_config(self):
        from kunming_cli.skills_config import get_disabled_skills
...
class TestSaveDisabledSkills:
    @patch("kunming_cli.skills_config.save_config")
    def test_saves_global_sorted(self, mock_save):
        from kunming_cli.skills_config import save_disabled_skills
        config = {}
...
class TestIsSkillDisabled:
    @patch("kunming_cli.config.load_config")
    ...
    @patch("kunming_cli.config.load_config")
    @patch.dict("os.environ", {"KUNMING_PLATFORM": "discord"})
...
class TestGetDisabledSkillNames:
    ...
    def test_explicit_platform_param(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("KUNMING_PLATFORM", raising=False)
        monkeypatch.delenv("KUNMING_SESSION_PLATFORM", raising=False)
    ...
    def test_session_platform_env_var(self, tmp_path, monkeypatch):
        """KUNMING_SESSION_PLATFORM should be used when KUNMING_PLATFORM is unset."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  disabled:\n"
        ...
        monkeypatch.setenv("KUNMING_HOME", str(tmp_path))
        monkeypatch.delenv("KUNMING_PLATFORM", raising=False)
        monkeypatch.setenv("KUNMING_SESSION_PLATFORM", "discord")

        from agent.skill_utils import get_disabled_skill_names
        result = get_disabled_skill_names()
        assert result == {"discord-skill"}

    def test_KUNMING_platform_takes_precedence(self, tmp_path, monkeypatch):
        """KUNMING_PLATFORM should win over KUNMING_SESSION_PLATFORM."""
        config = tmp_path / "config.yaml"
        config.write_text(
            "skills:\n"
            "  platform_disabled:\n"
        ...
        monkeypatch.setenv("KUNMING_HOME", str(tmp_path))
        monkeypatch.setenv("KUNMING_PLATFORM", "telegram")
        monkeypatch.setenv("KUNMING_SESSION_PLATFORM", "discord")
    ...
    def test_explicit_param_overrides_env_vars(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("KUNMING_HOME", str(tmp_path))
        monkeypatch.setenv("KUNMING_PLATFORM", "telegram")
        monkeypatch.setenv("KUNMING_SESSION_PLATFORM", "telegram")
...
class TestGetCategories:
    def test_extracts_unique_categories(self):
        from kunming_cli.skills_config import _get_categories
        skills = [
