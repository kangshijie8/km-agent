"""Tests for kunming_cli.tools_config platform tool persistence."""

from unittest.mock import patch

from kunming_cli.tools_config import (
    _configure_provider,
    _get_platform_tools,
...
def test_save_platform_tools_preserves_mcp_server_names():
    ...
    Regression test for https://github.com/kangshijie8/KM-Agent/issues/1247
    """
    config = {
        "platform_toolsets": {
    ...
    with patch("kunming_cli.tools_config.save_config"):
...
def test_save_platform_tools_does_not_preserve_platform_default_toolsets():
    """Platform default toolsets (kunming-cli, kunming-telegram, etc.) must NOT
    ...
    ``km tools``, but kunming-cli stays in the config and re-enables
    ...
    config = {
        "platform_toolsets": {
            "cli": [
                ...
                "delegation", "file", "kunming-cli",  # <-- the culprit
    ...
    # kunming-cli must NOT survive it's a platform default, not an MCP server
    assert "kunming-cli" not in saved
...
def test_save_platform_tools_does_not_preserve_KUNMING_telegram():
    """Same bug for Telegram kunming-telegram must not be preserved."""
    config = {
        "platform_toolsets": {
            "telegram": [
                "browser", "file", "kunming-telegram", "terminal", "web",
            ]
        }
    }
    ...
    assert "kunming-telegram" not in saved
    assert "web" in saved
...
def test_save_platform_tools_still_preserves_mcp_with_platform_default_present():
    ...
    config = {
        "platform_toolsets": {
            "cli": [
                "web", "terminal", "kunming-cli", "my-mcp-server", "github-tools",
            ]
        }
    }
...
def test_visible_providers_include_nous_subscription_when_logged_in(monkeypatch):
    ...
    monkeypatch.setattr(
        "kunming_cli.nous_subscription.get_nous_auth_status",
...
def test_visible_providers_hide_nous_subscription_when_feature_flag_is_off(monkeypatch):
    monkeypatch.delenv("KUNMING_ENABLE_NOUS_MANAGED_TOOLS", raising=False)
    config = {"model": {"provider": "nous"}}
...
def test_local_browser_provider_is_saved_explicitly(monkeypatch):
    ...
    monkeypatch.setattr("kunming_cli.tools_config._run_post_setup", lambda key: None)
...
def test_first_install_nous_auto_configures_managed_defaults(monkeypatch):
    ...
    monkeypatch.setattr(
        "kunming_cli.tools_config._prompt_toolset_checklist",
        lambda *args, **kwargs: {"web", "image_gen", "tts", "browser"},
    )
    monkeypatch.setattr("kunming_cli.tools_config.save_config", lambda config: None)
    monkeypatch.setattr(
        "kunming_cli.nous_subscription.get_nous_auth_status",
        lambda: {"logged_in": True},
    )
    ...
    configured = []
    monkeypatch.setattr(
        "kunming_cli.tools_config._configure_toolset",
...
class TestPlatformToolsetConsistency:
    ...
    def test_all_platforms_have_toolset_definitions(self):
        ...
        from kunming_cli.tools_config import PLATFORMS
    ...
    def test_gateway_toolset_includes_all_messaging_platforms(self):
        """kunming-gateway includes list should cover all messaging platforms."""
        from kunming_cli.tools_config import PLATFORMS
        from toolsets import TOOLSETS

        gateway_includes = set(TOOLSETS["kunming-gateway"]["includes"])
    ...
    def test_skills_config_covers_tools_config_platforms(self):
        ...
        from kunming_cli.tools_config import PLATFORMS as TOOLS_PLATFORMS
        from kunming_cli.skills_config import PLATFORMS as SKILLS_PLATFORMS
