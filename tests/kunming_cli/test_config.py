"""Tests for kunming_cli configuration management."""
...
class TestGetKunmingHome:
    def test_default_path(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("KUNMING_HOME", None)
            home = get_kunming_home()
            assert home == Path.home() / ".kunming"

    def test_env_override(self):
        with patch.dict(os.environ, {"KUNMING_HOME": "/custom/path"}):
            home = get_kunming_home()
            assert home == Path("/custom/path")


class TestEnsureKunmingHome:
...
class TestSaveAndLoadRoundtrip:
    def test_roundtrip(self, tmp_path):
        with patch.dict(os.environ, {"KUNMING_HOME": str(tmp_path)}):
            config = load_config()
            config["model"]["default"] = "test/custom-model"
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_updates_process_environment(self, tmp_path):
        with patch.dict(os.environ, {"KUNMING_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"KUNMING_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ(self, tmp_path):
        ...
        with patch.dict(os.environ, {"KUNMING_HOME": str(tmp_path), "MY_KEY": "my_value"}):
    ...
    def test_handles_missing_env_file(self, tmp_path):
        with patch.dict(os.environ, {"KUNMING_HOME": str(tmp_path), "GHOST_KEY": "ghost"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"KUNMING_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
...
class TestSaveConfigAtomicity:
    ...
    def test_no_partial_write_on_crash(self, tmp_path):
        ...
        with patch.dict(os.environ, {"KUNMING_HOME": str(tmp_path)}):
            ...
            config = load_config()
            config["model"]["default"] = "original-model"
            save_config(config)
            ...
            with patch("utils.yaml.dump", side_effect=OSError("disk full")):
                try:
                    config["model"]["default"] = "should-not-persist"
                    save_config(config)
                except OSError:
                    pass
    ...
    def test_atomic_write_creates_valid_yaml(self, tmp_path):
        ...
        with patch.dict(os.environ, {"KUNMING_HOME": str(tmp_path)}):
            config = load_config()
            config["model"]["default"] = "test/atomic-model"
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_tavily_in_env_vars_by_version(self):
        ...
        from kunming_cli.config import ENV_VARS_BY_VERSION
        all_vars = []
