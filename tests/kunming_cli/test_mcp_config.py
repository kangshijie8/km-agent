"""
Tests for kunming_cli.mcp_config ``km mcp`` subcommands.
...
def _isolate_config(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "kunming_cli.config.get_kunming_home", lambda: tmp_path
    )
    config_path = tmp_path / "config.yaml"
    env_path = tmp_path / ".env"
    monkeypatch.setattr(
        "kunming_cli.config.get_config_path", lambda: config_path
    )
    monkeypatch.setattr(
        "kunming_cli.config.get_env_path", lambda: env_path
    )
    return tmp_path
...
class TestMcpList:
    def test_list_empty_config(self, tmp_path, capsys):
        from kunming_cli.mcp_config import cmd_mcp_list
...
class TestMcpRemove:
    def test_remove_existing_server(self, tmp_path, capsys, monkeypatch):
        ...
        from kunming_cli.mcp_config import cmd_mcp_remove
    ...
    def test_remove_cleans_oauth_tokens(self, tmp_path, capsys, monkeypatch):
        ...
        monkeypatch.setattr("builtins.input", lambda _: "y")
        # Also patch get_kunming_home in the mcp_config module namespace
        monkeypatch.setattr(
            "kunming_cli.mcp_config.get_kunming_home", lambda: tmp_path
        )
...
class TestMcpAdd:
    def test_add_no_transport(self, capsys):
        ...
        from kunming_cli.mcp_config import cmd_mcp_add
    ...
    def test_add_http_server_all_tools(self, tmp_path, capsys, monkeypatch):
        ...
        monkeypatch.setattr(
            "kunming_cli.mcp_config._probe_single_server", mock_probe
        )
    ...
    def test_add_connection_failure_save_disabled(
        ...
        monkeypatch.setattr(
            "kunming_cli.mcp_config._probe_single_server", mock_probe_fail
        )
...
class TestMcpTest:
    def test_test_not_found(self, tmp_path, capsys):
        _seed_config(tmp_path, {})
        from kunming_cli.mcp_config import cmd_mcp_test
...
class TestConfigHelpers:
    def test_save_and_load_mcp_server(self, tmp_path):
        from kunming_cli.mcp_config import _save_mcp_server, _get_mcp_servers
    ...
    def test_remove_mcp_server(self, tmp_path):
        from kunming_cli.mcp_config import (
            _save_mcp_server,
            _remove_mcp_server,
            _get_mcp_servers,
        )
    ...
    def test_remove_nonexistent(self, tmp_path):
        from kunming_cli.mcp_config import _remove_mcp_server

        assert _remove_mcp_server("ghost") is False

    def test_env_key_for_server(self):
        from kunming_cli.mcp_config import _env_key_for_server
