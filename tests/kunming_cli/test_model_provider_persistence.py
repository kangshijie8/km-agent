"""Tests that provider selection via `km model` always persists correctly.
...
def config_home(tmp_path, monkeypatch):
    """Isolated KUNMING_HOME with a minimal string-format config."""
    ...
    monkeypatch.delenv("KUNMING_MODEL", raising=False)
    monkeypatch.delenv("LLM_MODEL", raising=False)
    monkeypatch.delenv("KUNMING_INFERENCE_PROVIDER", raising=False)
...
class TestSaveModelChoiceAlwaysDict:
    def test_string_model_becomes_dict(self, config_home):
        ...
        from kunming_cli.auth import _save_model_choice
...
class TestProviderPersistsAfterModelSave:
    def test_api_key_provider_saved_when_model_was_string(self, config_home, monkeypatch):
        ...
        from kunming_cli.main import _model_flow_api_key_provider
        ...
        with patch("kunming_cli.auth._prompt_model_selection", return_value="kimi-k2.5"), \
             patch("kunming_cli.auth.deactivate_provider"), \
    ...
    def test_copilot_provider_saved_when_selected(self, config_home):
        ...
        from kunming_cli.main import _model_flow_copilot
        from kunming_cli.config import load_config

        with patch(
            "kunming_cli.auth.resolve_api_key_provider_credentials",
            return_value={
                "provider": "copilot",
            ...
            "kunming_cli.auth._prompt_model_selection",
            return_value="gpt-5.4",
        ), patch(
            "kunming_cli.main._prompt_reasoning_effort_selection",
            return_value="high",
        ), patch(
            "kunming_cli.auth.deactivate_provider",
        ):
    ...
    def test_copilot_acp_provider_saved_when_selected(self, config_home):
        ...
        from kunming_cli.main import _model_flow_copilot_acp
        from kunming_cli.config import load_config

        with patch(
            "kunming_cli.auth.get_external_process_provider_status",
            return_value={
            ...
            "kunming_cli.auth.resolve_external_process_provider_credentials",
            return_value={
    ...
    def test_opencode_go_models_are_selectable_and_persist_normalized(self, config_home, monkeypatch):
        ...
        with patch("kunming_cli.models.fetch_api_models", return_value=["opencode-go/kimi-k2.5", "opencode-go/minimax-m2.7"]), \
             patch("kunming_cli.auth._prompt_model_selection", return_value="kimi-k2.5"), \
             patch("kunming_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value=""):
    ...
    def test_opencode_go_same_provider_switch_recomputes_api_mode(self, config_home, monkeypatch):
        ...
        with patch("kunming_cli.models.fetch_api_models", return_value=["opencode-go/kimi-k2.5", "opencode-go/minimax-m2.5"]), \
             patch("kunming_cli.auth._prompt_model_selection", return_value="minimax-m2.5"), \
             patch("kunming_cli.auth.deactivate_provider"), \
             patch("builtins.input", return_value=""):
