"""Tests for the Kunming plugin system (kunming_cli.plugins)."""
...
from kunming_cli.plugins import (
    ENTRY_POINTS_GROUP,
    VALID_HOOKS,
    LoadedPlugin,
    PluginContext,
...
class TestPluginDiscovery:
    ...
    def test_discover_user_plugins(self, tmp_path, monkeypatch):
        """Plugins in ~/.kunming/plugins/ are discovered."""
        plugins_dir = tmp_path / "KUNMING_test" / "plugins"
        _make_plugin_dir(plugins_dir, "hello_plugin")
        monkeypatch.setenv("KUNMING_HOME", str(tmp_path / "KUNMING_test"))
    ...
    def test_discover_project_plugins(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setenv("KUNMING_ENABLE_PROJECT_PLUGINS", "true")
        plugins_dir = project_dir / ".kunming" / "plugins"
...
class TestPluginLoading:
    ...
    def test_load_registers_namespace_module(self, tmp_path, monkeypatch):
        """Directory plugins are importable under KUNMING_plugins.<name>."""
        plugins_dir = tmp_path / "KUNMING_test" / "plugins"
        _make_plugin_dir(plugins_dir, "ns_plugin")
        monkeypatch.setenv("KUNMING_HOME", str(tmp_path / "KUNMING_test"))

        # Clean up any prior namespace module
        sys.modules.pop("KUNMING_plugins.ns_plugin", None)

        mgr = PluginManager()
        mgr.discover_and_load()

        assert "KUNMING_plugins.ns_plugin" in sys.modules
...
class TestPluginHooks:
    ...
    def test_invalid_hook_name_warns(self, tmp_path, monkeypatch, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="kunming_cli.plugins"):
            mgr = PluginManager()
            mgr.discover_and_load()
...
class TestPluginToolVisibility:
    ...
    def test_plugin_tools_in_definitions(self, tmp_path, monkeypatch):
        ...
        import kunming_cli.plugins as plugins_mod
...
# in PluginContext (kunming_cli/plugins.py).  The tests referenced _plugin_commands,
