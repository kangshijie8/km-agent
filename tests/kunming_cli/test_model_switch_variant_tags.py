...
from kunming_cli.model_switch import switch_model
...
def _run_switch(raw_input: str, current_provider: str = "openrouter") -> str:
    ...
    with patch("kunming_cli.model_switch.resolve_alias", return_value=None), \
         patch("kunming_cli.model_switch.list_provider_models", return_value=[]), \
         patch("kunming_cli.runtime_provider.resolve_runtime_provider",
               return_value={"api_key": "test", "base_url": "", "api_mode": "chat_completions"}), \
         patch("kunming_cli.models.validate_requested_model", return_value=_MOCK_VALIDATION), \
         patch("kunming_cli.model_switch.get_model_info", return_value=None), \
         patch("kunming_cli.model_switch.get_model_capabilities", return_value=None), \
         patch("kunming_cli.models.detect_provider_for_model", return_value=None):
        result = switch_model(
            raw_input=raw_input,
