"""Tests for the top-level `./kunming` launcher script."""
...
def test_launcher_delegates_to_argparse_entrypoint(monkeypatch):
    """`./kunming` should use `kunming_cli.main`, not the legacy Fire wrapper."""
    launcher_path = Path(__file__).resolve().parents[2] / "kunming"
    called = []

    fake_main_module = types.ModuleType("kunming_cli.main")

    def fake_main():
        called.append("kunming_cli.main")

    fake_main_module.main = fake_main
    monkeypatch.setitem(sys.modules, "kunming_cli.main", fake_main_module)
    ...
    assert called == ["kunming_cli.main"]
