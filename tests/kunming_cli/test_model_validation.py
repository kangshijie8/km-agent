"""Tests for provider-aware `/model` validation in kunming_cli.models."""
...
def _validate(model, provider="openrouter", api_models=FAKE_API_MODELS, **kw):
    ...
    with patch("kunming_cli.models.fetch_api_models", return_value=api_models), \
         patch("kunming_cli.models.probe_api_models", return_value=probe_payload):
...
class TestParseModelInput:
    ...
    def test_nous_provider_switch(self):
        provider, model = parse_model_input("nous:kunming-3", "openrouter")
        assert provider == "nous"
        assert model == "kunming-3"
    ...
    def test_custom_triple_empty_model_falls_back(self):
        """custom:name: with no model 鈫?treated as custom:name (bare)."""
        provider, model = parse_model_input("custom:name:", "openrouter")
        # Empty model after second colon 鈫?no triple match, falls through
        assert provider == "custom"
        assert model == "name:"
...
class TestProviderModelIds:
    ...
    def test_copilot_prefers_live_catalog(self):
        with patch("kunming_cli.auth.resolve_api_key_provider_credentials", return_value={"api_key": "gh-token"}), \
             patch("kunming_cli.models._fetch_github_models", return_value=["gpt-5.4", "claude-sonnet-4.6"]):
    ...
    def test_copilot_acp_falls_back_to_copilot_defaults(self):
        with patch("kunming_cli.auth.resolve_api_key_provider_credentials", side_effect=Exception("no token")), \
             patch("kunming_cli.models._fetch_github_models", return_value=None):
...
class TestFetchApiModels:
    ...
    def test_returns_none_on_network_error(self):
        with patch("kunming_cli.models.urllib.request.urlopen", side_effect=Exception("timeout")):
    ...
    def test_probe_api_models_tries_v1_fallback(self):
        ...
        with patch("kunming_cli.models.urllib.request.urlopen", side_effect=_fake_urlopen):
    ...
    def test_probe_api_models_uses_copilot_catalog(self):
        ...
        with patch("kunming_cli.models.urllib.request.urlopen", return_value=_Resp()) as mock_urlopen:
    ...
    def test_fetch_github_model_catalog_filters_non_chat_models(self):
        ...
        with patch("kunming_cli.models.urllib.request.urlopen", return_value=_Resp()):
...
class TestCopilotNormalization:
    ...
    def test_copilot_api_mode_gpt5_mini_uses_chat(self):
        """gpt-5-mini is the exception 鈥?uses Chat Completions."""
...
# -- validate 鈥?API unreachable 鈥?accept and persist everything ----------------
...
class TestValidateApiFallback:
    ...
    def test_unknown_model_also_accepted_when_api_down(self):
        """No hardcoded catalog gatekeeping 鈥?accept, persist, and warn."""
