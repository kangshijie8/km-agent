...
class TestNonInteractiveSetup:
    ...
    def test_non_interactive_flag_skips_wizard(self, capsys):
        ...
        with (
            patch("kunming_cli.setup.ensure_kunming_home"),
            patch("kunming_cli.setup.load_config", return_value={}),
            patch("kunming_cli.setup.get_kunming_home", return_value="/tmp/.kunming"),
            patch("kunming_cli.auth.get_active_provider", side_effect=AssertionError("wizard continued")),
            patch("builtins.input", side_effect=AssertionError("input should not be called")),
        ):
            run_setup_wizard(args)

        out = capsys.readouterr().out
        assert "km config set model.provider custom" in out
    ...
    def test_chat_first_run_headless_skips_setup_prompt(self, capsys):
        """Bare `km` should not prompt for input when no provider exists and stdin is headless."""
        from kunming_cli.main import cmd_chat

        args = _make_chat_args()

        with (
            patch("kunming_cli.main._has_any_provider_configured", return_value=False),
            patch("kunming_cli.main.cmd_setup") as mock_setup,
            patch("sys.stdin") as mock_stdin,
            patch("builtins.input", side_effect=AssertionError("input should not be called")),
        ):
            mock_stdin.isatty.return_value = False
    ...
    def test_returning_user_terminal_menu_choice_dispatches_terminal_section(self, tmp_path):
        ...
        from kunming_cli import setup as setup_mod
        ...
        with (
            patch.object(setup_mod, "ensure_kunming_home"),
            patch.object(setup_mod, "load_config", return_value=config),
            patch.object(setup_mod, "get_kunming_home", return_value=tmp_path),
            ...
            patch("kunming_cli.auth.get_active_provider", return_value=None),
