...
import kunming_cli.doctor as doctor
import kunming_cli.gateway as gateway_cli
from kunming_cli import doctor as doctor_mod
from kunming_cli.doctor import _has_provider_env_config
...
def test_run_doctor_sets_interactive_env_for_tool_checks(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(doctor_mod, "KUNMING_HOME", KUNMING_home)
    monkeypatch.delenv("KUNMING_INTERACTIVE", raising=False)

    seen = {}

    def fake_check_tool_availability(*args, **kwargs):
        seen["interactive"] = os.getenv("KUNMING_INTERACTIVE")
        raise SystemExit(0)
...
class TestDoctorMemoryProviderSection:
    ...
    def _make_KUNMING_home(self, tmp_path, provider=""):
        """Create a minimal KUNMING_HOME with config.yaml."""
    ...
    def _run_doctor_and_capture(self, monkeypatch, tmp_path, provider=""):
        ...
        home = self._make_KUNMING_home(tmp_path, provider)
        monkeypatch.setattr(doctor_mod, "KUNMING_HOME", home)
        ...
        try:
            from kunming_cli import auth as _auth_mod
