...
import kunming_cli.gateway as gateway_cli


class TestSystemdServiceRefresh:
    def test_systemd_install_repairs_outdated_unit_without_force(self, tmp_path, monkeypatch):
        unit_path = tmp_path / "kunming-gateway.service"
...
class TestLaunchdServiceRecovery:
    def test_launchd_install_repairs_outdated_plist_without_force(self, tmp_path, monkeypatch):
        plist_path = tmp_path / "ai.kunming.gateway.plist"
    ...
    def test_gateway_restart_does_not_fallback_to_foreground_when_launchd_restart_fails(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda: (_ for _ in ()).throw(
                gateway_cli.subprocess.CalledProcessError(5, ["launchctl", "kickstart", "-k", "gui/501/ai.kunming.gateway"])
            ),
        )
...
class TestSystemUnitKunmingHome:
    """KUNMING_HOME in system units must reference the target user, not root."""
    ...
    def test_system_unit_uses_target_user_home_not_calling_user(self, monkeypatch):
        ...
        assert 'KUNMING_HOME=/home/alice/.kunming' in unit
        assert '/root/.kunming' not in unit

    def test_system_unit_remaps_profile_to_target_user(self, monkeypatch):
        # Simulate sudo with a profile: KUNMING_HOME was resolved under root
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        ...
        assert 'KUNMING_HOME=/home/alice/.kunming/profiles/coder' in unit
        assert '/root/' not in unit

    def test_system_unit_preserves_custom_KUNMING_home(self, monkeypatch):
        # Custom KUNMING_HOME not under any user's home keep as-is
        monkeypatch.setattr(Path, "home", staticmethod(lambda: Path("/root")))
        monkeypatch.setenv("KUNMING_HOME", "/opt/kunming-shared")
        monkeypatch.setattr(
            gateway_cli, "_system_service_identity",
            lambda run_as_user=None: ("alice", "alice", "/home/alice"),
        )
        ...
        assert 'KUNMING_HOME=/opt/kunming-shared' in unit

    def test_user_unit_unaffected_by_change(self):
        # User-scope units should still use the calling user's KUNMING_HOME
        unit = gateway_cli.generate_systemd_unit(system=False)

        KUNMING_home = str(gateway_cli.get_kunming_home().resolve())
        assert f'KUNMING_HOME={KUNMING_home}' in unit
...
class TestProfileArg:
    ...
    def test_default_KUNMING_home_returns_empty(self, tmp_path, monkeypatch):
        """Default ~/.kunming should not produce a --profile flag."""
        KUNMING_home = tmp_path / ".kunming"
        KUNMING_home.mkdir()
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
        result = gateway_cli._profile_arg(str(KUNMING_home))
        assert result == ""

    def test_named_profile_returns_flag(self, tmp_path, monkeypatch):
        """~/.kunming/profiles/mybot should return '--profile mybot'."""
        profile_dir = tmp_path / ".kunming" / "profiles" / "mybot"
    ...
    def test_hash_path_returns_empty(self, tmp_path, monkeypatch):
        """Arbitrary non-profile KUNMING_HOME should return empty string."""
        custom_home = tmp_path / "custom" / "kunming"
    ...
    def test_nested_profile_path_returns_empty(self, tmp_path, monkeypatch):
        """~/.kunming/profiles/mybot/subdir should NOT match too deep."""
        nested = tmp_path / ".kunming" / "profiles" / "mybot" / "subdir"
    ...
    def test_invalid_profile_name_returns_empty(self, tmp_path, monkeypatch):
        ...
        bad_profile = tmp_path / ".kunming" / "profiles" / "My Bot!"
    ...
    def test_systemd_unit_includes_profile(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(gateway_cli, "get_kunming_home", lambda: profile_dir)
        unit = gateway_cli.generate_systemd_unit(system=False)
