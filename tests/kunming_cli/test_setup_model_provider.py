...
from kunming_cli.main, these tests mock the delegation point and verify
that the setup wizard correctly syncs config from disk after the call.
"""

from __future__ import annotations

from kunming_cli.config import load_config, save_config, save_env_value
from kunming_cli.nous_subscription import NousFeatureState, NousSubscriptionFeatures
from kunming_cli.setup import _print_setup_summary, setup_model_provider
...
def _stub_tts(monkeypatch):
    monkeypatch.setattr("kunming_cli.setup.prompt_choice", lambda q, c, d=0: (
        _maybe_keep_current_tts(q, c) if _maybe_keep_current_tts(q, c) is not None
        else d
    ))
    monkeypatch.setattr("kunming_cli.setup.prompt_yes_no", lambda *a, **kw: False)
...
def test_setup_keep_current_custom_from_config_does_not_fall_through(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("kunming_cli.main.select_provider_and_model", fake_select)
...
def test_setup_same_provider_rotation_strategy_saved_for_multi_credential_pool(tmp_path, monkeypatch):
    ...
    import kunming_cli.main as _main_mod
    import kunming_cli.setup as _setup_mod
...
def test_setup_same_provider_fallback_can_add_another_credential(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("kunming_cli.main.select_provider_and_model", fake_select)
    _stub_tts(monkeypatch)
    monkeypatch.setattr("kunming_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("kunming_cli.setup.prompt_yes_no", fake_prompt_yes_no)
    monkeypatch.setattr("kunming_cli.setup.prompt", lambda *args, **kwargs: "")
    monkeypatch.setattr("agent.credential_pool.load_pool", fake_load_pool)
    monkeypatch.setattr("kunming_cli.auth_commands.auth_add_command", fake_auth_add_command)
...
def test_setup_pool_step_shows_manual_vs_auto_detected_counts(tmp_path, monkeypatch, capsys):
    ...
    _stub_tts(monkeypatch)
    monkeypatch.setattr("kunming_cli.setup.prompt_choice", fake_prompt_choice)
    monkeypatch.setattr("kunming_cli.setup.prompt_yes_no", lambda *args, **kwargs: False)
    monkeypatch.setattr("kunming_cli.setup.prompt", lambda *args, **kwargs: "")
...
def test_setup_copilot_acp_skips_same_provider_pool_step(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("kunming_cli.setup.prompt_yes_no", fake_prompt_yes_no)
    monkeypatch.setattr("kunming_cli.setup.prompt", lambda *args, **kwargs: "")
    monkeypatch.setattr("kunming_cli.auth.get_active_provider", lambda: None)
    monkeypatch.setattr("kunming_cli.auth.detect_external_credentials", lambda: [])
    monkeypatch.setattr("agent.auxiliary_client.get_available_vision_backends", lambda: [])
...
def test_setup_summary_marks_anthropic_auth_as_vision_available(tmp_path, monkeypatch, capsys):
    ...
    assert "missing run 'km setup' to configure" not in output
...
def test_setup_summary_shows_camofox_when_browser_feature_is_camofox(tmp_path, monkeypatch, capsys):
    ...
    _clear_provider_env(monkeypatch)
    monkeypatch.setattr(
        "kunming_cli.setup.get_nous_subscription_features",
