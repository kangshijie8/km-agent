"""Tests for the update check mechanism in kunming_cli.banner."""
...
def test_version_string_no_v_prefix():
    ...
    from kunming_cli import __version__
...
def test_check_for_updates_uses_cache(tmp_path, monkeypatch):
    ...
    repo_dir = tmp_path / "kunming-agent"
    repo_dir.mkdir()
    (repo_dir / ".git").mkdir()
    ...
    with patch("kunming_cli.banner.subprocess.run") as mock_run:
...
def test_check_for_updates_expired_cache(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("KUNMING_HOME", str(tmp_path))
    with patch("kunming_cli.banner.subprocess.run", return_value=mock_result) as mock_run:
        result = check_for_updates()
...
def test_check_for_updates_no_git_dir(tmp_path, monkeypatch):
    ...
    fake_banner = tmp_path / "kunming_cli" / "banner.py"
...
def test_check_for_updates_fallback_to_project_root(tmp_path, monkeypatch):
    """Dev install: falls back to Path(__file__).parent.parent when KUNMING_HOME has no git repo."""
    import kunming_cli.banner as banner
    ...
    # Point KUNMING_HOME at a temp dir with no kunming-agent/.git
    monkeypatch.setenv("KUNMING_HOME", str(tmp_path))
    with patch("kunming_cli.banner.subprocess.run") as mock_run:
...
def test_invalidate_update_cache_clears_all_profiles(tmp_path):
    ...
    from kunming_cli.main import _invalidate_update_cache

    # Build a fake ~/.kunming with default + two named profiles
