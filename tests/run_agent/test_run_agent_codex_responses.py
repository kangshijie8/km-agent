...
def test_build_api_kwargs_codex(monkeypatch):
    ...
    kwargs = agent._build_api_kwargs(
        [
            {"role": "system", "content": "You are Kunming."},
            {"role": "user", "content": "Ping"},
        ]
    )

    assert kwargs["model"] == "gpt-5-codex"
    assert kwargs["instructions"] == "You are Kunming."
...
def test_run_conversation_codex_continues_after_ack_stop_message(monkeypatch):
    ...
    responses = [
        _codex_ack_message_response(
            "Absolutely -I can do that. I'll inspect ~/km-studio and report back with a walkthrough."
        ),
        _codex_tool_call_response(),
        _codex_message_response("Architecture summary complete."),
    ]
    ...
    result = agent.run_conversation("look into ~/km-studio and tell me how it works")
    ...
    assert any(
        msg.get("role") == "assistant"
        and msg.get("finish_reason") == "incomplete"
        and "inspect ~/km-studio" in (msg.get("content") or "")
        for msg in result["messages"]
    )
    assert any(
        msg.get("role") == "user"
