...
class TestBuildSystemPrompt:
    ...
    def test_includes_kunming_subscription_prompt(self, agent, monkeypatch):
        monkeypatch.setattr(run_agent, "build_kunming_subscription_prompt", lambda tool_names: "KUNMING SUBSCRIPTION BLOCK")
        prompt = agent._build_system_prompt()
        assert "KUNMING SUBSCRIPTION BLOCK" in prompt
...
class TestBuildApiKwargs:
    ...
    def test_reasoning_sent_for_nous_route(self, agent):
        agent.base_url = "https://inference-api.kunming.dev/v1"
        agent.model = "minimax/minimax-m2.5"
        messages = [{"role": "user", "content": "hi"}]
        kwargs = agent._build_api_kwargs(messages)
        assert kwargs["extra_body"]["reasoning"]["effort"] == "medium"
...
class TestRunConversation:
    ...
    def test_request_scoped_api_hooks_fire_for_each_api_call(self, agent):
        ...
        with (
            ...
            patch("kunming_cli.plugins.invoke_hook", side_effect=_record_hook),
            patch.object(agent, "_persist_session"),
            patch.object(agent, "_save_trajectory"),
            patch.object(agent, "_cleanup_task_resources"),
        ):
    ...
    def test_try_refresh_nous_client_credentials_rebuilds_client(
        ...
        def _fake_resolve(**kwargs):
            ...
            return {
                ...
                "base_url": "https://inference-api.kunming.dev/v1",
            }
        ...
        monkeypatch.setattr(
            "kunming_cli.auth.resolve_nous_runtime_credentials", _fake_resolve
        )
        ...
        assert (
            rebuilt["kwargs"]["base_url"] == "https://inference-api.kunming.dev/v1"
        )
...
class TestSystemPromptStability:
    ...
    def test_fresh_build_when_no_history(self, agent):
        ...
        assert "Kunming Agent" in agent._cached_system_prompt
...
class TestSafeWriter:
    ...
    # test_installed_before_init_time_honcho_error_prints removed     # Honcho integration extracted to plugin (PR #4154).
