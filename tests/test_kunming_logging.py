"""Tests for kunming_logging — centralized logging setup."""
...
def _reset_logging_state():
    ...
    """
    kunming_logging._logging_initialized = False
...
@pytest.fixture
def kunming_home(tmp_path, monkeypatch):
    """Provide an isolated KUNMING_HOME for logging tests.

    Uses the same tmp_path as the autouse _isolate_KUNMING_home from conftest,
    reading it back from the env var to avoid double-mkdir conflicts.
    """
    home = Path(os.environ["KUNMING_HOME"])
    return home
...
class TestSetupLogging:
    ...
    def test_creates_log_directory(self, kunming_home):
        log_dir = kunming_logging.setup_logging(kunming_home=kunming_home)
        assert log_dir == kunming_home / "logs"
        assert log_dir.is_dir()

    def test_creates_agent_log_handler(self, kunming_home):
        kunming_logging.setup_logging(kunming_home=kunming_home)
        root = logging.getLogger()
    ...
    def test_creates_errors_log_handler(self, kunming_home):
        kunming_logging.setup_logging(kunming_home=kunming_home)
        root = logging.getLogger()
    ...
    def test_idempotent_no_duplicate_handlers(self, kunming_home):
        kunming_logging.setup_logging(kunming_home=kunming_home)
        kunming_logging.setup_logging(kunming_home=kunming_home)  # second call — should be no-op
    ...
    def test_force_reinitializes(self, kunming_home):
        kunming_logging.setup_logging(kunming_home=kunming_home)
        # Force still won't add duplicate handlers because _add_rotating_handler
        # checks by resolved path.
        kunming_logging.setup_logging(kunming_home=kunming_home, force=True)
    ...
    def test_custom_log_level(self, kunming_home):
        kunming_logging.setup_logging(kunming_home=kunming_home, log_level="DEBUG")
    ...
    def test_custom_max_size_and_backup(self, kunming_home):
        kunming_logging.setup_logging(
            kunming_home=kunming_home, max_size_mb=10, backup_count=5
        )
    ...
    def test_suppresses_noisy_loggers(self, kunming_home):
        kunming_logging.setup_logging(kunming_home=kunming_home)
    ...
    def test_writes_to_agent_log(self, kunming_home):
        kunming_logging.setup_logging(kunming_home=kunming_home)

        test_logger = logging.getLogger("test_kunming_logging.write_test")
        ...
        agent_log = kunming_home / "logs" / "agent.log"
        assert agent_log.exists()
        content = agent_log.read_text()
        assert "test message for agent.log" in content

    def test_warnings_appear_in_both_logs(self, kunming_home):
        kunming_logging.setup_logging(kunming_home=kunming_home)

        test_logger = logging.getLogger("test_kunming_logging.warning_test")
        ...
        agent_log = kunming_home / "logs" / "agent.log"
        errors_log = kunming_home / "logs" / "errors.log"
        assert "this is a warning" in agent_log.read_text()
        assert "this is a warning" in errors_log.read_text()

    def test_info_not_in_errors_log(self, kunming_home):
        kunming_logging.setup_logging(kunming_home=kunming_home)

        test_logger = logging.getLogger("test_kunming_logging.info_test")
    ...
    def test_explicit_params_override_config(self, kunming_home):
        """Explicit function params take precedence over config.yaml."""
        import yaml
        config = {"logging": {"level": "DEBUG"}}
        (kunming_home / "config.yaml").write_text(yaml.dump(config))

        kunming_logging.setup_logging(kunming_home=kunming_home, log_level="WARNING")
...
class TestSetupVerboseLogging:
    ...
    def test_adds_stream_handler(self, kunming_home):
        kunming_logging.setup_logging(kunming_home=kunming_home)
        kunming_logging.setup_verbose_logging()
        ...
        verbose_handlers = [
            ...
            and getattr(h, "_kunming_verbose", False)
        ]
        assert len(verbose_handlers) == 1
        assert verbose_handlers[0].level == logging.DEBUG

    def test_idempotent(self, kunming_home):
        kunming_logging.setup_logging(kunming_home=kunming_home)
        kunming_logging.setup_verbose_logging()
        kunming_logging.setup_verbose_logging()  # second call
...
class TestAddRotatingHandler:
    ...
    def test_creates_directory(self, tmp_path):
        ...
        kunming_logging._add_rotating_handler(
            logger, log_path,
...
class TestReadLoggingConfig:
    ...
    def test_returns_none_when_no_config(self, kunming_home):
        level, max_size, backup = kunming_logging._read_logging_config()
        assert level is None
        assert max_size is None
        assert backup is None

    def test_reads_logging_section(self, kunming_home):
        import yaml
    ...
    def test_handles_missing_logging_section(self, kunming_home):
        import yaml
        config = {"model": "test"}
        (kunming_home / "config.yaml").write_text(yaml.dump(config))
