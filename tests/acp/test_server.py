"""Tests for acp_adapter.server -KunmingACPAgent ACP server."""
...
from acp_adapter.server import KunmingACPAgent, KUNMING_VERSION
...
def agent(mock_manager):
    """KunmingACPAgent backed by a mock session manager."""
    return KunmingACPAgent(session_manager=mock_manager)
...
class TestInitialize:
    ...
    async def test_initialize_returns_agent_info(self, agent):
        ...
        assert resp.agent_info.name == "kunming-agent"
        assert resp.agent_info.version == KUNMING_VERSION
...
class TestSlashCommands:
    ...
    def test_version(self, agent, mock_manager):
        ...
        assert KUNMING_VERSION in result
    ...
    def test_model_switch_uses_requested_provider(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("kunming_cli.config.load_config", lambda: {
        ...
        with patch("run_agent.AIAgent", side_effect=fake_agent):
            acp_agent = KunmingACPAgent(session_manager=manager)
...
class TestRegisterSessionMcpServers:
    ...
    async def test_registers_stdio_servers(self, agent, mock_manager):
        ...
        state.agent.enabled_toolsets = ["kunming-acp"]
