"""
Tests for timezone support (KUNMING_time module + integration points).
...
import kunming_time as KUNMING_time


# =========================================================================
# KUNMING_time.now() — core helper
# =========================================================================

class TestKunmingTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        KUNMING_time.reset_cache()

    def teardown_method(self):
        KUNMING_time.reset_cache()
        os.environ.pop("KUNMING_TIMEZONE", None)

    def test_valid_timezone_applies(self):
        """With a valid IANA timezone, now() returns time in that zone."""
        os.environ["KUNMING_TIMEZONE"] = "Asia/Kolkata"
        result = KUNMING_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["KUNMING_TIMEZONE"] = "UTC"
        result = KUNMING_time.now()
        assert result.utcoffset() == timedelta(0)

    def test_us_eastern(self):
        """US/Eastern timezone works (DST-aware zone)."""
        os.environ["KUNMING_TIMEZONE"] = "America/New_York"
        result = KUNMING_time.now()
        assert result.tzinfo is not None
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["KUNMING_TIMEZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="KUNMING_time"):
            result = KUNMING_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["KUNMING_TIMEZONE"] = "Europe/London"
        tz = KUNMING_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        os.environ["KUNMING_TIMEZONE"] = "Not/A/Timezone"
        tz = KUNMING_time.get_timezone()
        assert tz is None

    def test_get_timezone_name(self):
        os.environ["KUNMING_TIMEZONE"] = "Asia/Tokyo"
        assert KUNMING_time.get_timezone_name() == "Asia/Tokyo"
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When KUNMING_TIMEZONE is set, child process sees TZ env var."""
        import json as _json
        os.environ["KUNMING_TIMEZONE"] = "Asia/Kolkata"
    ...
    def test_tz_not_injected_when_empty(self):
        """When KUNMING_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("KUNMING_TIMEZONE", None)
    ...
    def test_KUNMING_timezone_not_leaked_to_child(self):
        """KUNMING_TIMEZONE itself must NOT appear in child env (only TZ)."""
        import json as _json
        os.environ["KUNMING_TIMEZONE"] = "Asia/Kolkata"

        with patch("model_tools.handle_function_call", side_effect=self._mock_handle):
            result = _json.loads(self._execute_code(
                code='import os; print(os.environ.get("KUNMING_TIMEZONE", "NOT_SET"))',
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=KUNMING_tz) which shifted
        absolute time when system-local tz != Kunming tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_KUNMING_tz(self):
        """Already-aware datetimes should be normalized to Kunming tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in Kunming tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of Kunming tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), Kunming is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Kunming tz
        ...
        # Use a Kunming timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _KUNMING_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["KUNMING_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        KUNMING_time.reset_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Kunming timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["KUNMING_TIMEZONE"] = "US/Eastern"
        KUNMING_time.reset_cache()
