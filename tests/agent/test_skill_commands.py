...
"""
    ...
    def test_gateway_still_loads_skill_but_returns_setup_guidance(
        ...
        with patch.dict(
            os.environ, {"KUNMING_SESSION_PLATFORM": "telegram"}, clear=False
        ):
            with patch("tools.skills_tool.SKILLS_DIR", tmp_path):
                _make_skill(
                    tmp_path,
...
class TestPlanSkillHelpers:
    def test_build_plan_path_uses_workspace_relative_dir_and_slugifies_request(self):
        ...
        assert path == Path(".kunming") / "plans" / "2026-03-15_093045-implement-oauth-login-refresh-tokens.md"
    ...
    def test_plan_skill_message_can_include_runtime_save_path_note(self, tmp_path):
        with patch("tools.skills_tool.SKILLS_DIR", tmp_path):
            _make_skill(
                tmp_path,
                "plan",
                body="Save plans under .kunming/plans in the active workspace and do not execute the work.",
            )
            scan_skill_commands()
            msg = build_skill_invocation_message(
                ...
                runtime_note=(
                    ...
                    "the active workspace/backend cwd: .kunming/plans/plan.md"
                ),
            )

        assert msg is not None
        assert "Save plans under $KUNMING_HOME/plans" not in msg
        assert ".kunming/plans" in msg
        assert "Add a /plan command" in msg
        assert ".kunming/plans/plan.md" in msg
