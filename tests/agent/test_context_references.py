...
def sample_repo(tmp_path: Path) -> Path:
    ...
    repo.mkdir()
    _git(repo, "init")
    _git(repo, "config", "user.name", "Kunming Tests")
...
@pytest.mark.asyncio
async def test_blocks_sensitive_home_and_KUNMING_paths(tmp_path: Path, monkeypatch):
    from agent.context_references import preprocess_context_references_async
    ...
    KUNMING_env.parent.mkdir(parents=True)
    KUNMING_env.write_text("API_KEY=super-secret\n", encoding="utf-8")
    ...
    result = await preprocess_context_references_async(
        "read @file:.kunming/.env and @file:.ssh/id_rsa",
        cwd=tmp_path,
