...
def provider(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "plugins.memory.hindsight.get_kunming_home", lambda: tmp_path
    )

    p = HindsightMemoryProvider()
    p.initialize(session_id="test-session", KUNMING_home=str(tmp_path), platform="cli")
...
class TestConfig:
    def test_default_values(self, provider):
        ...
        assert provider._retain_context == "conversation between Kunming Agent and the User"
    ...
    def test_config_from_env_fallback(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "plugins.memory.hindsight.get_kunming_home",
        ...
        assert cfg["banks"]["kunming"]["bankId"] == "env-bank"
        assert cfg["banks"]["kunming"]["budget"] == "high"
...
class TestSyncTurn:
    ...
    def test_sync_turn_uses_aretain_batch(self, provider):
        ...
        assert call_kwargs["items"][0]["context"] == "conversation between Kunming Agent and the User"
