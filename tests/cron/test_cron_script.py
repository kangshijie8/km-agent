...
- Path resolution (absolute, relative to KUNMING_HOME/scripts/)
"""
...
def cron_env(tmp_path, monkeypatch):
    """Isolated cron environment with temp KUNMING_HOME."""
    KUNMING_home = tmp_path / ".kunming"
    KUNMING_home.mkdir()
    (KUNMING_home / "cron").mkdir()
    (KUNMING_home / "cron" / "output").mkdir()
    (KUNMING_home / "scripts").mkdir()
    monkeypatch.setenv("KUNMING_HOME", str(KUNMING_home))

    # Clear cached module-level paths
    import cron.jobs as jobs_mod
    monkeypatch.setattr(jobs_mod, "KUNMING_DIR", KUNMING_home)
    monkeypatch.setattr(jobs_mod, "CRON_DIR", KUNMING_home / "cron")
    monkeypatch.setattr(jobs_mod, "JOBS_FILE", KUNMING_home / "cron" / "jobs.json")
    monkeypatch.setattr(jobs_mod, "OUTPUT_DIR", KUNMING_home / "cron" / "output")
...
class TestCronjobToolScript:
    ...
    def test_create_with_script(self, cron_env, monkeypatch):
        monkeypatch.setenv("KUNMING_INTERACTIVE", "1")
...
class TestScriptPathContainment:
    ...
    def test_absolute_path_outside_scripts_dir_blocked(self, cron_env):
        """Absolute paths outside ~/.kunming/scripts/ must be rejected."""
        from cron.scheduler import _run_job_script
...
class TestRunJobEnvVarCleanup:
    ...
    def test_env_vars_cleaned_on_early_error(self, cron_env, monkeypatch):
        ...
        assert os.environ.get("KUNMING_SESSION_PLATFORM") is None
        assert os.environ.get("KUNMING_SESSION_CHAT_ID") is None
        assert os.environ.get("KUNMING_SESSION_CHAT_NAME") is None
