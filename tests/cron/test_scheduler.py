...
class TestRunJobSessionPersistence:
    def test_run_job_passes_session_db_and_cron_platform(self, tmp_path):
        ...
        with patch("cron.scheduler._kunming_home", tmp_path), \
             patch("cron.scheduler._resolve_origin", return_value=None), \
             patch("dotenv.load_dotenv"), \
             patch("kunming_state.SessionDB", return_value=fake_db), \
             patch(
                 "kunming_cli.runtime_provider.resolve_runtime_provider",
                 return_value={
                     "api_key": "test-key",
    ...
    def test_run_job_sets_auto_delivery_env_from_dotenv_home_channel(self, tmp_path, monkeypatch):
        ...
        monkeypatch.delenv("TELEGRAM_HOME_CHANNEL", raising=False)
        monkeypatch.delenv("KUNMING_CRON_AUTO_DELIVER_PLATFORM", raising=False)
        monkeypatch.delenv("KUNMING_CRON_AUTO_DELIVER_CHAT_ID", raising=False)
        monkeypatch.delenv("KUNMING_CRON_AUTO_DELIVER_THREAD_ID", raising=False)
        ...
        class FakeAgent:
            ...
            def run_conversation(self, *args, **kwargs):
                seen["platform"] = os.getenv("KUNMING_CRON_AUTO_DELIVER_PLATFORM")
                seen["chat_id"] = os.getenv("KUNMING_CRON_AUTO_DELIVER_CHAT_ID")
                seen["thread_id"] = os.getenv("KUNMING_CRON_AUTO_DELIVER_THREAD_ID")
        ...
        assert os.getenv("KUNMING_CRON_AUTO_DELIVER_PLATFORM") is None
        assert os.getenv("KUNMING_CRON_AUTO_DELIVER_CHAT_ID") is None
        assert os.getenv("KUNMING_CRON_AUTO_DELIVER_THREAD_ID") is None
        fake_db.close.assert_called_once()
...
class TestRunJobPerJobOverrides:
    def test_job_level_model_provider_and_base_url_overrides_are_used(self, tmp_path):
        ...
        with patch("cron.scheduler._kunming_home", tmp_path), \
             ...
             patch("dotenv.load_dotenv"), \
             patch("kunming_state.SessionDB", return_value=fake_db), \
             patch("kunming_cli.runtime_provider.resolve_runtime_provider", return_value=fake_runtime) as runtime_mock, \
             patch("run_agent.AIAgent") as mock_agent_cls:
            mock_agent = MagicMock()
            mock_agent.run_conversation.return_value = {"final_response": "ok"}
            mock_agent_cls.return_value = mock_agent
