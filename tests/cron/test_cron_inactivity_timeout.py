...
- Unlimited timeout (KUNMING_CRON_TIMEOUT=0)
- Backward compat: KUNMING_CRON_TIMEOUT env var still works
...
class TestInactivityTimeout:
    ...
    def test_unlimited_timeout(self):
        """KUNMING_CRON_TIMEOUT=0 means no timeout at all."""
        agent = FakeAgent(idle_seconds=0.0)
    ...
    def test_timeout_env_var_parsing(self, monkeypatch):
        """KUNMING_CRON_TIMEOUT env var is respected."""
        monkeypatch.setenv("KUNMING_CRON_TIMEOUT", "1200")
    ...
    def test_timeout_zero_means_unlimited(self, monkeypatch):
        """KUNMING_CRON_TIMEOUT=0 yields None (unlimited)."""
        monkeypatch.setenv("KUNMING_CRON_TIMEOUT", "0")
        _cron_timeout = float(os.getenv("KUNMING_CRON_TIMEOUT", 600))
        _cron_inactivity_limit = _cron_timeout if _cron_timeout > 0 else None
...
class TestSysPathOrdering:
    ...
    def test_KUNMING_time_importable(self):
        """KUNMING_time should be importable when cron.scheduler loads."""
        # This import would fail if sys.path.insert comes after the import
        from cron.scheduler import _KUNMING_now
        assert callable(_KUNMING_now)

    def test_kunming_constants_importable(self):
        """kunming_constants should be importable from cron context."""
        from kunming_constants import get_kunming_home
        assert callable(get_kunming_home)
