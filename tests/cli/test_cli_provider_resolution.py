...
from kunming_cli.auth import AuthError
from kunming_cli import main as kunming_main
...
def test_kunming_cli_init_does_not_eagerly_resolve_runtime_provider(monkeypatch):
    cli = _import_cli()
    calls = {"count": 0}

    def _unexpected_runtime_resolve(**kwargs):
        calls["count"] += 1
        raise AssertionError("resolve_runtime_provider should not be called in KunmingCLI.__init__")

    monkeypatch.setattr("kunming_cli.runtime_provider.resolve_runtime_provider", _unexpected_runtime_resolve)
    monkeypatch.setattr("kunming_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))

    shell = cli.KunmingCLI(model="gpt-5", compact=True, max_turns=1)
...
def test_runtime_resolution_failure_is_not_sticky(monkeypatch):
    ...
    monkeypatch.setattr("kunming_cli.runtime_provider.resolve_runtime_provider", _runtime_resolve)
    monkeypatch.setattr("kunming_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
...
def test_cli_turn_routing_uses_cheap_model_when_simple(monkeypatch):
    ...
    shell = cli.KunmingCLI(model="anthropic/claude-sonnet-4", compact=True, max_turns=1)
    shell.provider = "openrouter"
    shell.api_mode = "chat_completions"
    shell.base_url = "https://openrouter.ai/api/v1"
...
def test_cli_prefers_config_provider_over_stale_env_override(monkeypatch):
    ...
    monkeypatch.setenv("KUNMING_INFERENCE_PROVIDER", "openrouter")
    ...
    shell = cli.KunmingCLI(model="fireworks/minimax-m2p5", compact=True, max_turns=1)
...
def test_codex_provider_replaces_incompatible_default_model(monkeypatch):
    ...
    monkeypatch.setattr("kunming_cli.runtime_provider.format_runtime_provider_error", lambda exc: str(exc))
    monkeypatch.setattr(
        "kunming_cli.codex_models.get_codex_model_ids",
        lambda access_token=None: ["gpt-5.2-codex", "gpt-5.1-codex-mini"],
    )

    shell = cli.KunmingCLI(compact=True, max_turns=1)
...
def test_model_flow_nous_prints_subscription_guidance_without_mutating_explicit_tts(monkeypatch, capsys):
    monkeypatch.setenv("KUNMING_ENABLE_NOUS_MANAGED_TOOLS", "1")
    config = {
    ...
    monkeypatch.setattr(
        "kunming_cli.auth.get_provider_auth_state",
        lambda provider: {"access_token": "nous-token"},
    )
    monkeypatch.setattr(
        "kunming_cli.auth.resolve_nous_runtime_credentials",
        lambda *args, **kwargs: {
    ...
    monkeypatch.setattr(
        "kunming_cli.auth.fetch_nous_models",
        lambda *args, **kwargs: ["claude-opus-4-6"],
    )
    monkeypatch.setattr("kunming_cli.auth._prompt_model_selection", lambda model_ids, current_model="", pricing=None, **kw: "claude-opus-4-6")
    monkeypatch.setattr("kunming_cli.auth._save_model_choice", lambda model: None)
    monkeypatch.setattr("kunming_cli.auth._update_config_for_provider", lambda provider, url: None)
    monkeypatch.setattr(
        "kunming_cli.nous_subscription.get_nous_subscription_explainer_lines",
        lambda: ["Nous subscription enables managed web tools."],
    )

    kunming_main._model_flow_nous(config, current_model="claude-opus-4-6")
...
def test_codex_provider_preserves_explicit_codex_model(monkeypatch):
    ...
    shell = cli.KunmingCLI(model="gpt-5.1-codex-mini", compact=True, max_turns=1)
...
def test_codex_provider_strips_provider_prefix_from_model(monkeypatch):
    ...
    shell = cli.KunmingCLI(model="openai/gpt-5.3-codex", compact=True, max_turns=1)
...
def test_cmd_model_falls_back_to_auto_on_invalid_provider(monkeypatch, capsys):
    monkeypatch.setattr(
        "kunming_cli.config.load_config",
        lambda: {"model": {"default": "gpt-5", "provider": "invalid-provider"}},
    )
    monkeypatch.setattr("kunming_cli.config.save_config", lambda cfg: None)
    monkeypatch.setattr("kunming_cli.config.get_env_value", lambda key: "")
    monkeypatch.setattr("kunming_cli.config.save_env_value", lambda key, value: None)
    ...
    monkeypatch.setattr("kunming_cli.auth.resolve_provider", _resolve_provider)
    monkeypatch.setattr(kunming_main, "_prompt_provider_choice", lambda choices, **kwargs: len(choices) - 1)
    monkeypatch.setattr("sys.stdin", type("FakeTTY", (), {"isatty": lambda self: True})())

    kunming_main.cmd_model(SimpleNamespace())
...
def test_model_flow_custom_saves_verified_v1_base_url(monkeypatch, capsys):
    monkeypatch.setattr(
        "kunming_cli.config.get_env_value",
        lambda key: "" if key in {"OPENAI_BASE_URL", "OPENAI_API_KEY"} else "",
    )
    saved_env = {}
    monkeypatch.setattr("kunming_cli.config.save_env_value", lambda key, value: saved_env.__setitem__(key, value))
    monkeypatch.setattr("kunming_cli.auth._save_model_choice", lambda model: saved_env.__setitem__("MODEL", model))
    monkeypatch.setattr("kunming_cli.auth.deactivate_provider", lambda: None)
    monkeypatch.setattr("kunming_cli.main._save_custom_provider", lambda *args, **kwargs: None)
    monkeypatch.setattr(
        "kunming_cli.models.probe_api_models",
    ...
    kunming_main._model_flow_custom({})
    output = capsys.readouterr().out
...
def test_cmd_model_forwards_nous_login_tls_options(monkeypatch):
    monkeypatch.setattr(kunming_main, "_require_tty", lambda *a: None)
    monkeypatch.setattr(
        "kunming_cli.config.load_config",
    ...
    monkeypatch.setattr("kunming_cli.config.get_env_value", lambda key: "")
    monkeypatch.setattr("kunming_cli.config.save_env_value", lambda key, value: None)
    monkeypatch.setattr("kunming_cli.auth.resolve_provider", lambda requested, **kwargs: "nous")
    monkeypatch.setattr("kunming_cli.auth.get_provider_auth_state", lambda provider_id: None)
    monkeypatch.setattr(kunming_main, "_prompt_provider_choice", lambda choices, **kwargs: 0)
    ...
    monkeypatch.setattr("kunming_cli.auth._login_nous", _fake_login)

    kunming_main.cmd_model(
        SimpleNamespace(
            portal_url="https://portal.kunming.dev",
            inference_url="https://inference.kunming.dev/v1",
            client_id="kunming-local",
