...
def session_db(tmp_path):
    ...
    os.environ["KUNMING_HOME"] = str(tmp_path / ".kunming")
    os.makedirs(tmp_path / ".kunming", exist_ok=True)
    from kunming_state import SessionDB
    db = SessionDB(db_path=tmp_path / ".kunming" / "test_sessions.db")
    yield db
    db.close()
...
def cli_instance(tmp_path, session_db):
    """Create a minimal KunmingCLI-like object for testing _handle_branch_command."""
...
class TestBranchCommandCLI:
    ...
    def test_branch_creates_new_session(self, cli_instance, session_db):
        ...
        KunmingCLI._handle_branch_command(cli_instance, "/branch")
    ...
    def test_branch_with_custom_name(self, cli_instance, session_db):
        ...
        KunmingCLI._handle_branch_command(cli_instance, "/branch refactor approach")
    ...
    def test_fork_alias(self):
        ...
        from kunming_cli.commands import resolve_command
...
class TestBranchCommandDef:
    ...
    def test_branch_in_registry(self):
        ...
        from kunming_cli.commands import COMMAND_REGISTRY
