...
def _isolate(tmp_path, monkeypatch):
    """Isolate KUNMING_HOME so tests don't touch real config."""
...
def cli_obj(_isolate):
    """Create a minimal KunmingCLI instance for banner testing."""
    with patch("cli.load_cli_config", return_value={
        ...
        obj = KunmingCLI.__new__(KunmingCLI)
        obj.model = "test-model"
        obj.enabled_toolsets = ["kunming-core"]
        obj.compact = False
