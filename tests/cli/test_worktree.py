...
class TestWorktreeCreation:
    ...
    def test_creates_worktree(self, git_repo):
        ...
        assert info["branch"].startswith("kunming/kunming-")
...
class TestOrphanedBranchPruning:
    """Test cleanup of orphaned kunming/* and pr-* branches."""

    def test_prunes_orphaned_KUNMING_branch(self, git_repo):
        """kunming/kunming-* branches with no worktree should be deleted."""
        # Create a branch that looks like a worktree branch but has no worktree
        subprocess.run(
            ["git", "branch", "kunming/kunming-deadbeef", "HEAD"],
            cwd=str(git_repo), capture_output=True,
        )

        # Verify it exists
        result = subprocess.run(
            ["git", "branch", "--list", "kunming/kunming-deadbeef"],
            capture_output=True, text=True, cwd=str(git_repo),
        )
        assert "kunming/kunming-deadbeef" in result.stdout
        ...
        assert "kunming/kunming-deadbeef" in orphaned
        ...
        assert "kunming/kunming-deadbeef" not in result.stdout
