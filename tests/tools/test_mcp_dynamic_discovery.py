...
class TestRegisterServerTools:
    ...
    def test_injects_kunming_toolsets(self, mock_registry, mock_toolsets):
        """Tools are injected into kunming-* toolsets but not custom ones."""
        ...
        # Injected into kunming-* toolsets
        assert "mcp_my_srv_my_tool" in mock_toolsets["kunming-cli"]["tools"]
        assert "mcp_my_srv_my_tool" in mock_toolsets["kunming-telegram"]["tools"]
        # NOT into non-km toolsets
...
class TestRefreshTools:
    ...
    async def test_nuke_and_repave(self, mock_registry, mock_toolsets):
        ...
        mock_toolsets["kunming-cli"]["tools"].append("mcp_live_srv_old_tool")
        ...
        assert "mcp_live_srv_old_tool" not in mock_toolsets["kunming-cli"]["tools"]

        # New tool registered
        assert "mcp_live_srv_new_tool" in mock_registry.get_all_tool_names()
        assert "mcp_live_srv_new_tool" in mock_toolsets["kunming-cli"]["tools"]
