...
class TestLoadMCPConfig:
    def test_no_config_returns_empty(self):
        ...
        with patch("kunming_cli.config.load_config", return_value={"model": "test"}):
    ...
    def test_valid_config_parsed(self):
        ...
        with patch("kunming_cli.config.load_config", return_value={"mcp_servers": servers}):
            from tools.mcp_tool import _load_mcp_config
            result = _load_mcp_config()
    ...
    def test_mcp_servers_not_dict_returns_empty(self):
        ...
        with patch("kunming_cli.config.load_config", return_value={"mcp_servers": "invalid"}):
            from tools.mcp_tool import _load_mcp_config
            result = _load_mcp_config()
            assert result == {}
...
class TestSchemaConversion:
    def test_converts_mcp_tool_to_kunming_schema(self):
...
class TestToolsetInjection:
    def test_mcp_tools_added_to_all_kunming_toolsets(self):
        """Discovered MCP tools are dynamically injected into all kunming-* toolsets."""
        from tools.mcp_tool import MCPServerTask
        ...
        fake_toolsets = {
            "kunming-cli": {"tools": ["terminal"], "description": "CLI", "includes": []},
            "kunming-telegram": {"tools": ["terminal"], "description": "TG", "includes": []},
            "kunming-gateway": {"tools": [], "description": "GW", "includes": []},
            "non-kunming": {"tools": [], "description": "other", "includes": []},
        }
        ...
        # All kunming-* toolsets get injection
        assert "mcp_fs_list_files" in fake_toolsets["kunming-cli"]["tools"]
        assert "mcp_fs_list_files" in fake_toolsets["kunming-telegram"]["tools"]
        assert "mcp_fs_list_files" in fake_toolsets["kunming-gateway"]["tools"]
        # Non-kunming toolset does not
        assert "mcp_fs_list_files" not in fake_toolsets["non-kunming"]["tools"]
        # Original tools preserved
        assert "terminal" in fake_toolsets["kunming-cli"]["tools"]
    ...
    def test_server_connection_failure_skipped(self):
        ...
        fake_toolsets = {
            "kunming-cli": {"tools": [], "description": "CLI", "includes": []},
        }
