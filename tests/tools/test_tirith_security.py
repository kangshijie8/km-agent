...
class TestEnsureInstalled:
    ...
    def test_not_found_returns_none(self, mock_cfg):
        ...
        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._KUNMING_bin_dir", return_value="/nonexistent"), \
...
class TestDiskFailureMarker:
    ...
    def test_install_failed_recovers_from_KUNMING_bin(self):
        """After _INSTALL_FAILED, manual install in KUNMING_HOME/bin is picked up."""
        from tools.tirith_security import _resolve_tirith_path, _INSTALL_FAILED
        import tempfile
        tmpdir = tempfile.mkdtemp()
        KUNMING_bin = os.path.join(tmpdir, "tirith")
        # Create a fake executable
        with open(KUNMING_bin, "w") as f:
            f.write("#!/bin/sh\n")
        os.chmod(KUNMING_bin, 0o755)

        _tirith_mod._resolved_path = _INSTALL_FAILED

        with patch("tools.tirith_security.shutil.which", return_value=None), \
             patch("tools.tirith_security._KUNMING_bin_dir", return_value=tmpdir), \
             patch("tools.tirith_security._clear_install_failed") as mock_clear:
            result = _resolve_tirith_path("tirith")
            assert result == KUNMING_bin
            assert _tirith_mod._resolved_path == KUNMING_bin
            mock_clear.assert_called_once()
...
# ---------------------------------------------------------------------------
# KUNMING_HOME isolation
# ---------------------------------------------------------------------------

class TestKunmingHomeIsolation:
    def test_KUNMING_bin_dir_respects_KUNMING_home(self):
        """_KUNMING_bin_dir must use KUNMING_HOME, not hardcoded ~/.kunming."""
        from tools.tirith_security import _KUNMING_bin_dir
        import tempfile
        tmpdir = tempfile.mkdtemp()
        with patch.dict(os.environ, {"KUNMING_HOME": tmpdir}):
            result = _KUNMING_bin_dir()
    ...
    def test_get_kunming_home_fallback(self):
        ...
        from tools.tirith_security import _get_kunming_home
        with patch.dict(os.environ, {}, clear=True):
            # Remove KUNMING_HOME entirely
            os.environ.pop("KUNMING_HOME", None)
            result = _get_kunming_home()
