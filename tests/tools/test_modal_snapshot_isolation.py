...
def _restore_tool_modules():
    original_KUNMING_home = os.environ.get("KUNMING_HOME")
    original_modules = {
        name: module
        ...
        or name == "kunming_cli"
        or name.startswith("kunming_cli.")
        or name == "modal"
    ...
    try:
        ...
        if original_KUNMING_home is None:
            os.environ.pop("KUNMING_HOME", None)
        else:
            os.environ["KUNMING_HOME"] = original_KUNMING_home
        _reset_modules(("tools", "kunming_cli", "modal"))
...
def _install_modal_test_modules(
    ...
    *,
    ...
    kunming_cli = types.ModuleType("kunming_cli")
    kunming_cli.__path__ = []  # type: ignore[attr-defined]
    sys.modules["kunming_cli"] = kunming_cli
    KUNMING_home = tmp_path / "kunming-home"
    os.environ["KUNMING_HOME"] = str(KUNMING_home)
    sys.modules["kunming_cli.config"] = types.SimpleNamespace(
        get_kunming_home=lambda: KUNMING_home,
    )
    ...
    async def _lookup_aio(_name: str, create_if_missing: bool = False):
        return types.SimpleNamespace(name="kunming-agent", create_if_missing=create_if_missing)
    ...
    return {
        "snapshot_store": KUNMING_home / "modal_snapshots.json",
