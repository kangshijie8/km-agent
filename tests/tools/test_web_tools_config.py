...
class TestFirecrawlClientConfig:
    ...
    def setup_method(self):
        ...
        for key in (
            "KUNMING_ENABLE_NOUS_MANAGED_TOOLS",
            "FIRECRAWL_API_KEY",
            "FIRECRAWL_API_URL",
        ...
        os.environ["KUNMING_ENABLE_NOUS_MANAGED_TOOLS"] = "1"
    ...
    def test_tool_gateway_domain_builds_firecrawl_gateway_origin(self):
        ...
        with patch.dict(os.environ, {"TOOL_GATEWAY_DOMAIN": "kunming.dev"}):
            with patch("tools.web_tools._read_nous_access_token", return_value="nous-token"):
                with patch("tools.web_tools.Firecrawl") as mock_fc:
                    ...
                    mock_fc.assert_called_once_with(
                        api_key="nous-token",
                        api_url="https://firecrawl-gateway.kunming.dev",
                    )
                    assert result is mock_fc.return_value
    ...
    def test_tool_gateway_scheme_can_switch_derived_gateway_origin_to_http(self):
        ...
        with patch.dict(os.environ, {
            "TOOL_GATEWAY_DOMAIN": "kunming.dev",
            ...
            with patch("tools.web_tools._read_nous_access_token", return_value="nous-token"):
                with patch("tools.web_tools.Firecrawl") as mock_fc:
                    ...
                    mock_fc.assert_called_once_with(
                        api_key="nous-token",
                        api_url="http://firecrawl-gateway.kunming.dev",
                    )
                    assert result is mock_fc.return_value
    ...
    def test_nous_auth_token_respects_kunming_home_override(self, tmp_path):
        """Auth lookup should read from KUNMING_HOME/auth.json, not ~/.kunming/auth.json."""
        real_home = tmp_path / "real-home"
        (real_home / ".kunming").mkdir(parents=True)

        kunming_home = tmp_path / "kunming-home"
        kunming_home.mkdir()
...
class TestBackendSelection:
    ...
    ``km tools``.  Falls back to key-based detection for legacy/manual
    setups.
    """
    ...
    def setup_method(self):
        ...
        for key in self._ENV_KEYS:
            if key != "KUNMING_ENABLE_NOUS_MANAGED_TOOLS":
                os.environ.pop(key, None)
