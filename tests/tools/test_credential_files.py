...
class TestRegisterCredentialFiles:
    def test_dict_with_path_key(self, tmp_path):
        ...
        KUNMING_home.mkdir()
        (KUNMING_home / "token.json").write_text("{}")

        with patch.dict(os.environ, {"KUNMING_HOME": str(KUNMING_home)}):
            missing = register_credential_files([{"path": "token.json"}])

        assert missing == []
        mounts = get_credential_file_mounts()
        assert len(mounts) == 1
        assert mounts[0]["host_path"] == str(KUNMING_home / "token.json")
        assert mounts[0]["container_path"] == "/root/.kunming/token.json"

    def test_dict_with_name_key_fallback(self, tmp_path):
        """Skills use 'name' instead of 'path' — both should work."""
        KUNMING_home = tmp_path / ".kunming"
        KUNMING_home.mkdir()
        (KUNMING_home / "google_token.json").write_text("{}")
    ...
    def test_string_entry(self, tmp_path):
        KUNMING_home = tmp_path / ".kunming"
        KUNMING_home.mkdir()
        (KUNMING_home / "secret.key").write_text("key")
    ...
    def test_path_takes_precedence_over_name(self, tmp_path):
        ...
        KUNMING_home = tmp_path / ".kunming"
        KUNMING_home.mkdir()
        (KUNMING_home / "real.json").write_text("{}")
...
class TestSkillsDirectoryMount:
    def test_returns_mount_when_skills_dir_exists(self, tmp_path):
        ...
        assert mounts[0]["container_path"] == "/root/.kunming/skills"
    ...
    def test_custom_container_base(self, tmp_path):
        KUNMING_home = tmp_path / ".kunming"
        (KUNMING_home / "skills").mkdir(parents=True)

        with patch.dict(os.environ, {"KUNMING_HOME": str(KUNMING_home)}):
            mounts = get_skills_directory_mount(container_base="/home/user/.kunming")
...
class TestIterSkillsFiles:
    ...
    def test_returns_files_skipping_symlinks(self, tmp_path):
        ...
        assert "/root/.kunming/skills/cat/myskill/SKILL.md" in paths
        assert "/root/.kunming/skills/cat/myskill/scripts/run.sh" in paths
...
class TestPathTraversalSecurity:
    ...
    def test_dotdot_traversal_rejected(self, tmp_path, monkeypatch):
        """'../sensitive' must not escape KUNMING_HOME."""
        monkeypatch.setenv("KUNMING_HOME", str(tmp_path / ".kunming"))
        (tmp_path / ".kunming").mkdir()

        # Create a sensitive file one level above KUNMING_home
    ...
    def test_deep_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a fake sensitive file outside KUNMING_home
    ...
    def test_legitimate_file_still_works(self, tmp_path, monkeypatch):
        """Normal files inside KUNMING_HOME must still be registered."""
        KUNMING_home = tmp_path / ".kunming"
        KUNMING_home.mkdir()
        monkeypatch.setenv("KUNMING_HOME", str(KUNMING_home))
        (KUNMING_home / "token.json").write_text('{"token": "abc"}')
    ...
    def test_nested_subdir_inside_KUNMING_home_allowed(self, tmp_path, monkeypatch):
        """Files in subdirectories of KUNMING_HOME must be allowed."""
        KUNMING_home = tmp_path / ".kunming"
        KUNMING_home.mkdir()
        subdir = KUNMING_home / "creds"
        subdir.mkdir()
    ...
    def test_symlink_traversal_rejected(self, tmp_path, monkeypatch):
        ...
        # Create a sensitive file outside KUNMING_home
        sensitive = tmp_path / "sensitive.json"
        sensitive.write_text('{"secret": "value"}')

        # Create a symlink inside KUNMING_home pointing outside
        symlink = KUNMING_home / "evil_link.json"
        try:
        ...
        # The resolved path escapes KUNMING_HOME — must be rejected
        assert result is False
        assert get_credential_file_mounts() == []
...
class TestConfigPathTraversal:
    ...
    def _write_config(self, KUNMING_home: Path, cred_files: list):
        import yaml
    ...
    def test_config_traversal_rejected(self, tmp_path, monkeypatch):
        """'../secret' in config.yaml must not escape KUNMING_HOME."""
        KUNMING_home = tmp_path / ".kunming"
        KUNMING_home.mkdir()
        monkeypatch.setenv("KUNMING_HOME", str(KUNMING_home))
        ...
        sensitive.write_text("{}")
        self._write_config(KUNMING_home, ["../secret.json"])
    ...
    def test_config_absolute_path_rejected(self, tmp_path, monkeypatch):
        ...
        sensitive.write_text("{}")
        self._write_config(KUNMING_home, [str(sensitive)])

        mounts = get_credential_file_mounts()
        assert mounts == []

    def test_config_legitimate_file_works(self, tmp_path, monkeypatch):
        """Normal files inside KUNMING_HOME via config must still mount."""
        KUNMING_home = tmp_path / ".kunming"
        KUNMING_home.mkdir()
        monkeypatch.setenv("KUNMING_HOME", str(KUNMING_home))

        (KUNMING_home / "oauth.json").write_text("{}")
        self._write_config(KUNMING_home, ["oauth.json"])
...
class TestCacheDirectoryMounts:
    ...
    def test_returns_existing_cache_dirs(self, tmp_path, monkeypatch):
        ...
        KUNMING_home = tmp_path / ".kunming"
        KUNMING_home.mkdir()
        (KUNMING_home / "cache" / "documents").mkdir(parents=True)
        (KUNMING_home / "cache" / "audio").mkdir(parents=True)
        monkeypatch.setenv("KUNMING_HOME", str(KUNMING_home))

        mounts = get_cache_directory_mounts()
        paths = {m["container_path"] for m in mounts}
        assert "/root/.kunming/cache/documents" in paths
        assert "/root/.kunming/cache/audio" in paths
    ...
    def test_skips_nonexistent_dirs(self, tmp_path, monkeypatch):
        ...
        mounts = get_cache_directory_mounts()
        assert len(mounts) == 1
        assert mounts[0]["container_path"] == "/root/.kunming/cache/documents"
    ...
    def test_legacy_dir_names_resolved(self, tmp_path, monkeypatch):
        ...
        KUNMING_home = tmp_path / ".kunming"
        KUNMING_home.mkdir()
        # Use legacy dir name — get_kunming_dir prefers old if it exists
        (KUNMING_home / "document_cache").mkdir()
        (KUNMING_home / "image_cache").mkdir()
        monkeypatch.setenv("KUNMING_HOME", str(KUNMING_home))

        mounts = get_cache_directory_mounts()
        host_paths = {m["host_path"] for m in mounts}
        assert str(KUNMING_home / "document_cache") in host_paths
        assert str(KUNMING_home / "image_cache") in host_paths
        # Container paths always use the new layout
        container_paths = {m["container_path"] for m in mounts}
        assert "/root/.kunming/cache/documents" in container_paths
        assert "/root/.kunming/cache/images" in container_paths

    def test_empty_KUNMING_home(self, tmp_path, monkeypatch):
...
class TestIterCacheFiles:
    ...
    def test_enumerates_files(self, tmp_path, monkeypatch):
        ...
        KUNMING_home = tmp_path / ".kunming"
        doc_dir = KUNMING_home / "cache" / "documents"
    ...
    def test_nested_files(self, tmp_path, monkeypatch):
        ...
        KUNMING_home = tmp_path / ".kunming"
        ss_dir = KUNMING_home / "cache" / "screenshots"
