...
class TestApprovalModeParsing:
    def test_unquoted_yaml_off_boolean_false_maps_to_off(self):
        with mock_patch("kunming_cli.config.load_config", return_value={"approvals": {"mode": False}}):
            assert _get_approval_mode() == "off"

    def test_string_off_still_maps_to_off(self):
        with mock_patch("kunming_cli.config.load_config", return_value={"approvals": {"mode": "off"}}):
            assert _get_approval_mode() == "off"
...
class TestSessionKeyContext:
    def test_context_session_key_overrides_process_env(self):
        ...
        try:
            with mock_patch.dict("os.environ", {"KUNMING_SESSION_KEY": "bob"}, clear=False):
    ...
    def test_context_keeps_pending_approval_attached_to_originating_session(self):
        ...
        def worker_alice():
            ...
            try:
                ...
                os.environ["KUNMING_SESSION_KEY"] = "alice"
                alice_ready.set()
                bob_ready.wait()
        ...
        def worker_bob():
            ...
            try:
                os.environ["KUNMING_SESSION_KEY"] = "bob"
                bob_ready.set()
            finally:
                approval_module.reset_current_session_key(token)
...
class TestTeePattern:
    ...
    def test_tee_KUNMING_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.kunming/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_custom_KUNMING_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $KUNMING_HOME/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_quoted_custom_KUNMING_home_env(self):
        dangerous, key, desc = detect_dangerous_command('echo x | tee "$KUNMING_HOME/.env"')
        assert dangerous is True
        assert key is not None
...
class TestSensitiveRedirectPattern:
    ...
    def test_redirect_to_custom_KUNMING_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x > $KUNMING_HOME/.env")
        assert dangerous is True
        assert key is not None
...
class TestGatewayProtection:
    ...
    def test_gateway_run_with_disown_detected(self):
        cmd = "kill 1605 && cd ~/.kunming/kunming-agent && source venv/bin/activate && python -m kunming_cli.main gateway run --replace &disown; echo done"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "systemctl" in desc

    def test_gateway_run_with_ampersand_detected(self):
        cmd = "python -m kunming_cli.main gateway run --replace &"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_nohup_detected(self):
        cmd = "nohup python -m kunming_cli.main gateway run --replace"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True

    def test_gateway_run_with_setsid_detected(self):
        cmd = "kunming_cli.main gateway run --replace &disown"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
    ...
    def test_systemctl_restart_not_flagged(self):
        ...
        cmd = "systemctl --user restart kunming-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is False

    def test_pkill_KUNMING_detected(self):
        """pkill targeting kunming/gateway processes must be caught."""
        cmd = 'pkill -f "cli.py --gateway"'
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc

    def test_killall_KUNMING_detected(self):
        cmd = "killall kunming"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is True
        assert "self-termination" in desc
...
class TestNormalizationBypass:
    ...
    def test_fullwidth_unicode_chmod(self):
        """Fullwidth 'ｃｈｍｏ777' must be caught."""
