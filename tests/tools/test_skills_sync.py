...
class TestGetBundledDir:
    def test_env_var_override(self, tmp_path, monkeypatch):
        """KUNMING_BUNDLED_SKILLS env var overrides the default path resolution."""
        custom_dir = tmp_path / "custom_skills"
        custom_dir.mkdir()
        monkeypatch.setenv("KUNMING_BUNDLED_SKILLS", str(custom_dir))
    ...
    def test_default_without_env_var(self, monkeypatch):
        ...
        monkeypatch.delenv("KUNMING_BUNDLED_SKILLS", raising=False)
        result = _get_bundled_dir()
        assert result.name == "skills"

    def test_env_var_empty_string_ignored(self, monkeypatch):
        """Empty KUNMING_BUNDLED_SKILLS should fall back to default."""
        monkeypatch.setenv("KUNMING_BUNDLED_SKILLS", "")
        result = _get_bundled_dir()
        assert result.name == "skills"
