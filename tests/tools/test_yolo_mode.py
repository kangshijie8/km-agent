"""Tests for --yolo (KUNMING_YOLO_MODE) approval bypass."""
...
class TestYoloMode:
    """When KUNMING_YOLO_MODE is set, all dangerous commands are auto-approved."""
    ...
    def test_dangerous_command_blocked_normally(self, monkeypatch):
        ...
        monkeypatch.setenv("KUNMING_SESSION_KEY", "test-session")
        monkeypatch.delenv("KUNMING_YOLO_MODE", raising=False)
        monkeypatch.delenv("KUNMING_GATEWAY_SESSION", raising=False)
        monkeypatch.delenv("KUNMING_EXEC_ASK", raising=False)
    ...
    def test_dangerous_command_approved_in_yolo_mode(self, monkeypatch):
        """With KUNMING_YOLO_MODE, dangerous commands are auto-approved."""
        monkeypatch.setenv("KUNMING_YOLO_MODE", "1")
        monkeypatch.setenv("KUNMING_INTERACTIVE", "1")
        monkeypatch.setenv("KUNMING_SESSION_KEY", "test-session")
    ...
    def test_yolo_mode_not_set_by_default(self):
        """KUNMING_YOLO_MODE should not be set by default."""
        # Clean env check — if it happens to be set in test env, that's fine,
        # we just verify the mechanism exists
        assert os.getenv("KUNMING_YOLO_MODE") is None or True  # no-op, documents intent

    def test_yolo_mode_empty_string_does_not_bypass(self, monkeypatch):
        """Empty string for KUNMING_YOLO_MODE should not trigger bypass."""
        monkeypatch.setenv("KUNMING_YOLO_MODE", "")
        monkeypatch.setenv("KUNMING_INTERACTIVE", "1")
        monkeypatch.setenv("KUNMING_SESSION_KEY", "test-session")

        # Empty string is falsy in Python, so getenv("KUNMING_YOLO_MODE") returns ""
