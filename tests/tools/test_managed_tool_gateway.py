...
def test_resolve_managed_tool_gateway_derives_vendor_origin_from_shared_domain():
    with patch.dict(
        os.environ,
        {
            "KUNMING_ENABLE_NOUS_MANAGED_TOOLS": "1",
    ...
    assert result.gateway_origin == "https://firecrawl-gateway.kunming.dev"
...
def test_resolve_managed_tool_gateway_is_disabled_without_feature_flag():
    with patch.dict(os.environ, {"TOOL_GATEWAY_DOMAIN": "kunming.dev"}, clear=False):
        result = resolve_managed_tool_gateway(
            "firecrawl",
            token_reader=lambda: "nous-token",
        )
...
def test_read_nous_access_token_refreshes_expiring_cached_token(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "kunming_cli.auth.resolve_nous_access_token",
