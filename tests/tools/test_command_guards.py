...
def _clean_state():
    ...
    key = os.getenv("KUNMING_SESSION_KEY", "default")
    clear_session(key)
    approval_module._permanent_approved.clear()
    saved = {}
    for k in ("KUNMING_INTERACTIVE", "KUNMING_GATEWAY_SESSION", "KUNMING_EXEC_ASK", "KUNMING_YOLO_MODE"):
        if k in os.environ:
...
class TestTirithBlock:
    ...
    def test_tirith_block_gateway_returns_approval_required(self, mock_tirith):
        ...
        os.environ["KUNMING_GATEWAY_SESSION"] = "1"
...
class TestTirithWarnSafe:
    ...
    def test_warn_session_approved(self, mock_tirith):
        os.environ["KUNMING_INTERACTIVE"] = "1"
        session_key = os.getenv("KUNMING_SESSION_KEY", "default")
    ...
    def test_warn_non_interactive_auto_allow(self, mock_tirith):
        # No KUNMING_INTERACTIVE or KUNMING_GATEWAY_SESSION set
        result = check_all_command_guards("curl https://bit.ly/abc", "local")
        assert result["approved"] is True
