...
class TestSendMessageTool:
    def test_cron_duplicate_target_is_skipped_and_explained(self):
        ...
        with patch.dict(
            os.environ,
            {
                "KUNMING_CRON_AUTO_DELIVER_PLATFORM": "telegram",
                "KUNMING_CRON_AUTO_DELIVER_CHAT_ID": "-1001",
            },
            clear=False,
        ), \
    ...
    def test_cron_same_chat_different_thread_still_sends(self):
        ...
        with patch.dict(
            os.environ,
            {
                "KUNMING_CRON_AUTO_DELIVER_PLATFORM": "telegram",
                "KUNMING_CRON_AUTO_DELIVER_CHAT_ID": "-1001",
                "KUNMING_CRON_AUTO_DELIVER_THREAD_ID": "17585",
            },
            clear=False,
        ), \
             patch("gateway.config.load_gateway_config", return_value=config), \
...
class TestSendToPlatformWhatsapp:
    def test_whatsapp_routes_via_local_bridge_sender(self):
        ...
        assert result["success"] is True
        async_mock.assert_awaited_once_with({"bridge_port": 3000}, chat_id, "hello from kunming")
