...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "kunming"
        assert config.workspace_id == "kunming"
...
class TestFromGlobalConfig:
    ...
    def test_context_tokens_host_block_wins(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "contextTokens": 1000,
            "hosts": {"kunming": {"contextTokens": 2000}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_recall_mode_from_config(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "recallMode": "tools",
            "hosts": {"kunming": {"recallMode": "context"}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_base_url_not_read_from_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"kunming": {"baseUrl": "http://host-block:9001"}},
        }))
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="kunming-agent"
        ):
            result = config.resolve_session_name("/home/user/kunming-agent/subdir")
        assert result == "kunming-agent"
...
class TestResolveConfigPath:
    def test_prefers_KUNMING_home_when_exists(self, tmp_path):
        KUNMING_home = tmp_path / "kunming"
        KUNMING_home.mkdir()
        local_cfg = KUNMING_home / "honcho.json"
    ...
    def test_falls_back_to_global_when_no_local(self, tmp_path):
        KUNMING_home = tmp_path / "kunming"
        KUNMING_home.mkdir()
        # No honcho.json in KUNMING_HOME also isolate ~/.kunming so
    ...
    def test_falls_back_to_global_without_KUNMING_home_env(self, tmp_path):
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
...
class TestResolveActiveHost:
    def test_default_returns_kunming(self):
        with patch.dict(os.environ, {}, clear=True):
            os.environ.pop("KUNMING_HONCHO_HOST", None)
            os.environ.pop("KUNMING_HOME", None)
            assert resolve_active_host() == "kunming"

    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"KUNMING_HONCHO_HOST": "kunming.coder"}):
            assert resolve_active_host() == "kunming.coder"

    def test_profile_name_derives_host(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("KUNMING_HONCHO_HOST", None)
            with patch("kunming_cli.profiles.get_active_profile_name", return_value="coder"):
                assert resolve_active_host() == "kunming.coder"

    def test_default_profile_returns_kunming(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("KUNMING_HONCHO_HOST", None)
            with patch("kunming_cli.profiles.get_active_profile_name", return_value="default"):
                assert resolve_active_host() == "kunming"

    def test_custom_profile_returns_kunming(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("KUNMING_HONCHO_HOST", None)
            with patch("kunming_cli.profiles.get_active_profile_name", return_value="custom"):
                assert resolve_active_host() == "kunming"
    ...
    def test_profiles_import_failure_falls_back(self):
        ...
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("KUNMING_HONCHO_HOST", None)
            # Temporarily remove kunming_cli.profiles to simulate import failure
            saved = sys.modules.get("kunming_cli.profiles")
            sys.modules["kunming_cli.profiles"] = None  # type: ignore
            try:
                assert resolve_active_host() == "kunming"
            finally:
                if saved is not None:
                    sys.modules["kunming_cli.profiles"] = saved
                else:
                    sys.modules.pop("kunming_cli.profiles", None)


class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="kunming.coder")
        assert config.host == "kunming.coder"
        assert config.workspace_id == "kunming"  # shared workspace
        assert config.ai_peer == "kunming.coder"

    def test_from_env_default_workspace_preserved_for_default_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="kunming")
        assert config.host == "kunming"
        assert config.workspace_id == "kunming"
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "shared-key",
            "hosts": {
                "kunming": {"aiPeer": "kunming", "peerName": "alice"},
                "kunming.coder": {
                    "aiPeer": "kunming.coder",
        ...
        config = HonchoClientConfig.from_global_config(
            host="kunming.coder", config_path=config_file,
        )
        assert config.host == "kunming.coder"
    ...
    def test_from_global_config_auto_resolves_host(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "kunming.dreamer": {"peerName": "dreamer-user"},
            },
        }))
        with patch("plugins.memory.honcho.client.resolve_active_host", return_value="kunming.dreamer"):
            config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.host == "kunming.dreamer"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"kunming": {"enabled": True, "aiPeer": "kunming"}},
        }))
    ...
    def test_explicit_directional_respected(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"kunming": {"enabled": True, "observationMode": "directional"}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "directional"
