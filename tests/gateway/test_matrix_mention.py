...
class TestIsBotMentioned:
    ...
    def test_full_user_id_in_body(self):
        assert self.adapter._is_bot_mentioned("hey @kunming:example.org help")

    def test_localpart_in_body(self):
        assert self.adapter._is_bot_mentioned("kunming can you help?")

    def test_localpart_case_insensitive(self):
        assert self.adapter._is_bot_mentioned("KUNMING can you help?")

    def test_matrix_pill_in_formatted_body(self):
        html = '<a href="https://matrix.to/#/@kunming:example.org">Kunming</a> help'
        assert self.adapter._is_bot_mentioned("Kunming help", html)
    ...
    def test_partial_localpart_no_match(self):
        # "kunmingbot" should not match word-boundary check for "kunming"
        assert not self.adapter._is_bot_mentioned("kunmingbot is here")
...
class TestStripMention:
    ...
    def test_strip_full_user_id(self):
        result = self.adapter._strip_mention("@kunming:example.org help me")
        assert result == "help me"

    def test_strip_localpart(self):
        result = self.adapter._strip_mention("kunming help me")
        assert result == "help me"

    def test_strip_returns_empty_for_mention_only(self):
        result = self.adapter._strip_mention("@kunming:example.org")
        assert result == ""
...
async def test_require_mention_default_processes_mentioned(monkeypatch):
    ...
    adapter = _make_adapter()
    room = _make_room()
    event = _make_event("@kunming:example.org help me")
...
async def test_require_mention_html_pill(monkeypatch):
    ...
    adapter = _make_adapter()
    room = _make_room()
    formatted = '<a href="https://matrix.to/#/@kunming:example.org">Kunming</a> help'
    event = _make_event("Kunming help", formatted_body=formatted)
...
async def test_bare_mention_passes_empty_string(monkeypatch):
    ...
    adapter = _make_adapter()
    room = _make_room()
    event = _make_event("@kunming:example.org")
