...
being persisted to ~/.kunming/discord_threads.json.
"""
...
class TestDiscordThreadPersistence:
    ...
    def _make_adapter(self, tmp_path):
        """Build a minimal DiscordAdapter with KUNMING_HOME pointed at tmp_path."""
    ...
    def test_missing_KUNMING_home_does_not_crash(self, tmp_path):
        """Load/save tolerate missing directories."""
        fake_home = tmp_path / "nonexistent" / "deep"
        with patch.dict(os.environ, {"KUNMING_HOME": str(fake_home)}):
            from gateway.platforms.discord import DiscordAdapter
