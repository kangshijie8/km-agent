...
class TestSmsToolset:
    def test_KUNMING_sms_toolset_exists(self):
        from toolsets import get_toolset

        ts = get_toolset("kunming-sms")
        assert ts is not None
        assert "tools" in ts

    def test_KUNMING_sms_in_gateway_includes(self):
        from toolsets import get_toolset

        gw = get_toolset("kunming-gateway")
        assert gw is not None
        assert "kunming-sms" in gw["includes"]
