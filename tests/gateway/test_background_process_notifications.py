...
class TestLoadBackgroundNotificationsMode:
    ...
    def test_defaults_to_all(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(gw, "_kunming_home", tmp_path)
        monkeypatch.delenv("KUNMING_BACKGROUND_NOTIFICATIONS", raising=False)
    ...
    def test_env_var_overrides_config(self, monkeypatch, tmp_path):
        ...
        import gateway.run as gw
        monkeypatch.setattr(gw, "_kunming_home", tmp_path)
        monkeypatch.setenv("KUNMING_BACKGROUND_NOTIFICATIONS", "off")
