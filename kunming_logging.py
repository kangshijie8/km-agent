"""Centralized logging setup for Kunming Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.kunming/logs/`` (profile-aware via ``get_kunming_home()``).
...
def setup_logging(
    *,
    kunming_home: Optional[Path] = None,
    ...
    """Configure the Kunming logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    kunming_home
        Override for the Kunming home directory.  Falls back to
        ``get_kunming_home()`` (profile-aware).
    log_level
    ...
    if _logging_initialized and not force:
        home = kunming_home or get_kunming_home()
        return home / "logs"
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_kunming_verbose", False):
                return

    handler = logging.StreamHandler()
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._kunming_verbose = True  # type: ignore[attr-defined]
