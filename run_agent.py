...
_loaded_env_paths = load_kunming_dotenv(kunming_home=_kunming_home, project_env=_project_env)
...
class _SafeWriter:
    ...
    When kunming-agent runs as a systemd service, Docker container, or headless
...
class AIAgent:
    ...
    def __init__(
        ...
        # Kunming is a tool-using client by default.
        ...
        # both live under ~/.kunming/logs/.  Idempotent, so gateway mode
        # (which creates a new AIAgent per message) won't duplicate handlers.
        from kunming_logging import setup_logging, setup_verbose_logging
        setup_logging(kunming_home=_kunming_home)
            ...
            if api_key and base_url:
                ...
                if "openrouter" in effective_base.lower():
                    client_kwargs["default_headers"] = {
                        "HTTP-Referer": "https://kunming-agent.kunming.dev",
                        "X-OpenRouter-Title": "Kunming Agent",
                        "X-OpenRouter-Categories": "productivity,cli-agent",
                    }
                elif "api.githubcopilot.com" in effective_base.lower():
                    from kunming_cli.models import copilot_default_headers
        ...
        if self.save_trajectories and not self.quiet_mode:
            print("[CONFIG] Trajectory saving enabled")
        
        # Show ephemeral system prompt status
        if self.ephemeral_system_prompt and not self.quiet_mode:
            prompt_preview = self.ephemeral_system_prompt[:60] + "..." if len(self.ephemeral_system_prompt) > 60 else self.ephemeral_system_prompt
            print(f"[SECURITY] Ephemeral system prompt: '{prompt_preview}' (not saved to trajectories)")
        ...
        # Session logs go into ~/.kunming/sessions/ alongside gateway sessions
        kunming_home = get_kunming_home()
        self.logs_dir = kunming_home / "sessions"
        ...
        if self._session_db:
            try:
                self._session_db.create_session(
                    ...
                    source=self.platform or os.environ.get("KUNMING_SESSION_SOURCE", "cli"),
                    model=self.model,
                    model_config={
        ...
        try:
            from kunming_cli.config import load_config as _load_agent_config
            ...
            try:
                ...
                if not _mem_provider_name:
                    try:
                        ...
                        if _hcfg.enabled and (_hcfg.api_key or _hcfg.base_url):
                            ...
                            try:
                                from kunming_cli.config import load_config as _lc, save_config as _sc
                                _cfg = _lc()
                ...
                if _mem_provider_name:
                    ...
                    if self._memory_manager.providers:
                        from kunming_constants import get_kunming_home as _ghh
                        _init_kwargs = {
                            "session_id": self.session_id,
                            "platform": platform or "cli",
                            "kunming_home": str(_ghh()),
    ...
    def switch_model(self, new_model, new_provider, api_key='', base_url='', api_mode=''):
        ...
        """
        import logging
        from kunming_cli.providers import determine_api_mode
    ...
    async def _save_to_reasoning_bank(
        self,
        messages: List[Dict[str, Any]],
        user_query: str,
        final_response: str,
        ...
        """
        Save conversation trajectory to ReasoningBank for learning.
        
        Args:
            messages (List[Dict]): Complete message history
            user_query (str): Original user query
            final_response (str): Final assistant response
            success (bool): Whether the conversation completed successfully
            execution_time_ms (float): Total execution time in milliseconds
        """
        if not self.enable_learning or self._reasoning_bank is None:
            return
        ...
        try:
            ...
            # Convert messages to reasoning steps
            steps = []
            for i, msg in enumerate(messages):
                if msg.get("role") == "assistant":
                    step_content = msg.get("content", "")
                    ...
                    steps.append({
                        "step_number": i,
                        "thought": step_content[:1000],  # Truncate for storage
                        "action": "tool_call" if tool_calls else "reasoning",
                        "observation": ""
                    })
                elif msg.get("role") == "tool":
                    # Add tool result to previous step
                    if steps:
                        steps[-1]["observation"] = msg.get("content", "")[:500]
            
            # Create trajectory
            trajectory = ReasoningTrajectory(
                id=generate_trajectory_id(),
                task_id=self.session_id or str(uuid.uuid4()),
                agent_id=self.session_id or "agent-1",
                steps=steps,
                final_result=final_response,
                success=success,
                total_steps=len(steps),
                total_time_ms=execution_time_ms,
                tags=["auto_captured", self.platform or "unknown"],
                metadata={
                    "model": self.model,
                    "provider": self.provider,
                    "platform": self.platform,
                    "query": user_query[:200]  # Store query for searchability
                }
            )
            
            # Store in ReasoningBank
            await self._reasoning_bank.store(trajectory)
            
            if self.verbose_logging:
                logger.info(f"Saved trajectory to ReasoningBank: {trajectory.id}")
                
        except Exception as e:
            # Don't let learning failures break the main flow
            logger.warning(f"Failed to save trajectory to ReasoningBank: {e}")
    ...
    def _dump_api_request_debug(
        ...
        *,
        ...
        try:
            ...
            if env_var_enabled("KUNMING_DUMP_REQUEST_STDOUT"):
    ...
    def _build_system_prompt(self, system_message: str = None, user_message: str = None) -> str:
        ...
        nous_subscription_prompt = build_kunming_subscription_prompt(self.valid_tool_names)
        ...
        if not self.skip_context_files:
            ...
            # mode).  The gateway process runs from the kunming-agent install
        ...
        from kunming_time import now as _kunming_now
        now = _kunming_now()
    ...
    def _try_refresh_nous_client_credentials(self, *, force: bool = True) -> bool:
        ...
        try:
            from kunming_cli.auth import resolve_nous_runtime_credentials

            creds = resolve_nous_runtime_credentials(
                min_key_ttl_seconds=max(60, int(os.getenv("KUNMING_NOUS_MIN_KEY_TTL_SECONDS", "1800"))),
                timeout_seconds=float(os.getenv("KUNMING_NOUS_TIMEOUT_SECONDS", "15")),
                force_mint=force,
            )
        except Exception as exc:
    ...
    def _interruptible_streaming_api_call(
        ...
        def _call_chat_completions():
            ...
            import httpx as _httpx
            _base_timeout = float(os.getenv("KUNMING_API_TIMEOUT", 1800.0))
            _stream_read_timeout = float(os.getenv("KUNMING_STREAM_READ_TIMEOUT", 60.0))
            stream_kwargs = {
                **api_kwargs,
                "stream": True,
        ...
        def _call():
            ...
            _max_stream_retries = int(os.getenv("KUNMING_STREAM_RETRIES", 2))
            ...
            try:
                for _stream_attempt in range(_max_stream_retries + 1):
                    try:
                        ...
                        # semantically identical to httpx connection drops ?                        # the upstream stream died ?and should be retried with
                        ...
                        if _is_timeout or _is_conn_err or _is_sse_conn_err:
                            ...
                            if _stream_attempt < _max_stream_retries:
                                ...
                                self._emit_status(
                                    f"[!] Connection to provider dropped "
        ...
        while t.is_alive():
            ...
            if _stale_elapsed > _stream_stale_timeout:
                ...
                self._emit_status(
                    f"[!] No response from provider for {int(_stale_elapsed)}s "
    ...
    def _build_api_kwargs(self, api_messages: list) -> dict:
        ...
        api_kwargs = {
            ...
            "timeout": float(os.getenv("KUNMING_API_TIMEOUT", 1800.0)),
        }
        if self._is_qwen_portal():
            api_kwargs["metadata"] = {
                "sessionId": self.session_id or "kunming",
        ...
        if _is_nous:
            extra_body["tags"] = ["product=kunming-agent"]
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        if "models.github.ai" in self._base_url_lower or "api.githubcopilot.com" in self._base_url_lower:
            try:
                from kunming_cli.models import github_model_reasoning_efforts
    ...
    def _emit_context_pressure(self, compaction_progress: float, compressor) -> None:
        ...
        Args:
            compaction_progress: How close to compaction (0.0?.0, where 1.0 = fires).
    ...
    def _handle_max_iterations(self, messages: list, api_call_count: int) -> str:
        ...
        try:
            ...
            if _is_nous:
                summary_extra_body["tags"] = ["product=kunming-agent"]
    ...
    def run_conversation(
        ...
        """
        # Record start time for learning metrics
        ...
        # are reused.  The system prompt is Kunming's territory; plugins
        ...
        while api_call_count < self.max_iterations and self.iteration_budget.remaining > 0:
            ...
            # cache prefix.  The system prompt is reserved for Kunming internals.
            if effective_system:
                api_messages = [{"role": "system", "content": effective_system}] + api_messages
            ...
            while retry_count < max_retries:
                try:
                    ...
                    if env_var_enabled("KUNMING_DUMP_REQUESTS"):
                    ...
                    if response_invalid:
                        ...
                        if self._fallback_index < len(self._fallback_chain):
                            self._emit_status("[!] Empty/malformed response ?switching to fallback...")
                        ...
                        self._vprint(f"{self.log_prefix}   [CONFIG] Provider message: {cleaned_provider_error}", force=True)
                        ...
                        if retry_count >= max_retries:
                            ...
                            self._emit_status(f"[!] Max retries ({max_retries}) for invalid responses ?trying fallback...")
                            if self._try_activate_fallback():
                                retry_count = 0
                                continue
                    ...
                    if isinstance(api_error, UnicodeEncodeError) and not getattr(self, '_surrogate_sanitized', False):
                        ...
                        if _sanitize_messages_surrogates(messages):
                            self._vprint(
                                f"{self.log_prefix}[!]  Stripped invalid surrogate characters from messages. Retrying...",
                                force=True,
                            )
                            continue
                    ...
                    if (
                        ...
                        _dhh = _dhh_fn()
                        print(f"{self.log_prefix}     ?Check ANTHROPIC_TOKEN in {_dhh}/.env for Kunming-managed OAuth/setup tokens")
                        print(f"{self.log_prefix}     ?Check ANTHROPIC_API_KEY in {_dhh}/.env for API keys or legacy token values")
                        print(f"{self.log_prefix}     ?For API keys: verify at https://console.anthropic.com/settings/keys")
                        print(f"{self.log_prefix}     ?For Claude Code: run 'claude /login' to refresh, then retry")
                        print(f"{self.log_prefix}     ?Legacy cleanup: km config set ANTHROPIC_TOKEN \"\"")
                        print(f"{self.log_prefix}     ?Clear stale keys: km config set ANTHROPIC_API_KEY \"\"")
                    ...
                    if (
                        ...
                        if "signature" in _err_msg_lower and "thinking" in _err_msg_lower:
                            ...
                            self._vprint(
                                f"{self.log_prefix}[!]  Thinking block signature invalid ?"
                    ...
                    self._vprint(f"{self.log_prefix}   [CONFIG] Error: {_error_summary}", force=True)
                    ...
                    if _is_long_context_tier_error:
                        ...
                        if self._compression_attempts <= max_compression_attempts:
                            ...
                            if len(messages) < original_len or old_ctx > _reduced_ctx:
                                self._emit_status(
                                    f"🗜?Context reduced to {_reduced_ctx:,} tokens "
                    ...
                    if is_rate_limited and self._fallback_index < len(self._fallback_chain):
                        ...
                        if not pool_may_recover:
                            self._emit_status("[!] Rate limited ?switching to fallback provider...")
                            if self._try_activate_fallback():
                                retry_count = 0
                                continue
                    ...
                    if is_payload_too_large:
                        ...
                        if len(messages) < original_len:
                            self._emit_status(f"🗜?Compressed {original_len} ?{len(messages)} messages, retrying...")
                    ...
                    if not is_context_length_error and status_code == 400:
                        ...
                        if is_large_session and is_generic_error:
                            ...
                            self._vprint(
                                f"{self.log_prefix}[!]  Generic 400 with large session "
                    ...
                    if not is_context_length_error and not status_code:
                        ...
                        if _is_server_disconnect:
                            ...
                            if _is_large:
                                is_context_length_error = True
                                self._vprint(
                                    f"{self.log_prefix}[!]  Server disconnected with large session "
                                    f"(~{approx_tokens:,} tokens, {len(api_messages)} msgs) ?"
                    ...
                    if is_client_error:
                        ...
                        self._emit_status(f"[!] Non-retryable error (HTTP {status_code}) ?trying fallback...")
                        if self._try_activate_fallback():
                            retry_count = 0
                            continue
                        ...
                        if status_code in (401, 403) or "unauthorized" in error_msg or "forbidden" in error_msg or "permission" in error_msg:
                            if _provider == "openai-codex" and status_code == 401:
                                ...
                                self._vprint(f"{self.log_prefix}      2. Then run `km auth` to re-authenticate.", force=True)
                            else:
                                self._vprint(f"{self.log_prefix}   💡 Your API key was rejected by the provider. Check:", force=True)
                                self._vprint(f"{self.log_prefix}      ?Is the key valid? Run: km setup", force=True)
                    ...
                    if retry_count >= max_retries:
                        ...
                        self._emit_status(f"[!] Max retries ({max_retries}) exhausted ?trying fallback...")
                        if self._try_activate_fallback():
                            retry_count = 0
                            continue
            ...
            try:
                ...
                if assistant_message.tool_calls:
                    ...
                    if invalid_tool_calls:
                        ...
                        self._vprint(f"{self.log_prefix}[!]  Unknown tool '{invalid_preview}' ?sending error to model for self-correction ({self._invalid_tool_retries}/3)")
        ...
        # Save trajectory to ReasoningBank for learning (async fire-and-forget)
        if self.enable_learning:
            try:
                execution_time_ms = (time.time() - start_time) * 1000 if 'start_time' in locals() else 0
                # Create async task to save without blocking
                asyncio.create_task(self._save_to_reasoning_bank(
                    messages=list(messages),
                    user_query=original_user_message or user_message,
                    final_response=final_response,
                    success=completed and not interrupted,
                    execution_time_ms=execution_time_ms
                ))
            except Exception as exc:
                logger.warning("Failed to queue trajectory save: %s", exc)
...
def main(
    ...
    print(f"[CHAT] Messages: {len(result['messages'])}")
