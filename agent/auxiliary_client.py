...
Resolution order for text tasks (auto mode):
  ...
  2. Nous Portal (~/.kunming/auth.json active provider)
...
class _CodexCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        try:
            # Collect output items and text deltas during streaming             # the Codex backend can return empty response.output from
            ...
            if isinstance(_output, list) and not _output:
                if collected_output_items:
                    ...
                    # Only synthesize text when no tool calls were streamed                     # a function_call response with incidental text should not
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.kunming/auth.json for an active Nous provider.
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from Kunming auth store."""
    ...
    try:
        from kunming_cli.auth import _read_codex_tokens
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    try:
        from kunming_cli.models import check_nous_free_tier
...
def _resolve_forced_provider(forced: str) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    if forced == "nous":
        ...
        if client is None:
            logger.warning("auxiliary.provider=nous but Nous Portal not configured (run: km auth)")
        return client, model

    if forced == "codex":
        client, model = _try_codex()
        if client is None:
            logger.warning("auxiliary.provider=codex but no Codex OAuth token found (run: km model)")
        return client, model
...
def resolve_provider_client(
    ...
    if provider == "nous":
        ...
        if client is None:
            logger.warning("resolve_provider_client: nous requested "
                           "but Nous Portal not configured (run: km auth)")
            return None, None
        final_model = model or default
        return (_to_async_client(client, final_model) if async_mode
                else (client, final_model))
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: km model)")
                return None, None
    ...
    try:
        from kunming_cli.runtime_provider import _get_named_custom_provider
    ...
    try:
        ...
        logger.debug("kunming_cli.auth not available for provider %s", provider)
        return None, None
...
def _build_call_kwargs(
    ...
    if provider == "nous" or auxiliary_is_nous:
        merged_extra.setdefault("tags", []).extend(["product=kunming-agent"])
    if merged_extra:
    ...
    *,
    ...
    if task == "vision":
        ...
        if client is None:
            ...
            # Pass model=None so each provider uses its own default             # resolved_model may be an OpenRouter-format slug that doesn't
