...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``KUNMING_PLATFORM`` or
            ``KUNMING_SESSION_PLATFORM`` env vars.  Falls back to the
    ...
    resolved_platform = (
        platform
        or os.getenv("KUNMING_PLATFORM")
        or os.getenv("KUNMING_SESSION_PLATFORM")
    )
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.kunming/skills/`` are silently skipped.
    """
    config_path = get_kunming_home() / "config.yaml"
    if not config_path.exists():
        return []
    ...
    local_skills = (get_kunming_home() / "skills").resolve()
    seen: Set[Path] = set()
    result: List[Path] = []
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.kunming/skills/`` first, then external.

    The local dir is always first (and always included even if it doesn't exist
    yet — callers handle that).  External dirs follow in config order.
    """
    dirs = [get_kunming_home() / "skills"]
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    kunming = metadata.get("kunming") or {}
    if not isinstance(kunming, dict):
        kunming = {}
    return {
        "fallback_for_toolsets": kunming.get("fallback_for_toolsets", []),
        "requires_toolsets": kunming.get("requires_toolsets", []),
        "fallback_for_tools": kunming.get("fallback_for_tools", []),
        "requires_tools": kunming.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    kunming = metadata.get("kunming")
    if not isinstance(kunming, dict):
        return []
    raw = kunming.get("config")
    if not raw:
        return []
