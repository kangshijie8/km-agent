...
Data resolution order (like TypeScript OpenCode):
  ...
  2. Disk cache (~/.kunming/models_dev_cache.json)
...
# ---------------------------------------------------------------------------
# Provider ID mapping: Kunming ↔ models.dev
# ---------------------------------------------------------------------------

# Kunming provider names → models.dev provider IDs
...
# Reverse mapping: models.dev → Kunming (built lazily)
_MODELS_DEV_TO_PROVIDER: Optional[Dict[str, str]] = None


def _get_reverse_mapping() -> Dict[str, str]:
    """Return models.dev ID → Kunming provider ID mapping."""
...
def _get_cache_path() -> Path:
    ...
    return get_kunming_home() / "models_dev_cache.json"
...
def _get_provider_models(provider: str) -> Optional[Dict[str, Any]]:
    """Resolve a Kunming provider ID to its models dict from models.dev.
...
def search_models_dev(
    ...
    Args:
        ...
        provider: Optional Kunming provider ID to restrict search scope.
    ...
    if provider is not None:
        ...
        for kunming_prov, mdev_prov in PROVIDER_TO_MODELS_DEV.items():
            provider_data = data.get(mdev_prov, {})
            if isinstance(provider_data, dict):
                models = provider_data.get("models", {})
                if isinstance(models, dict):
                    for mid, mdata in models.items():
                        candidates.append((kunming_prov, mid, mdata))
...
def get_provider_info(provider_id: str) -> Optional[ProviderInfo]:
    ...
    Accepts either a Kunming provider ID (e.g. "kilocode") or a models.dev
    ID (e.g. "kilo").  Returns None if the provider is not in the catalog.
    """
    # Resolve Kunming ID → models.dev ID
...
def list_all_providers() -> Dict[str, ProviderInfo]:
    ...
    a Kunming alias, both the models.dev ID and the Kunming ID are included.
    """
    data = fetch_models_dev()
...
def get_model_info(
    ...
    Accepts Kunming or models.dev provider ID.  Tries exact match then
...
def get_model_info_any_provider(model_id: str) -> Optional[ModelInfo]:
    ...
    a bare name and want to find it anywhere.  Checks Kunming-mapped providers
    first, then falls back to all models.dev providers.
    """
    data = fetch_models_dev()

    # Try Kunming-mapped providers first (more likely what the user wants)
