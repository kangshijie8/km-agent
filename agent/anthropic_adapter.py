"""Anthropic Messages API adapter for Kunming Agent.

Translates between Kunming's internal OpenAI-style message format and
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    Kunming historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by Kunming)
    ...
    # 1. Kunming-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for pre-migration Kunming configs.
    api_key = os.getenv("ANTHROPIC_API_KEY", "").strip()
    if api_key:
        return api_key
...
# -- Kunming-native PKCE OAuth flow ----------------------------------------
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.kunming/.anthropic_oauth.json (our own file).
...
def run_kunming_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run Kunming-native OAuth PKCE flow and return credential state."""
    import time
    import webbrowser
    ...
    print()
    print("Authorize Kunming with your Claude Pro/Max subscription.")
    print()
...
def _save_kunming_oauth_credentials(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Save OAuth credentials to ~/.kunming/.anthropic_oauth.json."""
    data = {
        "accessToken": access_token,
        "refreshToken": refresh_token,
        "expiresAt": expires_at_ms,
...
def read_kunming_oauth_credentials() -> Optional[Dict[str, Any]]:
    ...
    if oauth_file.exists():
        try:
            ...
            logger.debug("Failed to read Kunming OAuth credentials: %s", e)
    return None
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("Kunming Agent", "Claude Code")
                text = text.replace("Kunming agent", "Claude Code")
                text = text.replace("kunming-agent", "claude-code")
                block["text"] = text
