...
def _render_inline_unified_diff(diff: str) -> list[str]:
    """Render unified diff lines in Kunming' inline transcript style."""
    rendered: list[str] = []
    from_file = None
    to_file = None
...
class KawaiiSpinner:
    ...
    def _animate(self):
        ...
        while self.running:
            if os.getenv("KUNMING_SPINNER_PAUSE"):
                time.sleep(0.1)
                continue
...
def format_context_pressure(
    ...
    Args:
        compaction_progress: How close to compaction (0.0✨.0, 1.0 = fires).
