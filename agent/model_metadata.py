...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    "inference-api.kunming.dev": "nous",
...
def _get_context_cache_path() -> Path:
    ...
    return get_kunming_home() / "context_length_cache.yaml"
...
def _load_context_cache() -> Dict[str, int]:
    ...
    if not path.exists():
        return {}
    try:
        # Windows兼容: 指定utf-8编码，避免默认GBK编码读取含非ASCII字符(如中文模型名)的YAML时失败
...
def save_context_length(model: str, base_url: str, length: int) -> None:
    ...
    try:
        ...
        # Windows兼容: 指定utf-8编码写入，避免默认GBK编码写入非ASCII字符时失败
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets Kunming sends to providers:
