...
- POST /v1/chat/completions        -OpenAI Chat Completions format (stateless; opt-in session continuity via X-Kunming-Session-Id header)
- POST /v1/responses               -OpenAI Responses API format (stateful via previous_response_id)
- GET  /v1/responses/{response_id} -Retrieve a stored response
- DELETE /v1/responses/{response_id} -Delete a stored response
- GET  /v1/models                  -lists kunming-agent as an available model
- POST /v1/runs                    -start a run, returns run_id immediately (202)
- GET  /v1/runs/{run_id}/events    -SSE stream of structured lifecycle events
- GET  /health                     -health check

Any OpenAI-compatible frontend (Open WebUI, LobeChat, LibreChat,
AnythingLLM, NextChat, ChatBox, etc.) can connect to kunming-agent
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    and routes them through kunming-agent's AIAgent.
    """
    ...
    def _ensure_session_db(self):
        ...
        Sessions are persisted to ``state.db`` so that ``kunming sessions list``
    ...
    def _create_agent(
        ...
        gateway platforms), falling back to the kunming-api-server default.
        """
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"status": "ok", "platform": "kunming-agent"})

    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models -return kunming-agent as an available model."""
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        # Allow caller to continue an existing session by passing X-Kunming-Session-Id.
        # When provided, history is loaded from state.db instead of from the request body.
        provided_session_id = request.headers.get("X-Kunming-Session-Id", "").strip()
        ...
        model_name = body.get("model", "kunming-agent")
        created = int(time.time())
        ...
        return web.json_response(response_data, headers={"X-Kunming-Session-Id": session_id})
    ...
    async def _write_sse_chat_completion(
        ...
        if session_id:
            sse_headers["X-Kunming-Session-Id"] = session_id
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        response_data = {
            ...
            "model": body.get("model", "kunming-agent"),
            "output": output_items,
            "usage": {
