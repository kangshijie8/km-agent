...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        tokens_file = get_kunming_home() / "slack_tokens.json"
        ...
        try:
            ...
            @self._app.command("/kunming")
            async def handle_kunming_command(ack, command):
                await ack()
                await self._handle_slash_command(command)

            # Register Block Kit action handlers for approval buttons
            for _action_id in (
                "kunming_approve_once",
                "kunming_approve_session",
                "kunming_approve_always",
                "kunming_deny",
            ):
    ...
    async def _handle_slack_message(self, event: dict) -> None:
        ...
        # Add ð reaction to acknowledge receipt
        await self._add_reaction(channel_id, ts, "eyes")

        await self.handle_message(msg_event)

        # Replace ð with [OK]when done
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "kunming_approve_once",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Allow Session"},
                            "action_id": "kunming_approve_session",
                            "value": session_key,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Allow"},
                            "action_id": "kunming_approve_always",
                            "value": session_key,
                        },
                        {
                            "type": "button",
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "kunming_approve_once": "once",
            "kunming_approve_session": "session",
            "kunming_approve_always": "always",
            "kunming_deny": "deny",
        }
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        from kunming_cli.commands import slack_subcommand_map
