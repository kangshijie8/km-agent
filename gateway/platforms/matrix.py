...
# Uses get_kunming_home() so each profile gets its own Matrix store.
from kunming_constants import get_kunming_dir as _get_kunming_dir
_STORE_DIR = _get_kunming_dir("platforms/matrix/store", "matrix/store")
...
_KEY_EXPORT_PASSPHRASE = "kunming-matrix-e2ee-keys"
...
class MatrixAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        if self._encryption and getattr(client, "olm", None):
            ...
            # E2EE was requested but the crypto store failed to load -            # this means encrypted rooms will silently not work.  Hard-fail.
            logger.error(
    ...
    async def _run_e2ee_maintenance(self) -> None:
        ...
        Kunming uses a custom sync loop instead of matrix-nio's sync_forever(),
