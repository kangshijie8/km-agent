...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._media_batch_delay_seconds = float(os.getenv("KUNMING_TELEGRAM_MEDIA_BATCH_DELAY_SECONDS", "0.8"))
        ...
        self._text_batch_delay_seconds = float(os.getenv("KUNMING_TELEGRAM_TEXT_BATCH_DELAY_SECONDS", "0.6"))
    ...
    async def _handle_polling_conflict(self, error: Exception) -> None:
        ...
        message = (
            ...
            "Kunming stopped Telegram polling after %d retries. "
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            if not acquired:
                ...
                message = (
                    "Another local Kunming gateway is already using this Telegram bot token"
            ...
            try:
                ...
                from kunming_cli.commands import telegram_menu_commands
    ...
    async def send(
        ...
        try:
            ...
            for i, chunk in enumerate(chunks):
                ...
                for _send_attempt in range(3):
                    try:
                        ...
                        # indicates the request may have reached the server -                        # retrying risks duplicate message delivery.
            ...
            # TimedOut means the request may have reached Telegram -            # mark as non-retryable so _send_with_retry() doesn't re-send.
    ...
    async def edit_message(
        ...
        try:
            ...
            if "message_too_long" in err_str or "too long" in err_str:
                truncated = content[: self.MAX_MESSAGE_LENGTH - 20] + "â"
                try:
                    await self._bot.edit_message_text(
                        chat_id=int(chat_id),
                        message_id=int(message_id),
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``km update --gateway``
        ...
        try:
            ...
            text = f"[ON]*Update needs your input:*\n\n{prompt}{default_hint}"
            keyboard = InlineKeyboardMarkup([
                [
                    InlineKeyboardButton("[OK]Yes", callback_data="update_prompt:y"),
                    InlineKeyboardButton("[OK]No", callback_data="update_prompt:n"),
                ]
            ])
            msg = await self._bot.send_message(
                chat_id=int(chat_id),
    ...
    async def send_exec_approval(
        ...
        try:
            ...
            text = (
                f"â ï¸ *Command Approval Required*\n\n"
                f"`{cmd_preview}`\n\n"
            ...
            keyboard = InlineKeyboardMarkup([
                ...
                [
                    ...
                    InlineKeyboardButton("[X]Deny", callback_data=f"ea:deny:{approval_id}"),
                ],
            ])
    ...
    async def send_model_picker(
        ...
        try:
            ...
            rows.append([InlineKeyboardButton("[OK]Cancel", callback_data="mx")])
            keyboard = InlineKeyboardMarkup(rows)

            provider_label = get_label(current_provider)
            text = (
                f"[ON]*Model Configuration*\n\n"
    ...
    def _build_model_keyboard(self, models: list, page: int) -> tuple:
        ...
        rows.append([
            ...
            InlineKeyboardButton("[OK]Cancel", callback_data="mx"),
        ])
    ...
    async def _handle_callback_query(
        ...
        try:
            await query.edit_message_text(
                text=f"[ON]Update prompt answered: *{label}*",
                parse_mode=ParseMode.MARKDOWN,
                reply_markup=None,
            )
        except Exception:
