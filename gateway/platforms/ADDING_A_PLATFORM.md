...
Checklist for integrating a new messaging platform into the Kunming gateway.
...
And add it to the `kunming-gateway` composite:

```python
"kunming-gateway": {
    "includes": [..., "kunming-your-platform"]
}
```
...
## 12. Status Display (`kunming_cli/status.py`)
...
## 13. Gateway Setup Wizard (`kunming_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ kunming_cli/ toolsets.py \
  --include="*.py" -l | sort -u
