...
os.environ["KUNMING_QUIET"] = "1"

# Enable interactive exec approval for dangerous commands on messaging platforms
os.environ["KUNMING_EXEC_ASK"] = "1"
...
def _expand_whatsapp_auth_aliases(identifier: str) -> set:
    ...
    session_dir = _kunming_home / "whatsapp" / "session"
    resolved = set()
    queue = [normalized]
...
def _resolve_runtime_agent_kwargs() -> dict:
    ...
    from kunming_cli.runtime_provider import (
    ...
    try:
        runtime = resolve_runtime_provider(
            requested=os.getenv("KUNMING_INFERENCE_PROVIDER"),
        )
            ...
            for skill_md in skills_dir.rglob("SKILL.md"):
                ...
                if name == normalized and name in disabled:
                    return (
                        ...
                        f"Enable it with: `km skills config`"
                    )
            ...
            for skill_md in optional_dir.rglob("SKILL.md"):
                ...
                if name == normalized:
                    ...
                    return (
                        ...
                        f"Install it with: `km skills install {install_path}`"
                    )
    except Exception:
        pass
    return None
...
def _resolve_kunming_bin() -> Optional[list[str]]:
    """Resolve the Kunming update command as argv parts.

    Tries in order:
    1. ``shutil.which("km")`` -standard PATH lookup
    2. ``sys.executable -m kunming_cli.main`` -fallback when Kunming is running
       from a venv/module invocation and the ``km`` shim is not on PATH
    ...
    try:
        ...
        if importlib.util.find_spec("kunming_cli") is not None:
            return [sys.executable, "-m", "kunming_cli.main"]
    except Exception:
        pass
...
class GatewayRunner:
    ...
    def _has_setup_skill(self) -> bool:
        """Check if the kunming-agent-setup skill is installed."""
        try:
            from tools.skill_manager_tool import _find_skill
            return _find_skill("kunming-agent-setup") is not None
        except Exception:
            return False
    ...
    def _load_prefill_messages() -> List[Dict[str, Any]]:
        ...
        Checks KUNMING_PREFILL_MESSAGES_FILE env var first, then falls back to
        the prefill_messages_file key in ~/.kunming/config.yaml.
        Relative paths are resolved from ~/.kunming/.
        """
    ...
    def _load_ephemeral_system_prompt() -> str:
        ...
        Checks KUNMING_EPHEMERAL_SYSTEM_PROMPT env var first, then falls back to
        agent.system_prompt in ~/.kunming/config.yaml.
        """
    ...
    async def start(self) -> bool:
        ...
        """
        logger.info("Starting Kunming Gateway...")
        ...
        for platform, platform_config in self.config.platforms.items():
            ...
            try:
                ...
                if success:
                    ...
                    logger.warning("[OK]%s failed to connect", platform.value)
                ...
                logger.error("[OK]%s error: %s", platform.value, e)
        ...
        if not notified and any(
            path.exists()
            for path in (
                _kunming_home / ".update_pending.json",
                _kunming_home / ".update_pending.claimed.json",
            )
        ):
    ...
    async def stop(self) -> None:
        ...
        for platform, adapter in list(self.adapters.items()):
            try:
                ...
                logger.debug("[OK]%s background-task cancel error: %s", platform.value, e)
            try:
                await adapter.disconnect()
                logger.info("[OK]%s disconnected", platform.value)
            except Exception as e:
                logger.error("[OK]%s disconnect error: %s", platform.value, e)
    ...
    def _create_adapter(
        ...
        if platform == Platform.TELEGRAM:
            ...
            if not check_slack_requirements():
                logger.warning("Slack: slack-bolt not installed. Run: pip install 'kunming-agent[slack]'")
                return None
    ...
    async def _handle_message(self, event: MessageEvent) -> Optional[str]:
                ...
                if code:
                    ...
                    if adapter:
                        await adapter.send(
                            ...
                            f"`km pairing approve {platform_name} {code}`"
                        )
                else:
                    adapter = self.adapters.get(source.platform)
                    if adapter:
        ...
        if _update_prompts.get(_quick_key):
            ...
            if cmd in ("approve", "yes"):
                ...
                response_text = raw
            if response_text:
                response_path = _kunming_home / ".update_response"
                try:
                    ...
                    return f"[OK]Failed to send response to update process: {e}"
        ...
        if _quick_key in self._running_agents:
            ...
            from kunming_cli.commands import resolve_command as _resolve_cmd_inner
        ...
        # in kunming_cli/commands.py -no hardcoded set to maintain here.
        from kunming_cli.commands import GATEWAY_KNOWN_COMMANDS, resolve_command as _resolve_cmd
        ...
        if command:
            try:
                from kunming_cli.plugins import get_plugin_command_handler
                # Normalize underscores to hyphens so Telegram's underscored
                # autocomplete form matches plugin commands registered with
                # hyphens. See kunming_cli/commands.py:_build_telegram_menu.
    ...
    async def _handle_message_with_agent(self, event, source, _quick_key: str):
            ...
            try:
                ...
                if should_notify:
                    ...
                    if adapter:
                        ...
                        notice = (
                            f"oSession automatically reset ({reason_text}). "
        ...
        if history and len(history) >= 4:
            ...
            _hyg_api_key = None
            try:
                _hyg_cfg_path = _kunming_home / "config.yaml"
        ...
        if not history and source.platform and source.platform != Platform.LOCAL and source.platform != Platform.WEBHOOK:
            ...
            if not os.getenv(env_key):
                ...
                if adapter:
                    await adapter.send(
                        source.chat_id,
                        f"ð¬ No home channel is set for {platform_name.title()}. "
                        f"A home channel is where Kunming delivers cron job results "
            ...
            if audio_paths:
                ...
                if any(m in message_text for m in _stt_fail_markers):
                    ...
                    if _stt_adapter:
                        try:
                            _stt_msg = (
                                "[mic] I received your voice message but can't transcribe it -"
                                "no speech-to-text provider is configured.\n\n"
                                "To enable voice: install faster-whisper "
                                "(`pip install faster-whisper` in the Kunming venv) "
                                "and set `stt.enabled: true` in config.yaml, "
                                "then /restart the gateway."
                            )
                            # Point to setup skill if it's installed
                            if self._has_setup_skill():
                                _stt_msg += "\n\nFor full setup instructions, type: `/skill kunming-agent-setup`"
        ...
        try:
            ...
            if not response and agent_result.get("failed"):
                ...
                if _is_ctx_fail:
                    response = (
                        "â ï¸ Session too large for the model's context window.\n"
    ...
    def _format_session_info(self) -> str:
        ...
        lines = [
            f"oModel: `{model}`",
            f"oProvider: {provider or 'openrouter'}",
            f"oContext: {ctx_display} tokens ({ctx_source})",
        ]

        # Show endpoint for local/custom setups
        if base_url and ("localhost" in base_url or "127.0.0.1" in base_url or "0.0.0.0" in base_url):
            lines.append(f"oEndpoint: {base_url}")
    ...
    async def _handle_reset_command(self, event: MessageEvent) -> str:
        ...
        if new_entry:
            header = "[OK]Session reset! Starting fresh."
        else:
            # No existing session, just create one
            new_entry = self.session_store.get_or_create_session(source, force_new=True)
            header = "[OK]New session started!"
    ...
    async def _handle_help_command(self, event: MessageEvent) -> str:
        ...
        from kunming_cli.commands import gateway_help_lines
        lines = [
            "ð **Kunming Commands**\n",
            *gateway_help_lines(),
        ]
        try:
            ...
            skill_cmds = get_skill_commands()
            if skill_cmds:
                lines.append(f"\n[ON]**Skill Commands** ({len(skill_cmds)} active):")
    ...
    async def _handle_commands_command(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            if skill_cmds:
                entries.append("")
                entries.append("[ON]**Skill Commands**:")
        ...
        lines = [
            f"ð **Commands** ({len(entries)} total, page {page}/{total_pages})",
            "",
            *page_entries,
        ]
        if total_pages > 1:
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        """
        import yaml
        from kunming_cli.model_switch import (
            switch_model as _switch_model, parse_model_flags,
            list_authenticated_providers,
        )
        from kunming_cli.providers import get_label
        ...
        if persist_global:
            try:
                ...
                from kunming_cli.config import save_config
                save_config(cfg)
            except Exception as e:
    ...
    async def _handle_provider_command(self, event: MessageEvent) -> str:
        ...
        if current_provider == "auto":
            try:
                from kunming_cli.auth import resolve_provider as _resolve_provider
        ...
        lines = [
            f"ð **Current provider:** {current_label} (`{current_provider}`)",
            "",
            "**Available providers:**",
        ]
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        if args in ("none", "default", "neutral"):
            ...
            return f"ð­ Personality set to **{args}**\n_(takes effect on next message)_"
    ...
    async def _handle_undo_command(self, event: MessageEvent) -> str:
        ...
        return f"â©ï¸ Undid {removed_count} message(s).\nRemoved: \"{preview}\""
        ...
        try:
            ...
            if "pynacl" in err_lower or "nacl" in err_lower or "davey" in err_lower:
                return (
                    ...
                    "Install or reinstall Kunming with the messaging extra, e.g. "
                    "`pip install kunming-agent[messaging]`."
                )
    ...
    async def _send_voice_reply(self, event: MessageEvent, text: str) -> None:
        ...
        try:
            ...
            audio_path = os.path.join(
                tempfile.gettempdir(), "kunming_voice",
    ...
    async def _handle_rollback_command(self, event: MessageEvent) -> str:
        ...
        try:
            import yaml as _y
            _cfg_path = _kunming_home / "config.yaml"
            if _cfg_path.exists():
    ...
    async def _handle_background_command(self, event: MessageEvent) -> str:
        ...
        return f'ð Background task started: "{preview}"\nTask ID: {task_id}\nYou can keep chatting -results will appear when done.'
    ...
    async def _run_background_task(
        ...
        try:
            ...
            if not runtime_kwargs.get("api_key"):
                await adapter.send(
                    source.chat_id,
                    f"[X]Background task {task_id} failed: no provider credentials configured.",
            ...
            from kunming_cli.tools_config import _get_platform_tools
            ...
            try:
                await adapter.send(
                    chat_id=source.chat_id,
                    content=f"[X]Background task {task_id} failed: {e}",
                    metadata=_thread_metadata,
                )
            except Exception:
                pass
    ...
    async def _run_btw_task(
        ...
        try:
            ...
            if not runtime_kwargs.get("api_key"):
                await adapter.send(
                    source.chat_id,
                    "[X]/btw failed: no provider credentials configured.",
                    metadata=_thread_meta,
                )
                return
    ...
    async def _handle_yolo_command(self, event: MessageEvent) -> str:
        ...
        current = bool(os.environ.get("KUNMING_YOLO_MODE"))
        if current:
            os.environ.pop("KUNMING_YOLO_MODE", None)
            return "â ï¸ YOLO mode **OFF** -dangerous commands will require approval."
        else:
            os.environ["KUNMING_YOLO_MODE"] = "1"
            return "[ON]YOLO mode **ON** -all commands auto-approved. Use with caution."
    ...
    async def _handle_verbose_command(self, event: MessageEvent) -> str:
        ...
        descriptions = {
            "off": "âï¸ Tool progress: **OFF** -no tool activity shown.",
            "new": "âï¸ Tool progress: **NEW** -shown when tool changes (preview length: `display.tool_preview_length`, default 40).",
            "all": "âï¸ Tool progress: **ALL** -every tool call shown (preview length: `display.tool_preview_length`, default 40).",
            "verbose": "âï¸ Tool progress: **VERBOSE** -every tool call with full arguments.",
        }
    ...
    async def _handle_compress_command(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            return (
                f"ðï¸?Compressed: {original_count} ->{new_count} messages\n"
    ...
    async def _handle_title_command(self, event: MessageEvent) -> str:
        ...
        if title_arg:
            ...
            try:
                ...
                return f"â ï¸ {e}"
            if not sanitized:
                return "â ï¸ Title is empty after cleanup. Please use printable characters."
            # Set the title
            try:
                if self._session_db.set_session_title(session_id, sanitized):
                    return f"âï¸ Session title set: **{sanitized}**"
                else:
    ...
    async def _handle_resume_command(self, event: MessageEvent) -> str:
        ...
        if current_entry.session_id == target_id:
            return f"ð Already on session **{name}**."
    ...
    async def _handle_reload_mcp_command(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            lines = ["ð **MCP Servers Reloaded**\n"]
            if reconnected:
                lines.append(f"â»ï¸ Reconnected: {', '.join(sorted(reconnected))}")
            if added:
                lines.append(f"â?Added: {', '.join(sorted(added))}")
            if removed:
                lines.append(f"â?Removed: {', '.join(sorted(removed))}")
            if not connected_servers:
                lines.append("No MCP servers connected.")
            else:
                lines.append(f"\nð§ {len(new_tools)} tool(s) available from {len(connected_servers)} server(s)")
            ...
            return f"[X]MCP reload failed: {e}"
    ...
    async def _handle_approve_command(self, event: MessageEvent) -> Optional[str]:
        ...
        if not has_blocking_approval(session_key):
            if session_key in self._pending_approvals:
                ...
                return "â ï¸ Approval expired (agent is no longer waiting). Ask the agent to try again."
    ...
    async def _handle_deny_command(self, event: MessageEvent) -> str:
        ...
        if not has_blocking_approval(session_key):
            if session_key in self._pending_approvals:
                self._pending_approvals.pop(session_key)
                return "[X]Command denied (approval was stale)."
        ...
        return f"[X]Command{'s' if count > 1 else ''} denied{count_msg}."
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update command -update Kunming Agent to the latest version.

        Spawns ``km update`` in a detached session (via ``setsid``) so it
        survives the gateway restart that ``km update`` may trigger. Marker
        ...
        import subprocess
        from datetime import datetime
        from kunming_cli.config import is_managed, format_managed_message

        # Block non-messaging platforms (API server, webhooks, ACP)
        platform = event.source.platform
        if platform not in self._UPDATE_ALLOWED_PLATFORMS:
            return "[OK]/update is only available from messaging platforms. Run `km update` from the terminal."

        if is_managed():
            return f"[OK]{format_managed_message('update Kunming Agent')}"

        project_root = Path(__file__).parent.parent.resolve()
        git_dir = project_root / '.git'

        if not git_dir.exists():
            return "[OK]Not a git repository -cannot update."

        kunming_cmd = _resolve_kunming_bin()
        if not kunming_cmd:
            return (
                "[OK]Could not locate the `km` command. "
                "Kunming is running, but the update command could not find the "
                "executable on PATH or via the current Python interpreter. "
                "Try running `km update` manually in your terminal."
            )

        pending_path = _kunming_home / ".update_pending.json"
        output_path = _kunming_home / ".update_output.txt"
        exit_code_path = _kunming_home / ".update_exit_code"
        ...
        # Spawn `km update --gateway` detached so it survives gateway restart.
        ...
        kunming_cmd_str = " ".join(shlex.quote(part) for part in kunming_cmd)
        update_cmd = (
            f"PYTHONUNBUFFERED=1 {kunming_cmd_str} update --gateway"
        ...
        try:
            if _IS_WINDOWS:
                subprocess.Popen(
                    ["cmd", "/c", f"start /b {kunming_cmd_str} update --gateway > {output_path} 2>&1 & echo %errorlevel% > {exit_code_path}"],
            ...
            exit_code_path.unlink(missing_ok=True)
            return f"[OK]Failed to start update: {e}"

        self._schedule_update_notification_watch()
        return "[ON]Starting Kunming update-I'll stream progress here."
    ...
    async def _watch_update_progress(
        ...
        """Watch ``km update --gateway``, streaming output + forwarding prompts.
        ...
        pending_path = _kunming_home / ".update_pending.json"
        claimed_path = _kunming_home / ".update_pending.claimed.json"
        output_path = _kunming_home / ".update_output.txt"
        exit_code_path = _kunming_home / ".update_exit_code"
        prompt_path = _kunming_home / ".update_prompt.json"
        ...
        while loop.time() < deadline:
            ...
            if exit_code_path.exists():
                ...
                try:
                    ...
                    if exit_code == 0:
                        await adapter.send(chat_id, "[OK]Kunming update finished.")
                    else:
                        await adapter.send(chat_id, "[X]Kunming update failed (exit code {}).".format(exit_code))
                ...
                (_kunming_home / ".update_response").unlink(missing_ok=True)
        ...
        if not exit_code_path.exists():
            ...
            await _flush_buffer()
            try:
                await adapter.send(chat_id, "[X]Kunming update timed out after 30 minutes.")
            except Exception:
                pass
            for p in (pending_path, claimed_path, output_path,
                      exit_code_path, prompt_path):
            ...
            if adapter and chat_id:
                ...
                if output:
                    if len(output) > 3500:
                        output = "..." + output[-3500:]
                    if exit_code == 0:
                        msg = f"[OK]Kunming update finished.\n\n```\n{output}\n```"
                    else:
                        msg = f"[X]Kunming update failed.\n\n```\n{output}\n```"
                else:
                    if exit_code == 0:
                        msg = "[OK]Kunming update finished successfully."
                    else:
                        msg = "[X]Kunming update failed. Check the gateway logs or run `km update` manually for details."
    ...
    def _set_session_env(self, context: SessionContext) -> None:
        ...
        os.environ["KUNMING_SESSION_PLATFORM"] = context.source.platform.value
        os.environ["KUNMING_SESSION_CHAT_ID"] = context.source.chat_id
        if context.source.chat_name:
            os.environ["KUNMING_SESSION_CHAT_NAME"] = context.source.chat_name
        if context.source.thread_id:
            os.environ["KUNMING_SESSION_THREAD_ID"] = str(context.source.thread_id)
    
    def _clear_session_env(self) -> None:
        """Clear session environment variables."""
        for var in ["KUNMING_SESSION_PLATFORM", "KUNMING_SESSION_CHAT_ID", "KUNMING_SESSION_CHAT_NAME", "KUNMING_SESSION_THREAD_ID"]:
    ...
    async def _enrich_message_with_transcription(
        ...
        if not getattr(self.config, "stt_enabled", True):
            ...
            if self._has_setup_skill():
                disabled_note += (
                    " You have a skill called kunming-agent-setup that can help "
                    "users configure Kunming features including voice, tools, and more."
                )
            disabled_note += "]"
            if user_text:
            ...
            try:
                ...
                if result["success"]:
                    ...
                    if (
                        ...
                        if self._has_setup_skill():
                            _no_stt_note += (
                                " You have a skill called kunming-agent-setup "
                                "that can help users configure Kunming features "
    ...
    async def _run_agent(
        ...
        progress_mode = (
            _raw_tp
            or os.getenv("KUNMING_TOOL_PROGRESS_MODE")
            or "all"
        )
        ...
        def progress_callback(event_type: str, tool_name: str = None, preview: str = None, args: dict = None, **kwargs):
            ...
            emoji = get_tool_emoji(tool_name, default="âï¸")
        ...
        def run_sync():
            ...
            os.environ["KUNMING_SESSION_KEY"] = session_key or ""
            ...
            try:
                ...
                return {
                    "final_response": f"â ï¸ Provider authentication failed: {exc}",
                    "messages": [],
                    "api_calls": 0,
                    "tools": [],
                }
            ...
            # Background review delivery -send "ð¾ Memory updated" etc. to user
            ...
            for msg in history:
                ...
                # through intact so the API sees valid assistantâtool sequences
            ...
            def _approval_notify_sync(approval_data: dict) -> None:
                ...
                msg = (
                    f"â ï¸ **Dangerous command requires approval:**\n"
                    f"```\n{cmd_preview}\n```\n"
                    f"Reason: {desc}\n\n"
            ...
            if not final_response:
                error_msg = f"â ï¸ {result['error']}" if result.get("error") else "(No response generated)"
                return {
            ...
            while True:
                ...
                try:
                    await _notify_adapter.send(
                        source.chat_id,
                        f"â?Still working... ({_elapsed_mins} min elapsed{_status_detail})",
                        metadata=_status_thread_metadata,
                    )
        ...
        try:
            ...
            # KUNMING_AGENT_TIMEOUT env var (env var takes precedence).
            # Default 1800s (30 min inactivity).  0 = unlimited.
            _agent_timeout_raw = float(os.getenv("KUNMING_AGENT_TIMEOUT", 1800))
            _agent_timeout = _agent_timeout_raw if _agent_timeout_raw > 0 else None
            _agent_warning_raw = float(os.getenv("KUNMING_AGENT_TIMEOUT_WARNING", 900))
                        ...
                        if _warn_adapter:
                            ...
                            try:
                                await _warn_adapter.send(
                                    source.chat_id,
                                    f"â ï¸ No activity for {_elapsed_warn} min. "
            ...
            if _inactivity_timeout:
                ...
                _diag_lines = [
                    f"â±ï¸ Agent inactive for {_timeout_mins} min -no tool calls "
                    f"or API responses."
                ]
                if _cur_tool:
                    _diag_lines.append(
            ...
            if pending and pending.strip().startswith("/"):
                ...
                if _pending_cmd_word:
                    try:
                        from kunming_cli.commands import resolve_command as _rc_pending
...
async def start_gateway(config: Optional[GatewayConfig] = None, replace: bool = False, verbosity: Optional[int] = 0) -> bool:
    ...
    # Prevent two gateways from running under the same KUNMING_HOME.
    # The PID file is scoped to KUNMING_HOME, so future multi-profile
    # setups (each profile using a distinct KUNMING_HOME) will naturally
    ...
    if existing_pid is not None and existing_pid != os.getpid():
        if replace:
            ...
            for _ in range(20):
                ...
                try:
                    # Windows doesn't have SIGKILL, use terminate/kill methods
                    if _IS_WINDOWS:
                        import ctypes
                        handle = ctypes.windll.kernel32.OpenProcess(1, False, existing_pid)
                        if handle:
            ...
            kunming_home = str(get_kunming_home())
            logger.error(
                "Another gateway instance is already running (PID %d, KUNMING_HOME=%s). "
                "Use 'km gateway restart' to replace it, or 'km gateway stop' first.",
                existing_pid, kunming_home,
            )
            print(
                f"\n[X]Gateway already running (PID {existing_pid}).\n"
                f"   Use 'km gateway restart' to replace it,\n"
                f"   or 'km gateway stop' to kill it first.\n"
                f"   Or use 'km gateway run --replace' to auto-replace.\n"
            )
            return False
    ...
    log_dir = setup_logging(kunming_home=_kunming_home, mode="gateway")
    ...
    from kunming_logging import _add_rotating_handler
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Kunming Gateway - Multi-platform messaging")
