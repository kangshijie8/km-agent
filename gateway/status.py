...
The PID file lives at ``{KUNMING_HOME}/gateway.pid``.  KUNMING_HOME defaults to
``~/.kunming`` but can be overridden via the environment variable.  This means
separate KUNMING_HOME directories naturally get separate PID files — a property
...
_GATEWAY_KIND = "kunming-gateway"
_RUNTIME_STATUS_FILE = "gateway_state.json"
_LOCKS_DIRNAME = "gateway-locks"


def _get_pid_path() -> Path:
    """Return the path to the gateway PID file, respecting KUNMING_HOME."""
...
def _get_lock_dir() -> Path:
    ...
    override = os.getenv("KUNMING_GATEWAY_LOCK_DIR")
    if override:
        return Path(override)
    # Windows: use LOCALAPPDATA instead of XDG_STATE_HOME
    if _IS_WINDOWS:
        state_home = Path(os.getenv("LOCALAPPDATA", Path.home() / "AppData" / "Local"))
    else:
...
def _get_process_start_time(pid: int) -> Optional[int]:
    ...
    On Linux: reads from /proc/{pid}/stat
    On Windows: uses ctypes to query process creation time
    """
    if _IS_WINDOWS:
        try:
            import ctypes
            ...
            try:
                ...
                if GetProcessTimes(hProcess, creation_time, exit_time, kernel_time, user_time):
                    # Convert FILETIME to a hashable integer (100-nanosecond intervals since 1601)
...
def _read_process_cmdline(pid: int) -> Optional[str]:
    ...
    On Windows: uses wmic or psutil if available
    """
    if _IS_WINDOWS:
        try:
            # Try using wmic first (built into Windows)
            ...
            if result.returncode == 0:
                # Parse wmic output (format: CommandLine=<value>)
...
def _looks_like_gateway_process(pid: int) -> bool:
    """Return True when the live PID still looks like the Kunming gateway."""
...
def acquire_scoped_lock(scope: str, identity: str, metadata: Optional[dict[str, Any]] = None) -> tuple[bool, Optional[dict[str, Any]]]:
    ...
    at once (e.g. the same Telegram bot token across different KUNMING_HOME dirs).
    """
    ...
    if existing:
        ...
        if not stale:
            try:
                ...
                # Note: This check is Linux-only; Windows doesn't have the same signal concept
