...
# Without this, standalone invocations (e.g. after `km update` reloads
# the module) fail with ModuleNotFoundError for kunming_time et al.
...
def _run_job_script(script_path: str) -> tuple[bool, str]:
    ...
    Scripts must reside within KUNMING_HOME/scripts/.  Both relative and
    ...
    Args:
        script_path: Path to a Python script.  Relative paths are resolved
            against KUNMING_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    scripts_dir = get_kunming_home() / "scripts"
    ...
    # escape -scripts MUST reside within KUNMING_HOME/scripts/.
    try:
...
def _build_job_prompt(job: dict) -> str:
    ...
    if skipped:
        notice = (
            ...
            f"'â ï¸ Skill(s) not found and skipped: {', '.join(skipped)}']"
        )
...
def run_job(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    origin = _resolve_origin(job)
    _cron_session_id = f"cron_{job_id}_{_kunming_now().strftime('%Y%m%d_%H%M%S')}"
    ...
    try:
        ...
        if origin:
            os.environ["KUNMING_SESSION_PLATFORM"] = origin["platform"]
            os.environ["KUNMING_SESSION_CHAT_ID"] = str(origin["chat_id"])
            if origin.get("chat_name"):
                os.environ["KUNMING_SESSION_CHAT_NAME"] = origin["chat_name"]
        # Re-read .env and config.yaml fresh every run so provider/key
        # changes take effect without a gateway restart.
        from dotenv import load_dotenv
        try:
            load_dotenv(str(_kunming_home / ".env"), override=True, encoding="utf-8")
        except UnicodeDecodeError:
            load_dotenv(str(_kunming_home / ".env"), override=True, encoding="latin-1")

        delivery_target = _resolve_delivery_target(job)
        if delivery_target:
            os.environ["KUNMING_CRON_AUTO_DELIVER_PLATFORM"] = delivery_target["platform"]
            os.environ["KUNMING_CRON_AUTO_DELIVER_CHAT_ID"] = str(delivery_target["chat_id"])
            if delivery_target.get("thread_id") is not None:
                os.environ["KUNMING_CRON_AUTO_DELIVER_THREAD_ID"] = str(delivery_target["thread_id"])

        model = job.get("model") or os.getenv("KUNMING_MODEL") or ""

        # Load config.yaml for model, reasoning, prefill, toolsets, provider routing
        _cfg = {}
        try:
            import yaml
            _cfg_path = str(_kunming_home / "config.yaml")
        ...
        prefill_file = os.getenv("KUNMING_PREFILL_MESSAGES_FILE", "") or _cfg.get("prefill_messages_file", "")
        if prefill_file:
            import json as _json
            pfpath = Path(prefill_file).expanduser()
            if not pfpath.is_absolute():
                pfpath = _kunming_home / pfpath
            if pfpath.exists():
                try:
        ...
        try:
            runtime_kwargs = {
                "requested": job.get("provider") or os.getenv("KUNMING_INFERENCE_PROVIDER"),
            }
            if job.get("base_url"):
        ...
        # override via KUNMING_CRON_TIMEOUT env var.  0 = unlimited.
        #
        # Uses the agent's built-in activity tracker (updated by
        # _touch_activity() on every tool call, API call, and stream delta).
        _cron_timeout = float(os.getenv("KUNMING_CRON_TIMEOUT", 600))
...
**Job ID:** {job_id}
**Run Time:** {_kunming_now().strftime('%Y-%m-%d %H:%M:%S')}
...
"""
        ...
        for key in (
            "KUNMING_SESSION_PLATFORM",
            "KUNMING_SESSION_CHAT_ID",
            "KUNMING_SESSION_CHAT_NAME",
            "KUNMING_CRON_AUTO_DELIVER_PLATFORM",
            "KUNMING_CRON_AUTO_DELIVER_CHAT_ID",
            "KUNMING_CRON_AUTO_DELIVER_THREAD_ID",
        ):
...
def tick(verbose: bool = True, adapters=None, loop=None) -> int:
    ...
    try:
        ...
        if verbose and not due_jobs:
            logger.info("%s - No jobs due", _kunming_now().strftime('%H:%M:%S'))
            return 0

        if verbose:
            logger.info("%s - %s job(s) due", _kunming_now().strftime('%H:%M:%S'), len(due_jobs))
        ...
        for job in due_jobs:
            try:
                ...
                deliver_content = final_response if success else f"â ï¸ Cron job '{job.get('name', job['id'])}' failed:\n{error}"
