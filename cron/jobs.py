...
Jobs are stored in ~/.kunming/cron/jobs.json
Output is saved to ~/.kunming/cron/output/{job_id}/{timestamp}.md
"""
...
# 修复: 延迟求值避免import时profile未设置导致路径错误 [H3]
def _get_kunming_dir():
    return get_kunming_home()
def _get_cron_dir():
    return _get_kunming_dir() / "cron"
def _get_jobs_file():
...
def parse_schedule(schedule: str) -> Dict[str, Any]:
    ...
    try:
        ...
        run_at = _kunming_now() + timedelta(minutes=minutes)
        return {
            "kind": "once",
...
def _ensure_aware(dt: datetime) -> datetime:
    """Return a timezone-aware datetime in Kunming configured timezone.

    Backward compatibility:
    - Older stored timestamps may be naive.
    - Naive values are interpreted as *system-local wall time* (the timezone
      `datetime.now()` used when they were created), then converted to the
      configured Kunming timezone.

    This preserves relative ordering for legacy naive timestamps across
    timezone changes and avoids false not-due results.
    """
    target_tz = _kunming_now().tzinfo
    if dt.tzinfo is None:
...
def save_jobs(jobs: List[Dict[str, Any]]):
    ...
    try:
        with os.fdopen(fd, 'w', encoding='utf-8') as f:
            json.dump({"jobs": jobs, "updated_at": _kunming_now().isoformat()}, f, indent=2)
            f.flush()
            os.fsync(f.fileno())
...
def pause_job(job_id: str, reason: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    return update_job(
        job_id,
        {
            "enabled": False,
            "state": "paused",
            "paused_at": _kunming_now().isoformat(),
...
def trigger_job(job_id: str) -> Optional[Dict[str, Any]]:
    ...
    return update_job(
        job_id,
        {
            ...
            "paused_at": None,
            "paused_reason": None,
            "next_run_at": _kunming_now().isoformat(),
        },
    )
...
def save_job_output(job_id: str, output: str):
    ...
    timestamp = _kunming_now().strftime("%Y-%m-%d_%H-%M-%S")
