"""ACP session manager -maps ACP sessions to Kunming AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.kunming/state.db``) so they
...
@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Kunming agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Kunming AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Kunming runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.kunming/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``KUNMING_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_kunming_home``).
        """
        ...
        try:
            ...
            self._db_instance = SessionDB(db_path=kunming_home / "state.db")
            return self._db_instance
        except Exception:
