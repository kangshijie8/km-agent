"""ACP agent server 鈥?exposes Kunming Agent via the Agent Client Protocol."""
...
try:
    from kunming_cli import __version__ as KUNMING_VERSION
except Exception:
    KUNMING_VERSION = "0.0.0"
...
class KunmingACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Kunming AIAgent."""
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show Kunming version",
        },
    )
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = getattr(state.agent, "enabled_toolsets", None) or ["kunming-acp"]
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        if provider:
            auth_methods = [
                AuthMethodAgent(
                    ...
                    description=f"Authenticate Kunming using the currently configured {provider} runtime credentials.",
                )
            ]
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="kunming-agent", version=KUNMING_VERSION),
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Kunming on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        # Intercept slash commands 鈥?handle locally without calling the LLM
    ...
    def _handle_slash_command(self, text: str, state: SessionState) -> str | None:
        ...
        if handler is None:
            return None  # not a known command 鈥?let the LLM handle it
    ...
    def _cmd_model(self, args: str, state: SessionState) -> str:
        ...
        try:
            from kunming_cli.models import parse_model_input, detect_provider_for_model
    ...
    def _cmd_tools(self, args: str, state: SessionState) -> str:
        try:
            from model_tools import get_tool_definitions
            toolsets = getattr(state.agent, "enabled_toolsets", None) or ["kunming-acp"]
    ...
    def _cmd_compact(self, args: str, state: SessionState) -> str:
        if not state.history:
            return "Nothing to compress 鈥?conversation is empty."
        try:
            agent = state.agent
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Kunming Agent v{KUNMING_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Kunming has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
