# Streaming LLM Response Support for Kunming Agent
...
```
                              stream_callback(delta)
                                    â?  âââââââââââââââ?   âââââââââââââââ¼âââââââââââââââ?  â? LLM API    â?   â?     queue.Queue()          â?  â? (stream)   âââââºâ  thread-safe bridge between â?  â?            â?   â? agent thread & consumer    â?  âââââââââââââââ?   âââââââââââââââ¬âââââââââââââââ?                                   â?                    ââââââââââââââââ¼âââââââââââââââ?                    â?             â?             â?              âââââââ¼ââââââ?âââââââ¼ââââââ?âââââââ¼ââââââ?              â?   CLI     â?â? Gateway  â?â?API Serverâ?              â?print to   â?â?edit msg  â?â?SSE event â?              â?terminal   â?â?on Tg/Dc  â?â?to client â?              âââââââââââââ?âââââââââââââ?âââââââââââââ?```
...
```
KUNMING_STREAMING_ENABLED=true    # Master switch via env
```
...
try:
    ...
    pass
# Env var override
if os.getenv("KUNMING_STREAMING_ENABLED", "").lower() in ("true", "1", "yes"):
...
|----------|-------------|-------------|-------------------|
| Telegram | â?edit_message_text | ~20 edits/min | Edit every 1.5s |
| Discord | â?message.edit | 5 edits/5s per message | Edit every 1.2s |
| Slack | â?chat.update | Tier 3 (~50/min) | Edit every 1.5s |
| WhatsApp | â?no edit support | N/A | Skip streaming, use normal path |
| HomeAssistant | â?no edit | N/A | Skip streaming |
| API Server | â?SSE native | No limit | Real SSE events |
...
| `kunming_cli/config.py` | 1 | +streaming config defaults |
...
KUNMING_STREAMING_ENABLED=true
```
