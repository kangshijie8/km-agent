# OpenAI-Compatible API Server for Kunming Agent
...
SSE streaming. By exposing this endpoint, kunming-agent becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  kunming-agent   │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.kunming/.env`
2. Run `kunming gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with kunming-agent through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns kunming-agent as a model) |
...
Authorization: Bearer kunming-api-key-here
...
But kunming-agent sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "kunming-agent"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured kunming-agent model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of kunming-agent's core prompt
...
Once implemented, kunming-agent works as a drop-in backend for:
