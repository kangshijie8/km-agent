"""km webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    km webhook subscribe <name> [options]
    km webhook list
    km webhook remove <name>
    km webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.kunming/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _kunming_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: km gateway run
"""
...
def webhook_command(args):
    """Entry point for 'km webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: km webhook {subscribe|list|remove|test}")
        print("Run 'km webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print(f"  The gateway must be running to receive events (km gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: km webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from kunming webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (km gateway run)")
