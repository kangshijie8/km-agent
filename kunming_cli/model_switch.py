...
- ``kunming_cli.providers``        -- canonical provider identity + overlays
- ``kunming_cli.model_normalize``  -- per-provider name formatting
...
from kunming_cli.providers import (
    determine_api_mode,
    get_label,
    is_aggregator,
    resolve_provider_full,
)
from kunming_cli.model_normalize import (
...
_KUNMING_MODEL_WARNING = (
    "This model is NOT agentic and is not designed "
    "for use with Kunming Agent. It lacks the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)


def _check_kunming_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* looks like a Kunming LLM model."""
    if "kunming" in model_name.lower():
        return _KUNMING_MODEL_WARNING
    return ""
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'km model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'km doctor' — config issues detected:"
    ...
    # 修复: 移除set中重复的"opencode-go"项 [M11]
    ...
    kunming_warn = _check_kunming_model_warning(new_model)
    if kunming_warn:
        warnings.append(kunming_warn)
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from kunming_cli/models.py (OPENROUTER_MODELS,
    ...
    from kunming_cli.models import OPENROUTER_MODELS, _PROVIDER_MODELS
    ...
    # Build curated model lists keyed by kunming provider ID
    ...
    # --- 1. Check Kunming-mapped providers ---
    for kunming_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        model_ids = curated.get(kunming_id, [])
        total = len(model_ids)
    ...
    # --- 2. Check Kunming-only providers (nous, openai-codex, copilot) ---
    from kunming_cli.providers import KUNMING_OVERLAYS
    for pid, overlay in KUNMING_OVERLAYS.items():
        ...
        if overlay.auth_type in ("oauth_device_code", "oauth_external", "external_process"):
            ...
            try:
                from kunming_cli.auth import _load_auth_store
    ...
    if not cust_base or "openrouter.ai" in cust_base:
        return CustomAutoResult(
            ...
            error_message=(
                ...
                "in .env, or run: km setup -> Custom OpenAI-compatible endpoint"
            ),
        )
