"""
Configuration management for Kunming Agent.

Config files are stored in ~/.kunming/ for easy access:
- ~/.kunming/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.kunming/.env         - API keys and secrets

This module provides:
- km config          - Show current configuration
- km config edit     - Open config in editor
- km config set      - Set a specific value
- km config wizard   - Re-run setup wizard
"""
...
from kunming_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("KUNMING_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_kunming_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Kunming is running in package-manager-managed mode.

    Two signals: the KUNMING_MANAGED env var (set by the systemd service),
    or a .managed marker file in KUNMING_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade kunming-agent"
...
def recommended_update_command() -> str:
    ...
    return get_managed_update_command() or "km update"


def format_managed_message(action: str = "modify this Kunming installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("KUNMING_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Kunming installation is managed by NixOS "
            f"(KUNMING_MANAGED={env_hint}).\n"
            "Edit services.kunming-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Kunming installation is managed by Homebrew "
            f"(KUNMING_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade kunming-agent"
        )

    return (
        f"Cannot {action}: this Kunming installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Kunming."
    )
...
# Re-export from kunming_constants —canonical definition lives there.
from kunming_constants import get_kunming_home  # noqa: F811,E402

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_kunming_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_kunming_home() / ".env"
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the kunming group can
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into KUNMING_HOME if the user doesn't have one yet."""
...
def ensure_kunming_home():
    """Ensure ~/.kunming directory structure exists with secure permissions."""
...
DEFAULT_CONFIG = {
    "model": {
        "default": "",       # Model name (e.g., "anthropic/claude-sonnet-4")
        "provider": "",      # Provider ID (empty = auto-detect from model name)
        "base_url": "",      # Custom API endpoint
    },
    "providers": {},
    "fallback_providers": [],
    "credential_pool_strategies": {},
    "toolsets": ["kunming-cli"],
    "agent": {
        "max_turns": 90,
    ...
    "terminal": {
        ...
        # docker_env lets you specify exact key-value pairs — useful when Kunming
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Kunming sends a stable profile-scoped userId to Camofox
    ...
    # 100K chars ≈25—5K tokens across typical tokenisers.
    ...
    # always goes to ~/.kunming/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
    },

    # Honcho AI-native memory -- reads ~/.honcho/config.json as single source of truth.
    # This section is only needed for kunming-specific overrides; everything else
    ...
    "discord": {
        ...
        "reactions": True,             # Add ð/✓❌🧩reactions to messages during processing
    },

    # WhatsApp platform settings (gateway mode)
    "whatsapp": {
        # Reply prefix prepended to every outgoing WhatsApp message.
        # Default (None) uses the built-in "⚡*Kunming Agent*" header.
        # Set to "" (empty string) to disable the header entirely.
        # Supports \n for newlines, e.g. "ð¤ *My Bot*\n------\n"
    },
    ...
    # Logging —controls file logging to ~/.kunming/logs/.
...
OPTIONAL_ENV_VARS = {
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for Nous Subscribers only, used to derive vendor hosts, e.g. kunming.dev -> firecrawl-gateway.kunming.dev",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the Kunming auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @kunming:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. KUNMING_BOT)",
    ...
    # KUNMING_TOOL_PROGRESS and KUNMING_TOOL_PROGRESS_MODE are deprecated ——    # now configured via display.tool_progress in config.yaml (off|new|all|verbose).
    # Gateway falls back to these env vars for backward compatibility.
    "KUNMING_TOOL_PROGRESS": {
    ...
    "KUNMING_EPHEMERAL_SYSTEM_PROMPT": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.kunming.config`` entries, then checks
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'km setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Kunming won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("KUNMING_TOOL_PROGRESS")
            old_mode = get_env_value("KUNMING_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in ("false", "0", "no"):
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from KUNMING_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in ("new", "all"):
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from KUNMING_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("KUNMING_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from KUNMING_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in ("y", "yes"):
                ...
                print("  Set later with: km config set <key> <value>")
    ...
    # metadata.kunming.config in their SKILL.md frontmatter.
...
def _deep_merge(base: dict, override: dict) -> dict:
    ...
    Handles type mismatches gracefully: if user provides a string where
    ...
    for key, value in override.items():
        if key in result and isinstance(result[key], dict):
            if isinstance(value, dict):
                ...
                # Special case: model can be a string "provider/model"
                # Convert to dict structure while preserving other defaults
                result[key] = result[key].copy()
                result[key]["default"] = value
                # Auto-extract provider from model name if it contains "/"
                if "/" in value:
                    result[key]["provider"] = value.split("/", 1)[0]
            else:
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.kunming/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.kunming/config.yaml."""
    import copy
    ensure_kunming_home()
    config_path = get_config_path()
...
def bridge_config_to_env(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    This is the unified entry point for config-to-env bridging across CLI,
    gateway, and all other entry points. It handles:
    - Terminal configuration bridging (backend, cwd, timeout, etc.)
    - Auxiliary model configuration bridging (vision, web_extract)
    - Top-level simple values bridging
    
    Args:
        config: The configuration dict. If None, load_config() will be called.
    
    Note:
        This function modifies os.environ directly. It should be called once
        at application startup, before any tools that depend on env vars.
    """
    import json
    import os
    ...
    if terminal_cfg and isinstance(terminal_cfg, dict):
        # Normalize key: "backend" takes precedence over legacy "env_type"
        backend = terminal_cfg.get("backend") or terminal_cfg.get("env_type", "local")
        ...
        if cwd in (".", "auto", "cwd"):
            if backend == "local":
                ...
                # For remote/container backends, remove so tool uses backend default
                cwd = None
        
        # Terminal config to env var mapping
        terminal_env_map = {
            "backend": "TERMINAL_ENV",
            "env_type": "TERMINAL_ENV",  # Legacy key
            "cwd": "TERMINAL_CWD",
        ...
        for cfg_key, env_var in terminal_env_map.items():
            if cfg_key == "cwd" and cwd is not None:
                os.environ[env_var] = str(cwd)
            elif cfg_key in terminal_cfg:
    ...
    # Bridge auxiliary model configuration
    ...
    if auxiliary_cfg and isinstance(auxiliary_cfg, dict):
        ...
        for task, mapping in aux_task_env.items():
            task_cfg = auxiliary_cfg.get(task, {})
            if task_cfg and isinstance(task_cfg, dict):
                for cfg_key, env_var in mapping.items():
                    val = task_cfg.get(cfg_key)
                    if val:
                        os.environ[env_var] = str(val)
    
    # Bridge agent configuration (gateway-specific)
    agent_cfg = config.get("agent", {})
    if agent_cfg and isinstance(agent_cfg, dict):
        if "max_turns" in agent_cfg:
            os.environ["KUNMING_MAX_ITERATIONS"] = str(agent_cfg["max_turns"])
        # Bridge agent.gateway_timeout -> KUNMING_AGENT_TIMEOUT env var.
        # Env var from .env takes precedence (already in os.environ).
        if "gateway_timeout" in agent_cfg and "KUNMING_AGENT_TIMEOUT" not in os.environ:
            os.environ["KUNMING_AGENT_TIMEOUT"] = str(agent_cfg["gateway_timeout"])
        if "gateway_timeout_warning" in agent_cfg and "KUNMING_AGENT_TIMEOUT_WARNING" not in os.environ:
            os.environ["KUNMING_AGENT_TIMEOUT_WARNING"] = str(agent_cfg["gateway_timeout_warning"])
    
    # Bridge timezone configuration
    tz_cfg = config.get("timezone", "")
    if tz_cfg and isinstance(tz_cfg, str) and "KUNMING_TIMEZONE" not in os.environ:
        os.environ["KUNMING_TIMEZONE"] = tz_cfg.strip()
...
#   openai-codex (OAuth —km auth) —OpenAI Codex
#   nous         (OAuth —km auth) —Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.kunming/config.yaml."""
    if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.kunming/.env."""
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Kunming env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.kunming/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.kunming/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.kunming/.env and os.environ.
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.kunming/.env or environment."""
...
def show_config():
    ...
    print(color("|             ⚙ Kunming Configuration                                         |", Colors.CYAN))
    print(color("+-----------------------------------------------------------------------------+", Colors.CYAN))
    ...
    print(color("  km config edit     # Edit config file", Colors.DIM))
    print(color("  km config set <key> <value>", Colors.DIM))
    print(color("  km setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: km config set <key> <value>")
            print()
            print("Examples:")
            print("  km config set model anthropic/claude-sonnet-4")
            print("  km config set terminal.backend docker")
            print("  km config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        print()
        print(color("ð Checking configuration for updates...", Colors.CYAN, Colors.BOLD))
        print()
        ...
        if optional_missing:
            print(f"\n  ℹ  {len(optional_missing)} optional API key(s) not configured:")
        ...
        if missing_config:
            ...
            print("    Run 'km config migrate' to add them")
        ...
        print("  km config           Show current configuration")
        print("  km config edit      Open config file in editor")
        print("  km config set <key> <value>   Set a config value")
        print("  km config check     Check for missing/outdated config")
        print("  km config migrate   Update config with new options")
        print("  km config path      Show config file path")
        print("  km config env-path  Show .env file path")
        sys.exit(1)
