"""
Single source of truth for provider identity in Kunming Agent.
...
2. **Kunming overlays** — transport type, auth patterns, aggregator flags,
...
# -- Kunming overlay ----------------------------------------------------------
# Kunming-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class KunmingOverlay:
    """Kunming-specific provider metadata layered on top of models.dev."""
...
KUNMING_OVERLAYS: Dict[str, KunmingOverlay] = {
    "openrouter": KunmingOverlay(
    ...
    "nous": KunmingOverlay(
        ...
        base_url_override="https://inference-api.kunming.dev/v1",
    ),
    "openai-codex": KunmingOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "qwen-oauth": KunmingOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="KUNMING_QWEN_BASE_URL",
    ),
    "copilot-acp": KunmingOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": KunmingOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": KunmingOverlay(
    ...
    "kimi-for-coding": KunmingOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "minimax": KunmingOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-cn": KunmingOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": KunmingOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": KunmingOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "vercel": KunmingOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ),
    "opencode": KunmingOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": KunmingOverlay(
        transport="openai_chat",
        is_aggregator=True,
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "kunming", "user-config"
...
def get_overlay(provider_id: str) -> Optional[KunmingOverlay]:
    """Get Kunming overlay for a provider, if one exists."""
    canonical = normalize_provider(provider_id)
    return KUNMING_OVERLAYS.get(canonical)
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Kunming overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Kunming overlay
    ...
    overlay = KUNMING_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + kunming extra
    ...
    if overlay is not None:
        # Kunming-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
LABELS: Dict[str, str] = {
    ...
    # Legacy Kunming IDs (point to same providers)
