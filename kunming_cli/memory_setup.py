"""km memory setup|status — configure memory provider plugins.
...
# ---------------------------------------------------------------------------
# Curses-based interactive picker (same pattern as km tools)
# ---------------------------------------------------------------------------
        ...
        def _menu(stdscr):
            ...
            while True:
                ...
                try:
                    ...
                    stdscr.addnstr(1, 0, "  ââ navigate  â?select  q quit", max_x - 1,
...
def _install_dependencies(provider_name: str) -> None:
    ...
    if not uv_path:
        ...
        print(f"  Then re-run: km memory setup")
        return
    ...
    try:
        ...
        print(f"  [ON]Failed to install {', '.join(missing)}")
...
def cmd_setup_provider(provider_name: str) -> None:
    ...
    if not match:
        ...
        print("  Run 'km memory setup' to see available providers.\n")
        return
    ...
    if hasattr(provider, "post_setup"):
        ...
        provider.post_setup(kunming_home, config)
        return
...
def cmd_setup(args) -> None:
    ...
    if not providers:
        ...
        print("  Install a plugin to ~/.kunming/plugins/ and try again.\n")
        return
    ...
    if provider_config and hasattr(provider, "save_config"):
        try:
            provider.save_config(provider_config, kunming_home)
        except Exception as e:
...
def cmd_status(args) -> None:
    ...
    if provider_name:
        ...
        if found:
            ...
            print(f"  Install the '{provider_name}' memory plugin to ~/.kunming/plugins/")
