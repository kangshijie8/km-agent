"""
MCP Server Management CLI -``kunming mcp`` subcommand.

Implements ``kunming mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.kunming/config.yaml under the ``mcp_servers`` key.
"""
...
from kunming_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_kunming_home,  # noqa: F401 -used by test mocks
)
...
def cmd_mcp_add(args):
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  kunming mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  kunming mcp add github --command npx --args @modelcontextprotocol/server-github')
        return
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_kunming_home()}/.env as {env_key}")
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            _info("Fix the issue, then: kunming mcp test " + name)
        return
    ...
    print()
    _success(f"Saved '{name}' to {display_kunming_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  kunming mcp add <name> --url <endpoint>')
        _info('  kunming mcp add <name> --command <cmd> --args <args...>')
        print()
        return
    ...
    for name, cfg in servers.items():
        ...
        status = color("[OK]enabled", Colors.GREEN) if enabled else color("[DISABLED]", Colors.DIM)
...
# --- kunming mcp configure ----------------------------------------------------

def cmd_mcp_configure(args):
    """Reconfigure which tools are enabled for an existing MCP server."""
    import sys as _sys
    if not _sys.stdin.isatty():
        print("Error: 'kunming mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``kunming mcp`` subcommands."""
    ...
    if handler:
        ...
        _info("kunming mcp serve                              Run as MCP server")
        _info("kunming mcp add <name> --url <endpoint>        Add an MCP server")
        _info("kunming mcp add <name> --command <cmd>         Add a stdio server")
        _info("kunming mcp remove <name>                      Remove a server")
        _info("kunming mcp list                               List servers")
        _info("kunming mcp test <name>                        Test connection")
        _info("kunming mcp configure <name>                   Toggle tools")
        print()
