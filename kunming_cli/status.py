"""Status command for km CLI.

Shows the status of all Kunming Agent components."""
...
def show_status(args):
    """Show status of all Kunming Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)
    
    print()
    print(color("â---------------------------------------------------------+", Colors.CYAN))
    print(color("+                [ON]Kunming Agent Status                  +", Colors.CYAN))
    print(color("â---------------------------------------------------------+", Colors.CYAN))
    ...
    # =========================================================================
    print()
    print(color("oEnvironment", Colors.CYAN, Colors.BOLD))
    ...
    # =========================================================================
    print()
    print(color("oAPI Keys", Colors.CYAN, Colors.BOLD))
    ...
    try:
        from kunming_cli.auth import get_nous_auth_status, get_codex_auth_status, get_qwen_auth_status
    ...
    print(
        ...
        f"{'logged in' if nous_logged_in else 'not logged in (run: km model)'}"
    )
    if nous_logged_in:
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: km model)'}"
    )
    ...
    if managed_nous_tools_enabled():
        ...
        print()
        print(color("oNous Subscription Features", Colors.CYAN, Colors.BOLD))
        if not features.nous_auth_present:
            print("  Nous Portal   [OK]not logged in")
        else:
    ...
    # =========================================================================
    print()
    print(color("oAPI-Key Providers", Colors.CYAN, Colors.BOLD))
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: km model)"
    ...
    # =========================================================================
    print()
    print(color("oTerminal Backend", Colors.CYAN, Colors.BOLD))
    
    terminal_env = os.getenv("TERMINAL_ENV", "")
    if not terminal_env:
        # Fall back to config file value when env var isn't set
        # (km status doesn't go through cli.py's config loading)
        try:
            _cfg = load_config()
    ...
    # =========================================================================
    print()
    print(color("oMessaging Platforms", Colors.CYAN, Colors.BOLD))
    ...
    if sys.platform.startswith('linux'):
        ...
        from kunming_cli.gateway import get_launchd_label
        try:
            result = subprocess.run(
    ...
    # =========================================================================
    print()
    print(color("oScheduled Jobs", Colors.CYAN, Colors.BOLD))
    
    jobs_file = get_kunming_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    # =========================================================================
    print()
    print(color("oSessions", Colors.CYAN, Colors.BOLD))
    
    sessions_file = get_kunming_home() / "sessions" / "sessions.json"
    ...
    if deep:
        print()
        print(color("oDeep Checks", Colors.CYAN, Colors.BOLD))
    ...
    print(color("  Run 'km doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'km setup' to configure", Colors.DIM))
    print()
