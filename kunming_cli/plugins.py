"""
Kunming Plugin System
====================

Discovers, loads, and manages plugins from three sources:

1. **User plugins**   — ``~/.kunming/plugins/<name>/``
2. **Project plugins** — ``./.kunming/plugins/<name>/`` (opt-in via
   ``KUNMING_ENABLE_PROJECT_PLUGINS``)
3. **Pip plugins**     — packages that expose the ``kunming_agent.plugins``
   entry-point group.
...
ENTRY_POINTS_GROUP = "kunming_agent.plugins"

_NS_PARENT = "kunming_plugins"
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``km honcho ...``).
...
class PluginManager:
    ...
    def discover_and_load(self) -> None:
        ...
        # 1. User plugins (~/.kunming/plugins/)
        user_dir = get_kunming_home() / "plugins"
        manifests.extend(self._scan_directory(user_dir, source="user"))

        # 2. Project plugins (./.kunming/plugins/)
        if _env_enabled("KUNMING_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".kunming" / "plugins"
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``kunming_plugins.<name>``."""
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``km tools`` TUI so plugin-provided toolsets appear
    ...
    for ts_key in sorted(toolset_tools):
        ...
        label = f"ð {ts_key.replace('_', ' ').title()}"
