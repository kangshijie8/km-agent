...
import kunming_cli.auth as auth_mod
...
def auth_add_command(args) -> None:
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_kunming_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:kunming_pkce",
    ...
    raise SystemExit(f"`kunming auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    if removed.source.startswith("env:"):
        ...
        if env_var:
            from kunming_cli.config import remove_env_value
            cleared = remove_env_value(env_var)
            if cleared:
                print(f"Cleared {env_var} from .env")

    # If this was a singleton-seeded credential (OAuth device_code, kunming_pkce),
    ...
    elif removed.source == "kunming_pkce" and provider == "anthropic":
        from kunming_constants import get_kunming_home
        oauth_file = get_kunming_home() / ".anthropic_oauth.json"
        if oauth_file.exists():
            oauth_file.unlink()
            print("Cleared Kunming Anthropic OAuth credentials")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `kunming auth` is called bare."""
