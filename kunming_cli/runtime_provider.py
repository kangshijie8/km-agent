...
from kunming_cli import auth as auth_mod
...
def _copilot_runtime_api_mode(model_cfg: Dict[str, Any], api_key: str) -> str:
    ...
    try:
        from kunming_cli.models import copilot_model_api_mode
    ...
    *,
    ...
    if provider == "openai-codex":
        ...
        if configured_mode and _provider_supports_explicit_api_mode(provider, configured_provider):
            ...
            from kunming_cli.models import opencode_model_api_mode
...
def resolve_requested_provider(requested: Optional[str] = None) -> str:
    ...
    if isinstance(cfg_default, str) and "/" in cfg_default:
        prefix = cfg_default.split("/", 1)[0].strip().lower()
        if prefix and prefix in PROVIDER_REGISTRY:
            return prefix

    env_provider = os.getenv("KUNMING_INFERENCE_PROVIDER", "").strip().lower()
    if env_provider:
        return env_provider
    ...
    if provider == "nous":
        try:
            ...
            if requested_provider != "auto":
                raise
            # Auto-detected Nous but credentials are stale/revoked -            # fall through to env-var providers (e.g. OpenRouter).
    ...
    if provider == "openai-codex":
        try:
            ...
            return {
                ...
                "base_url": creds.get("base_url", "").rstrip("/"),
                "api_key": creds.get("api_key", ""),
                "source": creds.get("source", "kunming-auth-store"),
                "last_refresh": creds.get("last_refresh"),
                "requested_provider": requested_provider,
            }
        except AuthError:
            if requested_provider != "auto":
                raise
            # Auto-detected Codex but credentials are stale/revoked -            # fall through to env-var providers (e.g. OpenRouter).
