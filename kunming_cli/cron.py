"""
Cron subcommand for kunming CLI.
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        ...
        print(color("Create one with 'km cron create ...' or the /cron command in chat.", Colors.DIM))
        return
    ...
    if not find_gateway_pids():
        ...
        print(color("     Start it with: km gateway install", Colors.DIM))
        print(color("                    sudo km gateway install --system  # Linux servers", Colors.DIM))
        print()
...
def cron_status():
    ...
    if pids:
        ...
        print("    km gateway install    # Install as a user service")
        print("    sudo km gateway install --system  # Linux servers: boot-time system service")
        print("    km gateway            # Or run in foreground")
...
def cron_command(args):
    ...
    print("Usage: km cron [list|create|edit|pause|resume|run|remove|status|tick]")
    sys.exit(1)
