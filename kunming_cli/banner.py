...
Pure display functions with no KunmingCLI state dependency.
"""
...
def check_for_updates() -> Optional[int]:
    ...
    ``~/.kunming/.update_check``).  Returns the number of commits behind,
    or ``None`` if the check fails or isn't applicable.
    """
    kunming_home = get_kunming_home()
    repo_dir = kunming_home / "kunming-agent"
    cache_file = kunming_home / ".update_check"
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Kunming git checkout, or None if this isn't a git install."""
    kunming_home = get_kunming_home()
    repo_dir = kunming_home / "kunming-agent"
    if not (repo_dir / ".git").exists():
        repo_dir = Path(__file__).parent.parent.resolve()
...
def format_banner_version_label() -> str:
    ...
    return f"km agent v{VERSION} ({RELEASE_DATE})"
...
# Toolset name mapping (English -> Chinese)
_TOOLSET_NAME_MAP = {
    "browser": "浏览器",
    "clarify": "确认",
    "code_execution": "代码执行",
...
def _display_toolset_name(toolset_name: Optional[str]) -> str:
    """Normalize internal/legacy toolset identifiers for banner display."""
    if not toolset_name:
        return "其他"
    # Remove _tools suffix if present
    clean_name = (
        toolset_name[:-6]
        if toolset_name.endswith("_tools")
        else toolset_name
    )
    # Return Chinese name if available, otherwise return original
    return _TOOLSET_NAME_MAP.get(clean_name, clean_name)
...
def build_welcome_banner(console: Console, model: str, cwd: str,
    ...
    try:
        ...
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else KUNMING_CADUCEUS
    except Exception:
        _bskin = None
        _hero = KUNMING_CADUCEUS
    left_lines = ["", _hero, ""]
    
    # Store reference to skin for later use in logo selection
    _bskin_for_logo = _bskin
    ...
    left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} | [dim {dim}]Kunming[/]")
    ...
    for item in unavailable_toolsets:
        toolset_id = item.get("id") or item.get("name") or "other"
        ...
        for srv in mcp_status:
            if srv["connected"]:
                right_lines.append(
                    ...
                    f"[dim {dim}]â[/] [{text}]{_T('banner.tools_count', count=srv['tools'])}[/]"
                )
            else:
                right_lines.append(
    ...
    summary_parts = [f"🔧 {_T('banner.tools_count', count=len(tools))}", f"🎯 {_T('banner.skills_count', count=total_skills)}"]
    if mcp_connected:
        summary_parts.append(f"🔗 {_T('banner.mcp_count', count=mcp_connected)}")
    summary_parts.append(f"💡 {_T('banner.help_command')}")
    ...
    try:
        ...
        if behind and behind > 0:
            ...
            right_lines.append(
                f"[bold yellow][ON]{behind} {commits_word} {_T('banner.commits_behind')}[/]"
                f"[dim yellow] -run [bold]{recommended_update_command()}[/bold] {_T('banner.to_update')}[/]"
            )
    except Exception:
    ...
    agent_name = _skin_branding("agent_name", "km agent")
