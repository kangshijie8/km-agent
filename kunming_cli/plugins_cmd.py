"""``km plugins`` CLI subcommand — install, update, remove, and list plugins.

Plugins are installed from Git repositories into ``~/.kunming/plugins/``.
...
def _plugins_dir() -> Path:
    ...
    plugins = get_kunming_home() / "plugins"
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    from kunming_cli.config import get_env_value, save_env_value  # noqa: F811
    ...
    for spec in missing:
        ...
        try:
            ...
            console.print(f"\n[dim]  Skipped (you can set these later in {display_kunming_home()}/.env)[/dim]")
            return

        if value:
            save_env_value(name, value)
            os.environ[name] = value
            console.print(f"  [green]â[/green] Saved to {display_kunming_home()}/.env")
        else:
            console.print(f"  [dim]  Skipped (set {name} in {display_kunming_home()}/.env later)[/dim]")
...
def _display_removed(name: str, plugins_dir: Path) -> None:
    ...
    console = Console()
    console.print()
    console.print(f"[red]â[/red] Plugin [bold]{name}[/bold] removed from {plugins_dir}")
    console.print()
...
def cmd_install(identifier: str, force: bool = False) -> None:
        ...
        if target.exists():
            if not force:
                console.print(
                    ...
                    f"[bold]km plugins update {plugin_name}[/bold] to pull latest."
                )
                sys.exit(1)
    ...
    if not (target / "plugin.yaml").exists() and not (target / "__init__.py").exists():
        console.print(
            ...
            f"or __init__.py. It may not be a valid Kunming plugin."
        )
    ...
    console.print("[dim]  km gateway restart[/dim]")
    console.print()
...
def cmd_update(name: str) -> None:
    ...
    if "Already up to date" in output:
        console.print(
            f"[green]â[/green] Plugin [bold]{name}[/bold] is already up to date."
        )
    else:
        console.print(f"[green]â[/green] Plugin [bold]{name}[/bold] updated.")
...
def cmd_enable(name: str) -> None:
    ...
    console.print(f"[green]â[/green] Plugin [bold]{name}[/bold] enabled. Takes effect on next session.")
...
def cmd_disable(name: str) -> None:
    ...
    disabled.add(name)
    _save_disabled_set(disabled)
    console.print(f"[yellow]â[/yellow] Plugin [bold]{name}[/bold] disabled. Takes effect on next session.")
...
def cmd_list() -> None:
    ...
    if not dirs:
        ...
        console.print("[dim]Install with:[/dim] km plugins install owner/repo")
        return
    ...
    for d in dirs:
        ...
        # Check if it's a git repo (installed via km plugins install)
        if (d / ".git").exists():
            source = "git"
    ...
    console.print(table)
    console.print()
    console.print("[dim]Interactive toggle:[/dim] km plugins")
    console.print("[dim]Enable/disable:[/dim] km plugins enable/disable <name>")
...
def cmd_toggle() -> None:
    ...
    if new_disabled != disabled:
        ...
        console.print(
            f"\n[green]â[/green] {enabled_count} enabled, {len(new_disabled)} disabled. "
...
def plugins_command(args) -> None:
    """Dispatch km plugins subcommands."""
