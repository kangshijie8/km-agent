"""
Profile management for multiple isolated Kunming instances.

Each profile is a fully independent KUNMING_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.kunming/profiles/<name>/`` by default.

The "default" profile is ``~/.kunming`` itself -backward compatible,
zero migration needed.

Usage::

    km profile create coder          # fresh profile + bundled skills
    km profile create coder --clone  # also copy config, .env, SOUL.md
    km profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    km -p coder chat                 # or via flag
    km profile use coder             # set as sticky default
    km profile delete coder          # remove profile + alias + service
"""
...
# Directories/files to exclude when exporting the default (~/.kunming) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "kunming-agent",         # repo checkout (multi-GB)
...
_RESERVED_NAMES = frozenset({
    "kunming", "default", "test", "tmp", "root", "sudo",
})

# Kunming subcommands that cannot be used as profile names/aliases
_KUNMING_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Always ``~/.kunming/profiles/`` -anchored to the user's home,
...
def _get_active_profile_path() -> Path:
    ...
    return _get_default_kunming_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name == "default":
        return  # special alias for ~/.kunming
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its KUNMING_HOME directory."""
    if name == "default":
        return _get_default_kunming_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, kunming subcommands, existing binaries in PATH.
    """
    if name in _RESERVED_NAMES:
        return f"'{name}' is a reserved name"
    if name in _KUNMING_SUBCOMMANDS:
        return f"'{name}' conflicts with a km subcommand"
    ...
    try:
        result = subprocess.run(
            ...
            ["where", name], capture_output=True, text=True, timeout=5, shell=True,
        )
...
def create_wrapper_script(name: str) -> Optional[Path]:
    ...
    try:
        ...
        print(f"[ON]Could not create {wrapper_dir}: {e}")
        return None
    ...
    try:
        ...
        print(f"[ON]Could not create wrapper at {wrapper_path}: {e}")
        return None
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_kunming_home()
...
def create_profile(
    ...
    if name == "default":
        raise ValueError(
            "Cannot create a profile named 'default' -it is the built-in profile (~/.kunming)."
        )
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches KUNMING_HOME at module level.
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "KUNMING_HOME": str(profile_dir)},
        ...
        if not quiet:
            print(f"[ON]Skill seeding returned exit code {result.returncode}")
            if result.stderr.strip():
                print(f"  {result.stderr.strip()[:200]}")
        return None
    except subprocess.TimeoutExpired:
        if not quiet:
            print("[ON]Skill seeding timed out (60s)")
        return None
    except Exception as e:
        if not quiet:
            print(f"[ON]Skill seeding failed: {e}")
        return None
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if name == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.kunming).\n"
            "To remove everything, use: km uninstall"
        )
    ...
    for item in items:
        print(f"  -{item}")
    if gw_running:
        print(f"  [ON]Gateway is running -it will be stopped.")
    ...
    try:
        ...
        print(f"[ON]Could not remove {profile_dir}: {e}")
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set KUNMING_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("KUNMING_HOME")
    try:
        os.environ["KUNMING_HOME"] = str(profile_dir)
        from kunming_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        print(f"[ON]Service cleanup: {e}")
    finally:
        if old_home is not None:
            os.environ["KUNMING_HOME"] = old_home
        elif "KUNMING_HOME" in os.environ:
            del os.environ["KUNMING_HOME"]
...
def _stop_gateway_process(profile_dir: Path) -> None:
    ...
    try:
        ...
        data = json.loads(raw) if raw.startswith("{") else {"pid": int(raw)}
        pid = int(data["pid"])
        os.kill(pid, _signal.SIGTERM if sys.platform != "win32" else _signal.CTRL_BREAK_EVENT)
        ...
        try:
            if sys.platform == "win32":
                # Windows doesn't have SIGKILL, use ctypes
                import ctypes
        ...
        print(f"[ON]Could not stop gateway: {e}")
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.kunming/active_profile``. Use ``"default"`` to clear.
    """
    validate_profile_name(name)
    if name != "default" and not profile_exists(name):
        raise FileNotFoundError(
            f"Profile '{name}' does not exist. "
            f"Create it with: km profile create {name}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from KUNMING_HOME.

    Returns ``"default"`` if KUNMING_HOME is not set or points to ``~/.kunming``.
    Returns the profile name if KUNMING_HOME points into ``~/.kunming/profiles/<name>``.
    Returns ``"custom"`` if KUNMING_HOME is set to an unrecognized path.
    """
    from kunming_constants import get_kunming_home
    kunming_home = get_kunming_home()
    resolved = kunming_home.resolve()

    default_resolved = _get_default_kunming_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if name == "default":
        # The default profile IS ~/.kunming itself — its parent is ~/ and its
        # directory name is ".kunming", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: km profile import <archive> --name <name>"
        )

    # Archives exported from the default profile have "default/" as top-level
    # dir.  Importing as "default" would target ~/.kunming itself -disallow
    # that and guide the user toward a named profile.
    if inferred_name == "default":
        raise ValueError(
            "Cannot import as 'default' -that is the built-in root profile (~/.kunming). "
            "Specify a different name: km profile import <archive> --name <name>"
        )
...
def rename_profile(old_name: str, new_name: str) -> Path:
    ...
    if not collision:
        ...
        print(f"[ON]Cannot create alias '{new_name}' -{collision}")
...
def generate_bash_completion() -> str:
    """Generate a bash completion script for km profile names."""
    return '''# Kunming Agent profile completion
# Add to ~/.bashrc: eval "$(km completion bash)"

_kunming_profiles() {
    local profiles_dir="$HOME/.kunming/profiles"
...
_kunming_completion() {
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_kunming_profiles)" -- "$cur"))
        return
    fi
...
complete -F _kunming_completion km
'''


def generate_zsh_completion() -> str:
    """Generate a zsh completion script for km profile names."""
    return '''#compdef km
# Kunming Agent profile completion
# Add to ~/.zshrc: eval "$(km completion zsh)"

_kunming() {
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.kunming/profiles" ]]; then
        profiles+=("${(@f)$(ls $HOME/.kunming/profiles 2>/dev/null)}")
    fi
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a KUNMING_HOME path string.

    Called early in the CLI entry point, before any kunming modules
    are imported, to set the KUNMING_HOME environment variable.
    """
    ...
    if profile_name != "default" and not profile_dir.is_dir():
        raise FileNotFoundError(
            ...
            f"Create it with: km profile create {profile_name}"
        )
