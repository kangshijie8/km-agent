...
Add, remove, or reorder entries here both `km setup` and
`km` provider-selection will pick up the change automatically.
"""
...
def fetch_nous_account_tier(access_token: str, portal_base_url: str = "") -> dict[str, Any]:
    ...
    """
    base = (portal_base_url or "https://km.dev").rstrip("/")
...
def check_nous_free_tier() -> bool:
    ...
    try:
        from kunming_cli.auth import get_provider_auth_state, resolve_nous_runtime_credentials
...
# 整合: 删除本地 _PROVIDER_ALIASES，从 kunming_constants 导入统一版本 [M3]
...
def get_pricing_for_provider(provider: str) -> dict[str, dict[str, str]]:
    ...
    if normalized == "nous":
        ...
        if base_url:
            # Nous base_url typically looks like https://inference-api.kunming.dev/v1
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    for pid in _PROVIDER_ORDER:
        ...
        has_creds = False
        try:
            from kunming_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Kunming's canonical provider ids.

    Note: ``"auto"`` passes through unchanged use
    ``kunming_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    try:
        from kunming_cli.auth import resolve_api_key_provider_credentials
...
def provider_model_ids(provider: Optional[str]) -> list[str]:
    ...
    if normalized == "nous":
        ...
        try:
            from kunming_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
...
def copilot_default_headers() -> dict[str, str]:
    ...
    """
    try:
        from kunming_cli.copilot_auth import copilot_request_headers
        return copilot_request_headers(is_agent_turn=True)
    except ImportError:
        return {
            "Editor-Version": COPILOT_EDITOR_VERSION,
            "User-Agent": "KunmingAgent/1.0",
...
def _fetch_ai_gateway_models(timeout: float = 5.0) -> Optional[list[str]]:
    ...
    if not base_url:
        from kunming_constants import AI_GATEWAY_BASE_URL
    ...
    *,
    ...
    if normalized == "custom":
        ...
        message = (
            ...
            f"Kunming will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
    ...
    if api_models is not None:
        if requested_for_lookup in set(api_models):
            ...
            # API responded but model is not listed.  Accept anyway             # the user may have access to models not shown in the public
