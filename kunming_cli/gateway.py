"""
Gateway subcommand for km CLI.

Handles: km gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    scope_args + ["list-units", "kunming-gateway*",
...
def find_gateway_pids(exclude_pids: set | None = None) -> list:
    ...
    patterns = [
        "kunming_cli.main gateway",
        "kunming_cli/main.py gateway",
        "km gateway",
        "gateway/run.py",
    ]
...
def kill_gateway_processes(force: bool = False, exclude_pids: set | None = None) -> int:
    ...
    for pid in pids:
        try:
            ...
            # Windows may raise OSError for invalid signal
            if is_windows() and force:
                try:
                    # Fallback: use ctypes to force terminate on Windows
                    import ctypes
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (KUNMING_HOME-scoped).
...
_SERVICE_BASE = "kunming-gateway"
SERVICE_DESCRIPTION = "Kunming Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current KUNMING_HOME.

    Returns ``""`` for the default ``~/.kunming``, the profile name for
    ``~/.kunming/profiles/<name>``, or a short hash for any other custom
    KUNMING_HOME path.
    """
    import hashlib
    import re
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary KUNMING_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(kunming_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when KUNMING_HOME is a named profile.

    For ``~/.kunming/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        kunming_home: Optional explicit KUNMING_HOME path. Defaults to the current
            ``get_kunming_home()`` value. Should be passed when generating a
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this KUNMING_HOME.

    Default ``~/.kunming`` returns ``kunming-gateway`` (backward compatible).
    Profile ``~/.kunming/profiles/coder`` returns ``kunming-gateway-coder``.
    Any other KUNMING_HOME appends a short hash for uniqueness.
    """
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    km gateway uninstall")
    print_info("    sudo km gateway uninstall --system")
...
def _system_service_identity(run_as_user: str | None = None) -> tuple[str, str, str]:
    ...
    if user_info is not None and grp is not None:
        group_name = grp.getgrgid(user_info.pw_gid).gr_name
        home_dir = user_info.pw_dir
    else:
        group_name = "users"
        home_dir = str(Path.home())
    return username, group_name, home_dir
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if is_windows() or os.geteuid() != 0:
            print_warning("  System service install requires sudo, so Kunming can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo km gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo km gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo km gateway start --system")
            return scope, False
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.kunming`` ``ai.kunming.gateway.plist`` (backward compatible).
    Profile ``~/.kunming/profiles/coder`` ``ai.kunming.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.kunming.gateway-{suffix}" if suffix else "ai.kunming.gateway"
...
def get_kunming_cli_path() -> str:
    """Get the path to the kunming CLI."""
    ...
    return f"{get_python_path()} -m kunming_cli.main"
...
def _kunming_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current KUNMING_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_kunming_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.kunming                    /home/alice/.kunming
      /root/.kunming/profiles/coder     /home/alice/.kunming/profiles/coder
    ...
    if current_kunming == current_default:
        return str(target_default)

    # Profile or subdir of ~/.kunming preserve the relative structure
    try:
        relative = current_kunming.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.kunming) keep as-is
        return str(current_kunming)
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        kunming_home = _kunming_home_for_target_user(home_dir)
        profile_arg = _profile_arg(kunming_home)
...
User={username}
Group={group_name}
ExecStart={python_path} -m kunming_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
Environment="KUNMING_HOME={kunming_home}"
Restart=on-failure
RestartSec=30
KillMode=mixed
KillSignal=SIGTERM
...
"""
    ...
    kunming_home = str(get_kunming_home().resolve())
    profile_arg = _profile_arg(kunming_home)
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"Updated gateway {_service_scope_label(system)} service definition to match the current Kunming install")
    return True
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}km gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}km gateway status{scope_flag}             # Check status")
...
def systemd_status(deep: bool = False, system: bool = False):
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}km gateway install{scope_flag}")
        return
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}km gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}km gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.kunming.gateway-{suffix}" if suffix else "ai.kunming.gateway"
...
def generate_launchd_plist() -> str:
    ...
    prog_args = [
        ...
        "<string>-m</string>",
        "<string>kunming_cli.main</string>",
    ]
    if profile_arg:
...
<dict>
    ...
    <dict>
        ...
        <key>KUNMING_HOME</key>
        <string>{kunming_home}</string>
    </dict>
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("Updated gateway launchd service definition to match the current Kunming install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  km gateway status             # Check status")
        ...
        if not force_sent and time.monotonic() >= force_deadline:
            ...
            try:
                if is_windows():
                    ...
                    os.kill(pid, signal.SIGKILL)
                print(f"Gateway PID {pid} did not exit gracefully; force-terminated")
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("Service definition matches the current Kunming install")
    else:
        print("Service definition is stale relative to the current Kunming install")
        print("  Run: km gateway start")
    ...
    if deep:
        log_file = get_kunming_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    print("+---------------------------------------------------------")
    print("          Kunming Gateway Starting...                 ")
    print("+---------------------------------------------------------")
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and Kunming will log in directly",
        ...
        "vars": [
            ...
            {"name": "MATRIX_USER_ID", "prompt": "User ID (@bot:server required for password login)", "password": False,
             "help": "Full Matrix user ID, e.g. @kunming:matrix.example.org"},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. kunming) and copy the bot token",
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where Kunming delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your Kunming agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address Kunming will use (e.g., kunming@gmail.com)."},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. Kunming connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: km pairing generate bluebubbles",
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_kunming_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'km pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: km pairing approve {platform} {code}")
                else:
                    print_info("  Skipped configure later with 'km gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from kunming_cli.main import cmd_whatsapp
    import argparse
...
def _setup_signal():
    ...
    print()
    print(color("  --- [HOME] Signal Setup ---", Colors.CYAN))
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"KunmingAgent\"")
    ...
    if any_configured:
        ...
        if service_running:
            if prompt_yes_no("  Restart the gateway to pick up changes?", True):
                try:
                    if is_linux():
                        ...
                        print_info("Start manually: km gateway")
                except subprocess.CalledProcessError as e:
            ...
            if is_linux() or is_macos():
                ...
                print_info("  Run in foreground: km gateway")
    else:
        print()
        print_info("No platforms configured. Run 'km gateway setup' when ready.")
...
def gateway_command(args):
    ...
    if subcmd == "install":
        ...
        if is_linux():
            ...
            print("Run manually: km gateway run")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: km gateway start")
                sys.exit(1)
        ...
        if is_linux() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                print("  km gateway install")
                print("  sudo km gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  km gateway          # Run in foreground")
                print("  km gateway install  # Install as user service")
                print("  sudo km gateway install --system  # Install as boot-time system service")
