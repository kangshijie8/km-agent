"""Slash command definitions and autocomplete for the Kunming CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("distill", "Run memory distillation (consolidate short-term signals into long-term memory)",
               "Tools & Skills", aliases=("dream",), args_hint="[status|run]"),
    ...
    CommandDef("paste", "Check clipboard for an image and attach it", "Info",
               cli_only=True),
    CommandDef("update", "Update Kunming Agent to the latest version", "Info",
               gateway_only=True),
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``kunming skills
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /kunming handler.

    Maps both canonical names and aliases so /kunming bg do stuff works
    the same as /kunming background do stuff.
    """
    overrides = _resolve_config_gates()
