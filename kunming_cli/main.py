#!/usr/bin/env python3
"""
Kunming CLI - Main entry point.

Usage:
    km                         # Interactive chat (default)
    km chat                    # Interactive chat
    km gateway                 # Run gateway in foreground
    km gateway start           # Start gateway as service
    km gateway stop            # Stop gateway service
    km gateway status          # Show gateway status
    km gateway install         # Install gateway service
    km gateway uninstall       # Uninstall gateway service
    km setup                   # Interactive setup wizard
    km logout                  # Clear stored authentication
    km status                  # Show status of all components
    km cron                    # Manage cron jobs
    km cron list               # List cron jobs
    km cron status             # Check if cron scheduler is running
    km doctor                  # Check configuration and dependencies
    km honcho setup                    # Configure Honcho AI memory integration
    km honcho status                   # Show Honcho config and connection status
    km honcho sessions                 # List directory ?session name mappings
    km honcho map <name>               # Map current directory to a session name
    km honcho peer                     # Show peer names and dialectic settings
    km honcho peer --user NAME         # Set user peer name
    km honcho peer --ai NAME           # Set AI peer name
    km honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    km honcho mode                     # Show current memory mode
    km honcho mode [hybrid|honcho|local]  # Set memory mode
    km honcho tokens                   # Show token budget settings
    km honcho tokens --context N       # Set session.context() token cap
    km honcho tokens --dialectic N     # Set dialectic result char cap
    km honcho identity                 # Show AI peer identity representation
    km honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    km version                 Show version
    km update                  Update to latest version
    km uninstall               Uninstall Kunming Agent
    km acp                     Run as an ACP server for editor integration
    km sessions browse         Interactive session picker with search
"""
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (km tools, km setup, km model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'km {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override ?MUST happen before any kunming module import.
#
# Many modules cache KUNMING_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("KUNMING_HOME", ...)`` resolves correctly.
...
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set KUNMING_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # 2. If no flag, check ~/.kunming/active_profile
    if profile_name is None:
        try:
            active_path = _get_default_kunming_home() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set KUNMING_HOME
    if profile_name is not None:
        try:
            from kunming_cli.profiles import resolve_profile_env
            kunming_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent kunming from starting
            print(f"Warning: profile override failed ({exc}), using default", file=sys.stderr)
            return
        os.environ["KUNMING_HOME"] = kunming_home
...
# Initialize centralized file logging early ?all `km` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from kunming_logging import setup_logging as _setup_logging
...
from kunming_cli import __version__, __release_date__
...
def _has_any_provider_configured() -> bool:
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_kunming_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    auth_file = get_kunming_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if Kunming has been explicitly configured ?Claude Code
    # being installed doesn't mean the user wants Kunming to use their tokens.
    if _has_kunming_config:
        try:
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'km sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print("It looks like Kunming isn't configured yet -- no API keys or providers found.")
        print()
        print("  Run:  km setup")
        print()

        from kunming_cli.setup import is_interactive_stdin, print_noninteractive_setup_guidance
        ...
        if reply in ("", "y", "yes"):
            ...
            return
        print()
        print("You can run 'km setup' at any time to configure.")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from kunming_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["KUNMING_YOLO_MODE"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["KUNMING_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from kunming_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    import subprocess
    from pathlib import Path
    from kunming_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with Kunming?")
        print()
    ...
    session_dir = get_kunming_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("  Start the gateway with: km gateway")
            return
    ...
    print("-" * 50)
    if wa_mode == "bot":
        print("[MOBILE] Open WhatsApp (or WhatsApp Business) on the")
        print("   phone with the BOT's number, then scan:")
    else:
        print("[MOBILE] Open WhatsApp on your phone, then scan:")
    print()
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  km gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '?Kunming Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  km gateway")
        ...
        print()
        print("  Or install as a service: km gateway install")
    else:
        print("?Pairing may not have completed. Run 'km whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    _require_tty("setup")
    from kunming_cli.setup import run_setup_wizard
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``km model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from kunming_cli.auth import (
        resolve_provider, AuthError, format_auth_error,
    )
    from kunming_cli.config import load_config, get_env_value
    ...
    effective_provider = (
        config_provider
        or os.getenv("KUNMING_INFERENCE_PROVIDER")
        or "auto"
    )
    try:
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from kunming_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from kunming_cli.auth import _prompt_model_selection, _save_model_choice, deactivate_provider
    from kunming_cli.config import get_env_value, save_env_value
    ...
    from kunming_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from kunming_cli.config import get_env_value, save_config, save_env_value
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from kunming_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from kunming_cli.codex_models import get_codex_model_ids
    import argparse
    ...
    _codex_token = None
    # Prefer credential pool (where `km auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from kunming_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from kunming_cli.models import fetch_api_models
...
def _model_flow_custom(config):
    ...
    """
    from kunming_cli.auth import _save_model_choice, deactivate_provider
    from kunming_cli.config import get_env_value, load_config, save_config
    ...
    from kunming_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `km model` to set a model.")
...
from kunming_cli.models import _PROVIDER_MODELS
...
def _model_flow_copilot(config, current_model=""):
    ...
    from kunming_cli.config import save_env_value, load_config, save_config
    from kunming_cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from kunming_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from kunming_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates Kunming turns to `copilot --acp`.")
    print("  Kunming currently starts its own ACP subprocess for each request.")
    print("  Kunming uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ?{exc}")
        print("  Set KUNMING_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere.")
        return
...
def _model_flow_kimi(config, current_model=""):
    ...
    from kunming_cli.config import get_env_value, save_env_value, load_config, save_config
    ...
    if not existing_key:
        ...
        print(f"  {pconfig.name} API key: {existing_key[:8]}... <configured>")
        print()
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    from kunming_cli.config import get_env_value, save_env_value, load_config, save_config
    from kunming_cli.models import fetch_api_models, opencode_model_api_mode, normalize_opencode_model_id
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from kunming_constants import display_kunming_home as _dhh_fn
            print(f"    Kunming will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env.")
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    if has_creds:
        # Show what we found
        if existing_key:
            print(f"  Anthropic credentials: {existing_key[:12]}... <configured>")
        elif cc_available:
    ...
    if selected:
        ...
        # providers if the user switches without running 'km model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate Kunming CLI with a provider."""
    from kunming_cli.auth import login_command
    login_command(args)


def cmd_logout(args):
    """Clear provider authentication."""
    from kunming_cli.auth import logout_command
    logout_command(args)


def cmd_auth(args):
    """Manage pooled credentials."""
    from kunming_cli.auth_commands import auth_command
    auth_command(args)


def cmd_status(args):
    """Show status of all components."""
    from kunming_cli.status import show_status
    show_status(args)


def cmd_cron(args):
    """Cron job management."""
    from kunming_cli.cron import cron_command
    cron_command(args)


def cmd_webhook(args):
    """Webhook subscription management."""
    from kunming_cli.webhook import webhook_command
    webhook_command(args)


def cmd_doctor(args):
    """Check configuration and dependencies."""
    from kunming_cli.doctor import run_doctor
    run_doctor(args)


def cmd_config(args):
    """Configuration management."""
    from kunming_cli.config import config_command
    config_command(args)


def cmd_version(args):
    """Show version."""
    print(f"km agent v{__version__} ({__release_date__})")
    ...
    try:
        from kunming_cli.banner import check_for_updates
        from kunming_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall Kunming Agent."""
    _require_tty("uninstall")
    from kunming_cli.uninstall import run_uninstall
    run_uninstall(args)
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``km update --gateway`` so interactive prompts (stash restore,
...
def _update_via_zip(args):
    """Update Kunming Agent by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = f"https://github.com/kangshijie8/KM-Agent/archive/refs/heads/{branch}.zip"
    
    print("?Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="kunming-update-")
        zip_path = os.path.join(tmp_dir, f"kunming-agent-{branch}.zip")
        ...
        # GitHub ZIPs extract to kunming-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"kunming-agent-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime("kunming-update-autostash-%Y%m%d-%H%M%S")
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if Kunming behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        do_reset = True
        if prompt_user:
            print("\nReset working tree to clean state so Kunming can run?")
    ...
    if stash_selector is None:
        print("?Local changes were restored, but Kunming couldn't find the stash entry to drop.")
        ...
        if drop.returncode != 0:
            print("?Local changes were restored, but Kunming couldn't drop the saved stash entry.")
    ...
    print("  Review `git diff` / `git status` if Kunming behaves unexpectedly.")
    return True

# =========================================================================
# Fork detection and upstream management for `km update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/kangshijie8/KM-Agent.git",
    "git@github.com:kangshijie8/Kunming-Model",
    "https://github.com/kangshijie8/KM-Agent",
    "git@github.com:kangshijie8/Kunming-Model",
}
OFFICIAL_REPO_URL = "https://github.com/kangshijie8/KM-Agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    from kunming_constants import get_kunming_home
    return (get_kunming_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        from kunming_constants import get_kunming_home
        (get_kunming_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("?Your fork is not tracking the official Kunming repository.")
        print("  This means you may miss updates from kangshijie8/Kunming-Model")
        print()
        try:
        ...
        if response in ("", "y", "yes"):
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print("  ?Added upstream: https://github.com/kangshijie8/KM-Agent.git")
                has_upstream = True
            else:
                print("  ?Failed to add upstream remote. Skipping upstream sync.")
                return
        else:
            print("  Skipped. Run 'git remote add upstream https://github.com/kangshijie8/KM-Agent.git' to add later.")
...
def _invalidate_update_cache():
    ...
    ``km update``, every profile is now current.
    """
    homes = []
...
def _load_installable_optional_extras() -> list[str]:
    ...
    # Entries look like "kunming-agent[matrix]" or "package-name[extra]".
...
def cmd_update(args):
    """Update Kunming Agent to the latest version."""
    import shutil
    from kunming_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update Kunming Agent")
        return
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            print("  curl -fsSL https://raw.githubusercontent.com/kangshijie8/KM-Agent/main/scripts/install.sh | bash")
            sys.exit(1)
    ...
    try:
        ...
        try:
            ...
            if auto_stash_ref is not None:
                # Don't attempt stash restore if the code update itself failed ?                # working tree is in an unknown state.
        ...
        # the old bytecode (e.g. get_kunming_home added to kunming_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(f"  ?Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}")
        ...
        # modules in this process.  Reload kunming_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_kunming_home() added since the last release.
        try:
            import importlib
            import kunming_constants as _hc
        ...
        try:
            from kunming_cli.profiles import list_profiles, get_active_profile_name, seed_profile_skills
        ...
        if needs_migration:
            ...
            if gateway_mode:
                ...
                print("    Run 'km config migrate' later to apply any new config/env options.")
                response = "n"
            else:
                try:
            ...
            if response in ('', 'y', 'yes'):
                ...
                if gateway_mode and missing_env:
                    print("  [!]API keys require manual entry: km config migrate")
            else:
                print()
                print("Skipped. Run 'km config migrate' later to configure.")
        else:
        ...
        try:
            ...
            # Discover all kunming-gateway* units (default + profiles)
            if is_linux():
                try:
                ...
                for scope, scope_cmd in [("user", ["systemctl", "--user"]), ("system", ["systemctl"])]:
                    try:
                        result = subprocess.run(
                            scope_cmd + ["list-units", "kunming-gateway*", "--plain", "--no-legend", "--no-pager"],
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            if not parts:
                                continue
                            unit = parts[0]  # e.g. kunming-gateway.service or kunming-gateway-coder.service
            ...
            if is_macos():
                try:
                    from kunming_cli.gateway import launchd_restart, get_launchd_label, get_launchd_plist_path
            ...
            if restarted_services or killed_pids:
                ...
                if killed_pids:
                    ...
                    print("    Restart manually: km gateway run")
                    # Also restart for each profile if needed
                    if len(killed_pids) > 1:
                        print("    (or: km -p <profile> gateway run  for each profile)")
        ...
        print("  km model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``km -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from kunming_cli.profiles import (
    ...
    if action is None:
        # Bare `km profile` ?show current profile status
        profile_name = get_active_profile_name()
        dhh = display_kunming_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} ?km -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.kunming)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(f"  Choose a custom alias:  km profile alias {name} --name <custom>")
                    print(f"  Or access via flag:     km -p {name} chat")
                else:
            ...
            if clone or clone_all:
                profile_dir_display = f"~/.kunming/profiles/{name}"
        ...
        name = args.profile_name
        from kunming_cli.profiles import get_profile_dir, profile_exists, _read_config_model, _check_gateway_running, _count_skills
        ...
        from kunming_cli.profiles import profile_exists
        if not profile_exists(name):
            print(f"Error: Profile '{name}' does not exist.")
            sys.exit(1)
        ...
        from kunming_cli.profiles import rename_profile
        try:
        ...
        from kunming_cli.profiles import export_profile
        name = args.profile_name
        ...
        from kunming_cli.profiles import import_profile
        try:
...
def cmd_completion(args):
    ...
    from kunming_cli.profiles import generate_bash_completion, generate_zsh_completion
...
def cmd_logs(args):
    """View and filter Kunming log files."""
    from kunming_cli.logs import tail_log, list_logs
...
def main():
    """Main entry point for km CLI."""
    parser = argparse.ArgumentParser(
        prog="km",
        description="km agent - AI assistant with tool-calling capabilities",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
    km                            Start interactive chat
    km chat -q "Hello"            Single query mode
    km -c                         Resume the most recent session
    km -c "my project"            Resume a session by name (latest in lineage)
    km --resume <session_id>      Resume a specific session by ID
    km setup                      Run setup wizard
    km logout                     Clear stored authentication
    km auth add <provider>        Add a pooled credential
    km auth list                  List pooled credentials
    km auth remove <p> <t>        Remove pooled credential by index, id, or label
    km auth reset <provider>      Clear exhaustion status for a provider
    km model                      Select default model
    km config                     View configuration
    km config edit                Edit config in $EDITOR
    km config set model gpt-4     Set a config value
    km gateway                    Run messaging gateway
    km -s kunming-agent-dev,github-auth
    km -w                         Start in isolated git worktree
    km gateway install            Install gateway background service
    km sessions list              List past sessions
    km sessions browse            Interactive session picker
    km sessions rename ID T       Rename/title a session
    km logs                       View agent.log (last 50 lines)
    km logs -f                    Follow agent.log in real time
    km logs errors                View errors.log
    km logs --since 1h            Lines from the last hour
    km update                     Update to latest version
...
"""
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with Kunming Agent"
    )
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: kunming-cli)"
    )
    model_parser.add_argument(
        "--scope",
        default=None,
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Kunming Agent with an interactive wizard. "
                    "Run a specific section: km setup model|terminal|gateway|tools|agent"
    )
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for Kunming CLI"
    )
    ...
    login_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use (default: kunming-cli)"
    )
    login_parser.add_argument(
        "--scope",
        default=None,
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Kunming Agent components"
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Kunming Agent setup"
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Kunming Agent configuration"
    )
    ...
    def cmd_pairing(args):
        from kunming_cli.pairing import pairing_command
    ...
    def cmd_skills(args):
        ...
        if getattr(args, 'skills_action', None) == 'config':
            ...
            from kunming_cli.skills_config import skills_command as skills_config_command
            skills_config_command(args)
        else:
            from kunming_cli.skills_hub import skills_command
            skills_command(args)
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/kunming-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
    ...
    def cmd_plugins(args):
        from kunming_cli.plugins_cmd import plugins_command
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            from kunming_cli.memory_setup import memory_command
            memory_command(args)
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'km tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # km tools disable <name...> [--platform cli]
    ...
    # km tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in ("list", "disable", "enable"):
            from kunming_cli.tools_config import tools_disable_enable_command
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Kunming as an MCP server",
        description=(
            "Manage MCP server connections and run Kunming as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'km mcp add' to connect to a new server, or\n"
            "'km mcp serve' to expose Kunming conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Kunming as an MCP server (expose conversations to other agents)",
    )
    ...
    def cmd_mcp(args):
        from kunming_cli.mcp_config import mcp_command
        mcp_command(args)
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch km --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            import shutil
            kunming_bin = shutil.which("km")
            if kunming_bin:
                os.execvp(kunming_bin, ["km", "--resume", selected_id])
            else:
                # Fallback: re-invoke via python -m
                os.execvp(
                    sys.executable,
                    [sys.executable, "-m", "kunming_cli.main", "--resume", selected_id],
                )
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Kunming Agent to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Kunming Agent",
        description="Remove Kunming Agent from your system. Can keep configs/data for reinstall."
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Kunming Agent as an ACP (Agent Client Protocol) server",
        description="Start Kunming Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )

    def cmd_acp(args):
        """Launch Kunming Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles ?multiple isolated Kunming instances",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter Kunming log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    km logs                        Show last 50 lines of agent.log
    km logs -f                     Follow agent.log in real time
    km logs errors                 Show last 50 lines of errors.log
    km logs gateway -n 100         Show last 100 lines of gateway.log
    km logs --level WARNING        Only show WARNING and above
    km logs --session abc123       Filter by session ID
    km logs --since 1h             Lines from the last hour
    km logs --since 30m -f         Follow, starting from 30 min ago
    km logs list                   List available log files with sizes
""",
    )
    ...
    # e.g. ``km -c Pokemon Agent Dev`` ?``km -c 'Pokemon Agent Dev'``
