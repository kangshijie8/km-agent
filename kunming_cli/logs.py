"""Kunming Agent log viewer and management."""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_kunming_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when Kunming runs — try 'kunming chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_kunming_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_kunming_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_kunming_home()}/logs/")
        return

    print(f"Log files in {display_kunming_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'km chat' to generate logs)")
