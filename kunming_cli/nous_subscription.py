...
from kunming_cli.auth import get_nous_auth_status
from kunming_cli.config import get_env_value, load_config
...
def get_nous_subscription_explainer_lines() -> list[str]:
    ...
    return [
        ...
        "Change these later with: km setup tools, km setup terminal, or km status.",
    ]


def apply_nous_provider_defaults(config: Dict[str, object]) -> set[str]:
    """Apply provider-level Nous defaults shared by `km setup` and `km model`."""
    if not managed_nous_tools_enabled():
        return set()
