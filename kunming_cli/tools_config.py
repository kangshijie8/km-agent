"""
Unified tool configuration for Kunming Agent.

`km tools` and `km setup tools` both enter this module.
Select a platform ->toggle toolsets on/off ->for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.kunming/config.yaml under
...
CONFIGURABLE_TOOLSETS = [
    ("web",             "ð Web Search & Scraping",    "web_search, web_extract"),
    ("browser",         "ð Browser Automation",       "navigate, click, type, scroll"),
    ("terminal",        "ð» Terminal & Processes",      "terminal, process"),
    ("file",            "ð File Operations",           "read, write, patch, search"),
    ("code_execution",  "[ON]Code Execution",            "execute_code"),
    ("vision",          "ðï¸? Vision / Image Analysis",  "vision_analyze"),
    ("image_gen",       "ð¨ Image Generation",          "image_generate"),
    ("moa",             "ð§  Mixture of Agents",         "mixture_of_agents"),
    ("tts",             "ð Text-to-Speech",            "text_to_speech"),
    ("skills",          "ð Skills",                    "list, view, manage"),
    ("todo",            "ð Task Planning",             "todo"),
    ("memory",          "ð¾ Memory",                    "persistent memory across sessions"),
    ("session_search",  "ð Session Search",            "search past conversations"),
    ("clarify",         "[X]Clarifying Questions",      "clarify"),
    ("delegation",      "ð¥ Task Delegation",           "delegate_task"),
    ("cronjob",         "â?Cron Jobs",                 "create/list/update/pause/resume/run, with optional attached skills"),
    ("rl",              "ð§ª RL Training",               "Tinker-Atropos training tools"),
    ("homeassistant",    "ð  Home Assistant",           "smart home device control"),
]

# Toolsets that are OFF by default for new installs.
# They're still in _KUNMING_CORE_TOOLS (available at runtime if enabled),
...
def _get_effective_configurable_toolsets():
    ...
    try:
        from kunming_cli.plugins import discover_plugins, get_plugin_toolsets
    ...
    if post_setup_key in ("agent_browser", "browserbase"):
        ...
        if not node_modules.exists() and shutil.which("npm"):
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_kunming_home()}/kunming-agent && npm install")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in kunming-agent directory)")
...
def _get_platform_tools(
    ...
    *,
    ...
    # "kunming-cli" (which include all _KUNMING_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # (e.g. "kunming-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
    ...
    # A plugin toolset is "known" for a platform once `km tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by km tools -default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (kunming-cli, kunming-telegram, etc.)
    ...
    if not env_vars:
        ...
        if managed_feature:
            ...
            if any(get_env_value(env_var) for env_var in override_envs):
                _print_warning(
                    "  Direct credentials are still configured and may take precedence until you remove them from ~/.kunming/.env."
                )
        return
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `km tools` and `km setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print(color("[ON]Tool Summary", Colors.CYAN, Colors.BOLD))
        print()
        ...
        print()
        return
    print(color("[ON]Kunming Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://kunming-agent.kunming.dev/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    print()
    from kunming_constants import display_kunming_home
    print(color(f"  Tool configuration saved to {display_kunming_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'km' or gateway restart.", Colors.DIM))
    print()
