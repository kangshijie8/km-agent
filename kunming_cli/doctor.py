"""Doctor command for km CLI.

Diagnoses issues with Kunming Agent setup."""
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.kunming/.env contains provider auth/base URL settings."""
...
def _check_gateway_service_linger(issues: list[str]) -> None:
    ...
    print()
    print(color("oGateway Service", Colors.CYAN, Colors.BOLD))
...
def run_doctor(args):
    ...
    print(color("+                ð©º Kunming Doctor                        +", Colors.CYAN))
    print(color("â---------------------------------------------------------+", Colors.CYAN))
    ...
    # =========================================================================
    print()
    print(color("oPython Environment", Colors.CYAN, Colors.BOLD))
    ...
    # =========================================================================
    print()
    print(color("oRequired Packages", Colors.CYAN, Colors.BOLD))
    ...
    # =========================================================================
    print()
    print(color("oConfiguration Files", Colors.CYAN, Colors.BOLD))
    
    # Check ~/.kunming/.env (primary location for user config)
    env_path = KUNMING_HOME / '.env'
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'km setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'km setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'km setup' to create one")
                issues.append("Run 'km setup' to create .env")
    
    # Check ~/.kunming/config.yaml (primary) or project cli-config.yaml (fallback)
    ...
    if config_path.exists():
        try:
            from kunming_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'km setup' to migrate config")
                else:
                    issues.append("Run 'km doctor --fix' or 'km setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'km doctor --fix'")
        except Exception:
            pass
        ...
        try:
            ...
            if config_issues:
                print()
                print(color("oConfig Structure", Colors.CYAN, Colors.BOLD))
    ...
    # =========================================================================
    print()
    print(color("oAuth Providers", Colors.CYAN, Colors.BOLD))

    try:
        from kunming_cli.auth import get_nous_auth_status, get_codex_auth_status
    ...
    # =========================================================================
    print()
    print(color("oDirectory Structure", Colors.CYAN, Colors.BOLD))
    
    kunming_home = KUNMING_HOME
    if kunming_home.exists():
        check_ok(f"{_DHH} directory exists")
    else:
        if should_fix:
            kunming_home.mkdir(parents=True, exist_ok=True)
    ...
    for subdir_name in expected_subdirs:
        subdir_path = kunming_home / subdir_name
    ...
    soul_path = kunming_home / "SOUL.md"
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give Kunming a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# Kunming Agent Persona\n\n"
                "<!-- Edit this file to customize how Kunming communicates. -->\n\n"
                "You are Kunming, a helpful AI assistant.\n",
                encoding="utf-8",
            )
    ...
    state_db_path = kunming_home / "state.db"
    ...
    wal_path = kunming_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'km doctor --fix' to checkpoint")
    ...
    # =========================================================================
    print()
    print(color("oExternal Tools", Colors.CYAN, Colors.BOLD))
    ...
    # =========================================================================
    print()
    print(color("oAPI Connectivity", Colors.CYAN, Colors.BOLD))
    ...
    if openrouter_key:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"\r  {color('[OK]', Colors.RED)} OpenRouter API {color('(invalid API key)', Colors.DIM)}                ")
                issues.append("Check OPENROUTER_API_KEY in .env")
            else:
                print(f"\r  {color('[OK]', Colors.RED)} OpenRouter API {color(f'(HTTP {response.status_code})', Colors.DIM)}                ")
        except Exception as e:
            print(f"\r  {color('[OK]', Colors.RED)} OpenRouter API {color(f'({e})', Colors.DIM)}                ")
    ...
    if anthropic_key:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"\r  {color('[OK]', Colors.RED)} Anthropic API {color('(invalid API key)', Colors.DIM)}                 ")
            else:
                msg = "(couldn't verify)"
                print(f"\r  {color('[ON]', Colors.YELLOW)} Anthropic API {color(msg, Colors.DIM)}                 ")
        except Exception as e:
            print(f"\r  {color('[ON]', Colors.YELLOW)} Anthropic API {color(f'({e})', Colors.DIM)}                 ")
    ...
    _apikey_providers = [
        ...
        # MiniMax APIs don't support /models endpoint -https://github.com/kangshijie8/KM-Agent/issues/811
        ...
        if _key:
            ...
            try:
                ...
                if _resp.status_code == 200:
                    ...
                    print(f"\r  {color('[OK]', Colors.RED)} {_label} {color('(invalid API key)', Colors.DIM)}           ")
                    issues.append(f"Check {_env_vars[0]} in .env")
                else:
                    print(f"\r  {color('[ON]', Colors.YELLOW)} {_label} {color(f'(HTTP {_resp.status_code})', Colors.DIM)}           ")
            except Exception as _e:
                print(f"\r  {color('[ON]', Colors.YELLOW)} {_label} {color(f'({_e})', Colors.DIM)}           ")

    # =========================================================================
    # Check: Submodules
    # =========================================================================
    print()
    print(color("oSubmodules", Colors.CYAN, Colors.BOLD))
    ...
    # =========================================================================
    print()
    print(color("oTool Availability", Colors.CYAN, Colors.BOLD))
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'km setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    # =========================================================================
    print()
    print(color("oSkills Hub", Colors.CYAN, Colors.BOLD))

    hub_dir = KUNMING_HOME / "skills" / ".hub"
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: km skills list)")
    ...
    # =========================================================================
    print()
    print(color("oMemory Provider", Colors.CYAN, Colors.BOLD))

    _active_memory_provider = ""
    try:
        import yaml as _yaml
        _mem_cfg_path = KUNMING_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                check_warn("Honcho config not found", "run: km memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                check_fail("Honcho API key or base URL not set", "run: km memory setup")
                issues.append("No Honcho API key — run 'km memory setup'")
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                check_fail("Mem0 API key not set", "(set MEM0_API_KEY in .env or run km memory setup)")
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: km memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: km memory setup")
        except Exception as _e:
    ...
    # =========================================================================
    try:
        from kunming_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re

        named_profiles = [p for p in list_profiles() if not p.is_default]
        if named_profiles:
            print()
            print(color("oProfiles", Colors.CYAN, Colors.BOLD))
            ...
            for p in named_profiles:
                ...
                if not (p.path / "config.yaml").exists():
                    parts.append("[ON]missing config")
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "km -p" in content:
                            _m = _re.search(r"km -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'km doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("-" * 60, Colors.GREEN))
