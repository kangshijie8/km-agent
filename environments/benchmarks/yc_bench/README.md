...
The environment initialises the simulation via `yc-bench sim init` (NOT `yc-bench run`, which would start yc-bench's own built-in agent loop). Our `KunmingAgentLoop` then drives all interaction through CLI commands.
