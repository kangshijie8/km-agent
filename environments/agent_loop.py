"""Kunming Agent loop runner for environments.

Runs the kunming-agent tool-calling loop using standard OpenAI-spec tool calling.
...
identical to kunming-agent's run_agent.py. Tool execution is dispatched via
...
# Resized at runtime by KunmingAgentBaseEnv.__init__ via resize_tool_pool().
...
def resize_tool_pool(max_workers: int):
    ...
    Called by KunmingAgentBaseEnv.__init__ based on config.tool_pool_size.
...
class KunmingAgentLoop:
    """
    Runs kunming-agent's tool-calling loop using standard OpenAI-spec tool calling.
    ...
    async def run(self, messages: List[Dict[str, Any]]) -> AgentResult:
        ...
        for turn in range(self.max_turns):
            ...
            # kunming-agent's standalone parsers. This handles the case where
            ...
            if (
                ...
                try:
                    ...
                    fallback_parser = get_parser("kunming")
            ...
            if assistant_msg.tool_calls:
                ...
                # Execute each tool call via kunming-agent's dispatch
