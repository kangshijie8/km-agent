"""
KunmingAgentBaseEnv -- Abstract Base Environment for Kunming-Agent + Atropos

Provides the Atropos integration plumbing that all kunming-agent environments share:
- Two-mode operation (OpenAI server for Phase 1, VLLM ManagedServer for Phase 2)
- Per-group toolset/distribution resolution
- Agent loop orchestration via KunmingAgentLoop
...
# Ensure the kunming-agent repo root is on sys.path so that imports like
...
# Load API keys from kunming-agent/.env so all environments can access them
...
from environments.agent_loop import AgentResult, KunmingAgentLoop
...
# Import kunming-agent toolset infrastructure
...
class KunmingAgentEnvConfig(BaseEnvConfig):
    """
    Configuration for kunming-agent Atropos environments.
    ...
    enabled_toolsets: Optional[List[str]] = Field(
        default=None,
        description="Explicit list of kunming toolsets to enable (e.g., ['terminal', 'file', 'web']). "
    ...
    tool_call_parser: str = Field(
        ...
        "Options: kunming, mistral, llama3_json, qwen, deepseek_v3, etc.",
    )
    ...
    default_result_size_chars: int = Field(
        ...
        "to sandbox. Results exceeding this are written to /tmp/kunming-results/ "
...
class KunmingAgentBaseEnv(BaseEnv):
    """
    Abstract base environment for kunming-agent Atropos integration.
    ...
    name: Optional[str] = "kunming-agent"
    env_config_cls = KunmingAgentEnvConfig

    def __init__(
        self,
        config: KunmingAgentEnvConfig,
        ...
        # Set terminal environment variables so kunming tools pick them up.
    ...
    async def compute_reward(
        ...
        - ctx: ToolContext -- call ANY kunming-agent tool (terminal, file, web,
