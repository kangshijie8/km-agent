# Kunming Agent Atropos Environments

This directory contains the integration layer between **kunming-agent's** tool-calling capabilities and the **Atropos** RL training framework. It provides everything needed to run agentic LLMs through multi-turn tool-calling loops, score their output with arbitrary reward functions, and feed results into Atropos for training or evaluation.
...
```
                    ...
                    └───────────┬───────────┘
                                │ inherits
                    ┌───────────┴───────────┐
                    │  KunmingAgentBaseEnv   │  kunming_base_env.py
                    │  - Terminal backend    │
                    │  - Tool resolution     │
                    │  - Agent loop          │
                    │  - ToolContext          │
              ...
              ┌─────────────────┼─────────────────┐
              │                 │                  │
     TerminalTestEnv     KunmingSweEnv    TerminalBench2EvalEnv
...
**KunmingAgentBaseEnv** (`kunming_base_env.py`) extends BaseEnv with kunming-agent specifics:
- Sets `os.environ["TERMINAL_ENV"]` to configure the terminal backend (local, docker, modal, daytona, ssh, singularity)
- Resolves kunming-agent toolsets via `_resolve_tools_for_group()` (calls `get_tool_definitions()` which queries `tools/registry.py`)
- Implements `collect_trajectory()` which runs the full agent loop and computes rewards
- Supports two-phase operation (Phase 1: OpenAI server, Phase 2: VLLM ManagedServer)
- Applies monkey patches for async-safe tool operation at import time

Concrete environments inherit from `KunmingAgentBaseEnv` and implement:
...
`ToolContext` is a per-rollout handle that gives reward/verification functions direct access to **all** kunming-agent tools, scoped to the rollout's `task_id`. The same `task_id` means the terminal/browser session is the SAME one the model used during its rollout -- all state (files, processes, browser tabs) is preserved.
...
- **Generic**: `call_tool(name, args)` -- call any kunming-agent tool by name
- **Cleanup**: `cleanup()` -- release all resources (called automatically after `compute_reward`)

### Patches (`patches.py`)

**Problem**: Some kunming-agent tools use `asyncio.run()` internally (e.g., the Modal backend). This crashes when called from inside Atropos's event loop because `asyncio.run()` cannot be nested.
...
Available parsers:
- `kunming` -- ChatML `<tool_call>` XML format (Kunming format)
...
├── kunming_base_env.py           # Abstract base (KunmingAgentBaseEnv)
...
│
├── kunming_swe_env/               # SWE-bench style training environment
│   └── kunming_swe_env.py
│
...
2. Create your env file inheriting from `KunmingAgentBaseEnv`
...
2. Inherit from `KunmingAgentBaseEnv`
...
|-------|-------------|---------|
| `enabled_toolsets` | Which km toolsets to enable | `None` (all) |
...
| `tool_call_parser` | Parser name for Phase 2 | `kunming` |
