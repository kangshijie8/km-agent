...
_DEFAULT_CONTAINER_TAG = "kunming"
...
def _load_supermemory_config(kunming_home: str) -> dict:
    config = _default_config()
    config_path = Path(kunming_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, kunming_home: str) -> None:
    config_path = Path(kunming_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `km memory setup`.
        # All other options are documented for $KUNMING_HOME/supermemory.json
    ...
    def save_config(self, values, kunming_home):
        ...
        _save_supermemory_config(sanitized, kunming_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from kunming_constants import get_kunming_home
        self._kunming_home = kwargs.get("kunming_home") or str(get_kunming_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._kunming_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "kunming", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "kunming_memory", "target": target, "type": "explicit_memory"},
