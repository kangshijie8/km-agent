...
Config via environment variables:
  ...
  HINDSIGHT_BANK_ID   -memory bank identifier (default: kunming)
  HINDSIGHT_BUDGET    -recall budget: low/mid/high (default: mid)
  HINDSIGHT_API_URL   -API endpoint
  HINDSIGHT_MODE      -cloud or local (default: cloud)

Or via $KUNMING_HOME/hindsight/config.json (profile-scoped), falling back to
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $KUNMING_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_kunming_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "kunming": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "kunming"),
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Kunming Agent and the User"
    ...
    def save_config(self, values, kunming_home):
        """Write config to $KUNMING_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(kunming_home) / "hindsight"
    ...
    def post_setup(self, kunming_home: str, config: dict) -> None:
        ...
        from kunming_cli.memory_setup import _curses_select
        ...
        if not uv_path:
            print("  [ON]uv not found -install it: curl -LsSf https://astral.sh/uv/install.sh | sh")
        ...
        provider_config["bank_id"] = "kunming"
        provider_config["recall_budget"] = "mid"
        bank_id = "kunming"
        config["memory"]["provider"] = "hindsight"
        save_config(config)

        self.save_config(provider_config, kunming_home)

        if env_writes:
            env_path = Path(kunming_home) / ".env"
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name", "default": "kunming"},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Kunming Agent and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "kunming"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "kunming"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = self._config.get("banks", {}).get("kunming", {})
        self._bank_id = self._config.get("bank_id") or banks.get("bankId", "kunming")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Kunming Agent and the User")
