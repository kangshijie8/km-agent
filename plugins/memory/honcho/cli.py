...
Handles: km honcho setup | status | sessions | map | peer
"""
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "kunming" else ""
...
def cmd_sync(args) -> None:
    ...
    Scans all Kunming profiles and creates host blocks for any that don't
    have one yet. Inherits settings from the default host block.
    """
    try:
        from kunming_cli.profiles import list_profiles
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'km honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'km honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `km update` -- no output, no exceptions.
    """
    try:
        from kunming_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Kunming profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $KUNMING_HOME/honcho.json so each profile/instance gets
    ...
    """
    return get_kunming_home() / "honcho.json"
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "-" * 40)
    print("  Honcho gives Kunming persistent cross-session memory.")
    ...
    kunming_host = hosts.setdefault(_host_key(), {})
    ...
    if is_local:
        ...
        if not cfg.get("apiKey"):
            ...
            print("  Run 'km honcho setup' again once you have a key.\n")
            return

    # --- 3. Identity ---
    current_peer = kunming_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        kunming_host["peerName"] = new_peer

    current_ai = kunming_host.get("aiPeer") or cfg.get("aiPeer", "kunming")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        kunming_host["aiPeer"] = new_ai

    current_workspace = kunming_host.get("workspace") or cfg.get("workspace", "kunming")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        kunming_host["workspace"] = new_workspace

    # --- 4. Observation mode ---
    current_obs = kunming_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in ("unified", "directional"):
        kunming_host["observationMode"] = new_obs
    else:
        kunming_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(kunming_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        kunming_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        kunming_host["writeFrequency"] = new_wf if new_wf in ("async", "turn", "session") else "async"

    # --- 6. Recall mode ---
    _raw_recall = kunming_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in ("hybrid", "context", "tools"):
        kunming_host["recallMode"] = new_recall

    # --- 7. Session strategy ---
    current_strat = kunming_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-directory")
    print("\n  Session strategy:")
    ...
    if new_strat in ("per-session", "per-repo", "per-directory", "global"):
        kunming_host["sessionStrategy"] = new_strat

    kunming_host["enabled"] = True
    kunming_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        kunming_config = load_config()
        kunming_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(kunming_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}")
        print("  Run: km config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    km honcho status     -- show full config")
    print("    km honcho mode       -- change recall/observation mode")
    print("    km honcho tokens     -- tune context and dialectic budgets")
    print("    km honcho peer       -- update peer names")
    print("    km honcho map <name> -- map this directory to a session name\n")


def _active_profile_name() -> str:
    """Return the active Kunming profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: km honcho setup\n")
        return
    ...
    if not cfg:
        ...
        print("  Run 'km honcho setup' to configure.\n")
        return
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: km honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        kunming = hosts.get(_host_key(), {})
        user = kunming.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = kunming.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = kunming.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = kunming.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "-" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    Kunming' identity in Honcho. Seed with 'km honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print(f"\n  Set with: km honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = kunming.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = kunming.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = kunming.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "-" * 40)
        print()
        ...
        print("    AI-to-AI inference. Kunming asks Honcho's AI peer a question")
        ...
        print("\n  Set with: km honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'km honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'km honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    km honcho identity --show        -show both peer representations")
        print("    km honcho identity <file>        -seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'km memory setup'...\n")
        from kunming_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``km honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``km honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to km memory setup)",
    )
