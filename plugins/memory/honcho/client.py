"""Kunming memory provider for Honcho.

Resolution order for config file:
  1. $KUNMING_HOME/honcho.json  (instance-local, enables isolated Kunming instances)
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active Kunming profile.

    Resolution order:
      1. KUNMING_HONCHO_HOST env var (explicit override)
      2. Active profile name via profiles system -> ``kunming.<profile>``
      3. Fallback: ``"kunming"`` (default profile)
    """
    explicit = os.environ.get("KUNMING_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $KUNMING_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.kunming/honcho.json          (default profile -shared host blocks live here)
      3. ~/.honcho/config.json          (global, cross-app interop)

    Returns the global path if none exist (for first-time setup writes).
    """
    local_path = get_kunming_home() / "honcho.json"
...
class HonchoClientConfig:
    ...
    # Max chars of dialectic result to inject into Kunming system prompt
    ...
    # True when Honcho was explicitly configured for this host (hosts.kunming
    ...
    def from_global_config(
        ...
        Resolution: $KUNMING_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        When host is None, derives it from the active Kunming profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # A hosts.kunming block or explicit enabled flag means the user
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. Kunming session title (from /title command)
          3. per-session strategy -Kunming session_id ({timestamp}_{hex})
        ...
        # per-session: inherit Kunming session_id (new Honcho session each run)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'km honcho setup' or set HONCHO_API_KEY. "
    ...
    if not resolved_base_url:
        try:
            ...
            kunming_cfg = load_config()
            honcho_cfg = kunming_cfg.get("honcho", {})
