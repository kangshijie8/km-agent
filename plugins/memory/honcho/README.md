...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/kunming>
...
```bash
km honcho setup    # full interactive wizard (cloud or local)
km memory setup    # generic picker, also works
```

Or manually:
```bash
km config set memory.provider honcho
echo "HONCHO_API_KEY=your-key" >> ~/.kunming/.env
```
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$KUNMING_HOME/honcho.json` | Profile-local (isolated Kunming instances) |
| 2 | `~/.kunming/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Kunming profile: `km` (default) or `km.<profile>`.
...
Host key derivation: `KUNMING_HONCHO_HOST` env > active profile (`km.<profile>`) > `"km"`.
...
| `KUNMING_HONCHO_HOST` | Host key override |

## CLI Commands

| Command | Description |
|---------|-------------|
| `km honcho setup` | Full interactive setup wizard |
| `km honcho status` | Show resolved config for active profile |
| `km honcho enable` / `disable` | Toggle Honcho for active profile |
| `km honcho mode <mode>` | Change recall or observation mode |
| `km honcho peer --user <name>` | Update user peer name |
| `km honcho peer --ai <name>` | Update AI peer name |
| `km honcho tokens --context <N>` | Set context token budget |
| `km honcho tokens --dialectic <N>` | Set dialectic max chars |
| `km honcho map <name>` | Map current directory to a session name |
| `km honcho sync` | Create host blocks for all Kunming profiles |
