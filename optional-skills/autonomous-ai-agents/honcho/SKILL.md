---
name: honcho
description: Configure and use Honcho memory with Kunming -- cross-session user modeling, multi-profile peer isolation, observation config, and dialectic reasoning. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation and recall settings.
version: 1.0.0
...
metadata:
  kunming:
    ...
    related_skills: [kunming-agent]
prerequisites:
  pip: [honcho-ai]
---

# Honcho Memory for Kunming

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Kunming profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/kunming#running-honcho-locally-with-kunming

### Verify

```bash
kunming honcho status    # shows resolved config, connection test, peer info
```
...
Honcho models conversations as interactions between **peers**. Kunming creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Kunming instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Kunming run |
| `global` | Single session across all directories |

Manual override: `kunming honcho map my-project-name`
...
Each Kunming profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
```bash
kunming profile create coder --clone
# creates host block kunming.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `kunming.coder` host block in `honcho.json`
...
```bash
kunming honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Config file: `$KUNMING_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `kunming honcho setup`. Ensure `memory.provider: honcho` is in `~/.kunming/config.yaml`.

### Memory not persisting across sessions
Check `kunming honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `kunming profile create <name> --clone`. For existing profiles: `kunming honcho sync`.
...
| Command | Description |
|---------|-------------|
| `kunming honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `kunming honcho status` | Show resolved config, connection test, peer info for active profile |
| `kunming honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `kunming honcho disable` | Disable Honcho for the active profile |
| `kunming honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `kunming honcho peers` | Show peer identities across all profiles |
| `kunming honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `kunming honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `kunming honcho sessions` | List known directory-to-session-name mappings |
| `kunming honcho map <name>` | Map current working directory to a Honcho session name |
| `kunming honcho identity` | Seed AI peer identity or show both peer representations |
| `kunming honcho sync` | Create host blocks for all Kunming profiles that don't have one yet |
| `kunming honcho migrate` | Step-by-step migration guide from legacy memory to Kunming + Honcho |
| `kunming memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `kunming memory status` | Show active memory provider and config |
| `kunming memory off` | Disable external memory provider |
