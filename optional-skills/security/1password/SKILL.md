...
version: 1.0.0
author: arceus77-7, enhanced by Kunming Agent
license: MIT
metadata:
  kunming:
...
- `tmux` available for stable authenticated sessions during Kunming terminal calls (desktop app flow only)
...
### Service Account (recommended for Kunming)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.kunming/.env` (the skill will prompt for this on first load).
...
## Kunming Execution Pattern (desktop app flow)

Kunming terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/kunming-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/kunming-op.sock"
