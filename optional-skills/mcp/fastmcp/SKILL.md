...
- smoke-test a server with the FastMCP CLI before wiring it into Kunming or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Kunming. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.kunming/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.kunming/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Kunming integration, either:

- configure the server in `~/.kunming/config.yaml` using the `native-mcp` skill, or
...
### Kunming cannot see the deployed server

The server-building part may be correct while the Kunming config is not. Load the `native-mcp` skill and configure the server in `~/.kunming/config.yaml`, then restart Kunming.
